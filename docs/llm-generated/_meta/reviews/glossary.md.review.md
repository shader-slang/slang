...
| --- | --- | --- | --- | --- | --- |
| F-001 | minor | `## Terms` | The glossary is not strictly alphabetically ordered: `differential pair` appears before `DiagnosticSink`, but `DiagnosticSink` should sort first. | `docs/llm-generated/_meta/prompts/glossary.md` requires an alphabetically ordered glossary. | Move `DiagnosticSink` before `differential pair` and re-scan for similar ordering issues. |
