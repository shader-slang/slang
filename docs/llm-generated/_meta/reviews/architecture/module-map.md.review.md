...
| --- | --- | --- | --- | --- | --- |
| F-001 | major | multiple tables | The page lists many `.cpp` implementation files even though the manifest watched set for this doc includes CMake files, `prelude/*.h`, and `slang-*.h` headers, not arbitrary `.cpp` files. | `docs/llm-generated/_meta/manifest.yaml` watched paths for `architecture/module-map.md` exclude `source/slang/*.cpp`. | Either expand watched paths to include the cited `.cpp` files or rewrite rows to cite watched headers and CMake paths. |
