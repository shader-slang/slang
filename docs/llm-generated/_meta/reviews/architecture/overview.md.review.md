...
| F-002 | major | `## Top-level decomposition` | The prompt requires `tools/`, `tests/`, `extras/`, and `external/`; the page only mentions `tools/`. | `docs/llm-generated/_meta/prompts/architecture-overview.md` requires the auxiliary trees. | Add concise bullets for `tests/`, `extras/`, and `external`. |
