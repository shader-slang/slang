...
| --- | --- | --- | --- | --- | --- |
| F-001 | major | `## Algorithm` and edge cases | Required deduplication behavior is not covered, and source `AddToLookupResult` appends results without deduping by `DeclRef`. | `docs/llm-generated/_meta/prompts/name-resolution-lookup.md` requires deduplication coverage; `source/slang/slang-lookup.cpp:95-125` appends lookup items. | Document the actual no-dedupe accumulation behavior or identify the real dedupe site if elsewhere in watched files. |
