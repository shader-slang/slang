...
- [slang-emit-hlsl-prelude.cpp](../../../../source/slang/slang-emit-hlsl-prelude.cpp)
  ...
  _named_ constants and type spellings:
...
- `validateBarrierFlagsForHLSL` runs (line 1735), gated on
  ...
  This is a _diagnostic_ pass, not a transform, and it returns
...
| -   | _(skip)_ `lowerCooperativeVectors`             | [slang-ir-lower-coopvec.cpp](../../../../source/slang/slang-ir-lower-coopvec.cpp)                                                   | HLSL is the explicit `case HLSL: break;` arm at line 1685.                                                                    | DXC handles cooperative vectors directly.                                                                                                                                                               |
...
2739 — but validation and optimization _of the emitted HLSL_ is
...
- Immediately _before_ the pragma, and only when emit found an
...
HLSL source spelling is a name: the IR carries the _name_ (an
...
a different pass. A matrix reached through a _buffer_ element type
is rewritten by `lowerBufferElementTypeToStorageType` (Phase C row 21) into a storage struct with a single `data` field, whose name is
built at
...
The rewrite _is_ visible in the emitted text. `kIROp_MakeStruct` is
...
- A vector result — the shape the `vector<bool,N>` cast above
  ...
  _and_ `isTargetHLSL2018`; when either fails the emitter returns
