...
counterpart and is _not_ emitted; Metal normally takes the
...
- Entry-point varying-param legalization via
  ...
  **both** `Stage::Vertex` and `Stage::Fragment` (it previously ran
  ...
  the _original_ function, the wrapper-swap calls
...
2. **`Metal`** — the main invocation at line ~2476 (Phase C),
   ...
   in `device` buffers into arrays of _packed_ vectors —
...
it (lines 1181-1187), which _rejects_ out-of-contract widths with
`Diagnostics::CoverageCounterWidthBytesInvalid`. First, the cap
adjusts a _valid_ width to a platform limit rather than reporting
