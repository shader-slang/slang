...
watched_paths_digest: 343b3f944e7e2f5ed2341edd1e8c7884fed246c010d2d150f6d8d8fbaeb583fa
...
([slang-code-gen.cpp lines 1169-1189](../../../../source/slang/slang-code-gen.cpp)),
...
([slang-code-gen.cpp lines 1104-1183](../../../../source/slang/slang-code-gen.cpp)),
...
is not the subject of this page: it diverges at line 2542 of
...
- [slang-emit.cpp](../../../../source/slang/slang-emit.cpp)
  ...
  `emitSPIRVForEntryPointsDirectly` (line 3670) is the SPIR-V
  entry point; `createArtifactFromIR` (line 3436) wraps the
...
- [slang-emit-spirv.cpp](../../../../source/slang/slang-emit-spirv.cpp)
  — `emitSPIRVFromIR` (line 12161) calls `legalizeIRForSPIRV`,
...
- [slang-ir-spirv-legalize.cpp](../../../../source/slang/slang-ir-spirv-legalize.cpp)
  — `legalizeIRForSPIRV` (line 3352) is the top-level legalizer;
  `legalizeSPIRV` (line 3113) drives `SPIRVLegalizationContext::processModule`;
  `simplifyIRForSpirvLegalization` (line 3126) is the iterative
...
| 7 | `instrumentCoverage` | [slang-ir-coverage-instrument.cpp](../../../../source/slang/slang-ir-coverage-instrument.cpp) | `reqSet.coverageTracing` | Writes coverage metadata via the `ArtifactPostEmitMetadata` pointer created at line 1398. Now also passed a `counterByteWidth` (default `kDefaultCoverageCounterByteWidth`, overridable to 4 via `TraceCoverageCounterByteWidth`; the API path re-validates 4/8 and fails with `CoverageCounterWidthBytesInvalid` otherwise) and a `coverageBoolean` flag (from `TraceCoverageBoolean`, off by default). |
...
| 10 | `addDenormalModeDecorations` | [slang-emit.cpp](../../../../source/slang/slang-emit.cpp) | (always) | Static helper inside `slang-emit.cpp` (line 786). |
...
| 28 | `lowerSumVectorMatrixInsts` | [slang-emit.cpp](../../../../source/slang/slang-emit.cpp) | `reqSet.sumVectorMatrix` | Helper at line 909. Gate added by #11917/#12088; `kIROp_SumVectorElements` / `kIROp_SumMatrixElements` are produced only by the autodiff transpose pass, which runs before the second `calcRequiredLoweringPassSet` scan. |
...
| 14 | `introduceExplicitGlobalContext` | [slang-ir-explicit-global-context.cpp](../../../../source/slang/slang-ir-explicit-global-context.cpp) | `getBoolOption(EnableExperimentalPasses)` | Only fires under the experimental flag for SPIR-V (line 2337). |
| 15 | `transformParamsToConstRef` | [slang-ir-transform-params-to-constref.cpp](../../../../source/slang/slang-ir-transform-params-to-constref.cpp) | (`SPIRV` / `SPIRVAssembly` arm, line 2338) | |
...
| 41 | `simplifyNonSSAIR` | [slang-ir-ssa-simplification.cpp](../../../../source/slang/slang-ir-ssa-simplification.cpp) | (always) | After phi elimination; line 2627. |
...
| 46 | `checkUnsupportedInst` | [slang-ir-check-unsupported-inst.cpp](../../../../source/slang/slang-ir-check-unsupported-inst.cpp) | `!shouldPerformMinimumOptimizations()` | Last `SLANG_PASS` in `linkAndOptimizeIR`, line 2746. |
...
`emitSPIRVForEntryPointsDirectly` (line 3670 of
[slang-emit.cpp](../../../../source/slang/slang-emit.cpp)). The
SPIR-V backend in `emitSPIRVFromIR` (line 12280 of
[slang-emit-spirv.cpp](../../../../source/slang/slang-emit-spirv.cpp))
calls the top-level `legalizeIRForSPIRV` (line 3352 of
[slang-ir-spirv-legalize.cpp](../../../../source/slang/slang-ir-spirv-legalize.cpp))
...
`createArtifactFromIR` (line 3436): `spirv-link` for embedded-
...
| 13 | `diagnoseConflictingDescriptorHeapStrideOptions` | [slang-emit-spirv.cpp](../../../../source/slang/slang-emit-spirv.cpp) | (always; only diagnoses on conflict) | Direct call after the forward-pointer loop ([slang-emit-spirv.cpp line 12337](../../../../source/slang/slang-emit-spirv.cpp); the member is defined at line 7586). Re-checks the compile-API path for the `SPIRVUnifiedDescriptorHeapStride` + non-zero `SPIRVResourceHeapStride` conflict the CLI rejects at option-parse time; emits `SpirvConflictingDescriptorHeapStrideOptions`. See Notable passes. |
...
| 15 | `emitFrontMatter` | [slang-emit-spirv.cpp](../../../../source/slang/slang-emit-spirv.cpp) | (always) | Lines 1657-1687. Emits exactly two instructions unconditionally: `OpCapability Shader`, then `OpMemoryModel` with the module's addressing model (`Logical` unless `requirePhysicalStorageAddressing` moved it) and memory model — printing as `OpMemoryModel Logical GLSL450` on a default Vulkan compile. Called *last* (line 12478) but writing into the `Capabilities` / `MemoryModel` logical sections, so `emitPhysicalLayout` still flattens it to the front. Nothing else is unconditional front matter: `OpSource` comes from `emitSource` in step 11, and an extension such as `SPV_KHR_storage_buffer_storage_class` is requested on demand by the type needing it (line 2550, a `StorageBuffer` pointer). The `; SPIR-V` / `; Version:` lines are the disassembler rendering the module-header words written at lines 529-553, not instructions. |
| 16 | `emitPhysicalLayout` | [slang-emit-spirv.cpp](../../../../source/slang/slang-emit-spirv.cpp) | (always) | Produces the final word stream. |
| 17 | `optimizeSPIRV` | [slang-emit.cpp](../../../../source/slang/slang-emit.cpp) | `#if 0` (currently disabled) | Inline spirv-opt invocation inside `createArtifactFromIR` (lines 3456-3463), positioned before the downstream link/validate chain; left in for documentation, never executes. |
...
| 19 | `compiler->validate` (spirv-val) | (downstream tool) | `shouldRunSPIRVValidation(codeGenContext)` | True only when neither `SkipSPIRVValidation` nor `IncompleteLibrary` is set and the `SLANG_RUN_SPIRV_VALIDATION` env var equals exactly `"1"` (`shouldRunSPIRVValidation`, [slang-emit.cpp line 3409](../../../../source/slang/slang-emit.cpp)). |
| 20 | downstream `compile` (spirv-opt) | (downstream tool) | `compiler != nullptr` (line 3548) — i.e. `needsDownstreamCompiler` was true and `getOrLoadDownstreamCompiler(PassThroughMode::SpirvOpt, ...)` succeeded | `needsDownstreamCompiler` is the disjunction `needsLink \|\| needsOptimization \|\| needsValidation \|\| needsSeparateDebugInfo` (lines 3537-3538 of [slang-emit.cpp](../../../../source/slang/slang-emit.cpp)), so `needsOptimization` is only one of four reasons the compiler gets loaded — it is not itself the gate on the call. `compiler->compile` at line 3643 uses `downstreamOptions.targetType = SLANG_SPIRV`. See Notable passes for the `-Xspirv-opt` passthrough. |
...
after `lowerOptionalType` (line 1644). The second call **adds** to the
...
| `getMatrixLayoutMode()` (`-matrix-layout-row-major` / `-matrix-layout-column-major`) | Supplies the default layout that `specializeMatrixLayout` (Phase B step 3) fills into matrix types that left it unspecified. The emitted SPIR-V member decoration is the *transpose* of the source layout — a row-major source matrix emits `ColMajor`, a column-major one emits `RowMajor` — because what SPIR-V calls columns are what Slang calls rows ([slang-emit-spirv.cpp lines 7122-7145](../../../../source/slang/slang-emit-spirv.cpp)). |
| `getBoolOption(PreserveParameters)` | Two effects. Phase A: it is what sets `deadCodeEliminationOptions.keepGlobalParamsAlive` ([slang-emit.cpp](../../../../source/slang/slang-emit.cpp) line 1481) — note the field defaults to `true` in `IRDeadCodeEliminationOptions`, so this assignment turns it *off* when the option is unset. Phase D: the global-inst loop in `emitSPIRVFromIR` ([slang-emit-spirv.cpp](../../../../source/slang/slang-emit-spirv.cpp) lines 12267-12272) calls `ensureInst` on each `IRGlobalParam` that is still in the module, emitting it with its `Binding` / `DescriptorSet` decorations but *without* listing it in the entry point's `OpEntryPoint` interface. That loop can only re-emit a parameter that survived to emit, so any earlier removal is decisive — and an unreferenced module-scope global is not reliably preserved by the option alone: the reporting bundle saw an unreferenced second `RWStructuredBuffer` emit no `OpVariable` even at `-O0`, with or without an explicit `[[vk::binding]]`. Treat the option as keeping *surviving* parameters, not as resurrecting dead ones. |
...
| `target != CodeGenTarget::PyTorchCppBinding && targetCaps.atLeastOneSetImpliedInOther(CapabilitySet(CapabilityName::descriptor_handle)) == ImpliesReturnFlags::Implied` (lines 2871-2873) | Phase C's `targetProgram->getOrCreateLayout(sink)` direct call. True on the SPIR-V path, whose capability set implies `descriptor_handle`. |
| `compiler != nullptr` (line 3548), i.e. `needsDownstreamCompiler` held and `getOrLoadDownstreamCompiler(PassThroughMode::SpirvOpt, ...)` succeeded | The whole Phase D downstream block: `compiler->link` (spirv-link), `compiler->validate` (spirv-val), and `compiler->compile` (spirv-opt) at line 3643. `needsDownstreamCompiler` is `needsLink \|\| needsOptimization \|\| needsValidation \|\| needsSeparateDebugInfo` (lines 3537-3538). |
...
- Fixed-point condition: each pass returns `bool` indicating whether
  ...
  ([slang-ir-spirv-legalize.cpp lines 3133-3158](../../../../source/slang/slang-ir-spirv-legalize.cpp)),
...
Defined at lines 12438-12454 of
[slang-emit-spirv.cpp](../../../../source/slang/slang-emit-spirv.cpp).
...
At lines 2716-2717 of [slang-emit.cpp](../../../../source/slang/slang-emit.cpp)
...
the comment at lines 2647-2649 of `slang-emit.cpp`, is the SPIR-V rule
...
At lines 2627-2639 of `slang-emit.cpp` the address-space
...
(line 2358 of `slang-emit.cpp` selects on `case GLSL` /
...
([slang-emit-spirv.cpp line 7741](../../../../source/slang/slang-emit-spirv.cpp))
...
(line 4762, called at the end of `legalizeEntryPointForGLSL` at line
...
([slang-emit-spirv.cpp lines 7751-7758](../../../../source/slang/slang-emit-spirv.cpp)).
`SV_VulkanVertexID` / `SV_VulkanInstanceID` (lines 863 and 680) map
...
`linkAndOptimizeIR`. The SPIR-V arm (line 2474) runs it
unconditionally; the CUDA / Metal / CPU arm (line 2488) runs it
...
([slang-ir-spirv-legalize.cpp lines 2148-2277](../../../../source/slang/slang-ir-spirv-legalize.cpp))
...
([slang-emit-spirv.cpp line 4979](../../../../source/slang/slang-emit-spirv.cpp))
...
list twice. `processConstantBufferDescriptorHeapLoads` (line 1345 of
[slang-ir-spirv-legalize.cpp](../../../../source/slang/slang-ir-spirv-legalize.cpp))
...
2901, *after* `wrapRemainingConstantBufferElementTypes` (line 2901),
...
which are produced after the first `processWorkList()` (line 2896) has
already finished, a second `processWorkList()` follows at line 2910 to
...
([slang-emit-spirv.cpp line 7526](../../../../source/slang/slang-emit-spirv.cpp))
...
(line 7458) selects `SPIRVSamplerHeapStride` for sampler heaps and
...
([slang-emit-spirv.cpp line 6143](../../../../source/slang/slang-emit-spirv.cpp))
maps `SV_DepthGreaterEqual` to `SpvExecutionModeDepthGreater` and
`SV_DepthLessEqual` to `SpvExecutionModeDepthLess`, and
`maybeEmitEntryPointDepthReplacingExecutionMode` (line 6215) also
...
mixing two depth semantics collapses to `DepthReplacing` alone — a case no Slang source can reach, since the front end rejects a second depth output with `E30705` (*a fragment entry point can declare at most one depth output, but '<x>' conflicts with the earlier '<y>'*); the collapse is a backstop for IR that already carries both, such as a linked or generated module (lines
6154-6160). The
...
the comment at lines 12207-12216 of
[slang-emit-spirv.cpp](../../../../source/slang/slang-emit-spirv.cpp)).
...
the SPIR-V side lives in `emitSource` (line 2190): when the level is
...
by `getDebugInfoCommandLineArgumentForEntryPoint` (line 4101), which
...
- **`ImageGatherExtended` is requested only for a non-constant
  ...
  `isConstantGatherOffset(offset)` (line 6132). A constant offset
...
- **Shader-invocation reorder works from SPIR-V 1.4.**
  `requireShaderInvocationReorderExtension` (line 1904) picks the
  ...
  (line 2882). The 1.4 dependency on a physical-storage-buffer
  extension is handled centrally rather than at each call site:
  `requireSPIRVCapability` (line 11998) notices either reorder
...
- **`[Shader64BitIndexing]`** lowers to a
  ...
  (line 6748) turns into an `SPV_EXT_shader_64bit_indexing`
...
- **`NoContraction` under `-fp-mode precise`.**
  `maybeEmitNoContraction` (line 10613) decorates the emitted result
  ...
  results are decorated individually (line 10672); the
...
`needFlatDecorationForBuiltinVar` (line 7282 of
[slang-emit-spirv.cpp](../../../../source/slang/slang-emit-spirv.cpp))
...
emits an `OpSwitch` (line 8257) as a breakable region for a loop
...
`Uniform` or `StorageBuffer` (line 2521) — a `ConstantBuffer` fetched
...
`ensureUntypedPointerType` (line 7385). Since an untyped pointer
...
The float-to-bool cast path (line 9579 of
[slang-emit-spirv.cpp](../../../../source/slang/slang-emit-spirv.cpp))
...
`createArtifactFromIR` (line 3436 of `slang-emit.cpp`) wires up
...
conditional: `needsDownstreamCompiler` (line 3537) is the disjunction
of `needsLink`, `needsOptimization`, `needsValidation`, and
`needsSeparateDebugInfo`, and when it is false `compiler` stays
`nullptr` and the whole block at line 3548 is skipped. The point of
...
- **spirv-val** runs when `shouldRunSPIRVValidation` returns true.
  ...
  (uint32_t*)spirv.getBuffer(), ...)` at line 3425), not the linked
...
- **spirv-opt** is invoked via the generic downstream-compile
  ...
  at line 3643. The earlier in-source `optimizeSPIRV` call is inside
  a `#if 0` block opening at line 3456 and never executes — it is
...
into `spirvOptArgs` at line 3529 of
[slang-emit.cpp](../../../../source/slang/slang-emit.cpp) and folds
it into `needsOptimization` at line 3531. Two consequences follow.
...
forwarded into `downstreamOptions` at line 3616.
