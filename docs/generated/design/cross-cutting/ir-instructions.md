...
- [slang-ir-insts.h](../../../../source/slang/slang-ir-insts.h) holds
  ...
  opcode _not_ explicitly defined there, complete with `isaImpl`, a
...
(which owns the serialized opcode identities described under _Module
versioning_ below), and the two `extras/` checker scripts cited under
_Adding a new opcode_. They should be added to this document's
`watched_paths`.
...
| `constexprAdd` ... `constexprEnumCast`                                            | —                                      | 1-3 fixed operands; see Lua entries         | Compile-time-folded arithmetic / cast variants; hoistable, which the runtime `add` family is not. They carry _type-level_ values: arithmetic on a generic value parameter, as in `int b[N + 1]` inside `f<let N : int>`, has to produce a value usable as an array bound, so it lowers to `constexprAdd` rather than `add`. |
...
This document keeps the _conventions_ — schema, op-flag bits,
...
A number of opcodes are conceptually _decorations_: every entry in the
...
Note what this comment is _not_ about. Inserting an entry in the
...
_stable name_ — a small integer assigned once and never reused, listed
in
[slang-ir-insts-stable-names.lua](../../../../source/slang/slang-ir-insts-stable-names.lua)
...
What the supported-version range does track is module _semantics_: a
...
adding an instruction bumps only the maximum, whereas _removing_ one
