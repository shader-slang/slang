...
- `nameAndLoc: NameLoc` — the declared name paired with the source
  location of the _name token_ rather than of the declaration as a
...
- `type: QualType` — the type assigned to the expression by the
  ...
  _left of `=` is not an l-value_ error rather than a type error.
...
The abstract base for compile-time _values_. `Val`s are deduplicated
