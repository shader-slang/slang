...
The _surface spelling_ of an attribute is not declared in the C++
...
| `GLSLRowMajorLayoutModifier`    | `ColumnMajorLayoutModifier` | (no additional state) | (none)                                                    | GLSL `row_major`, parented on the _column_-major base because Slang reads matrix conventions backwards from GLSL; no parse path constructs it. |
| `GLSLColumnMajorLayoutModifier` | `RowMajorLayoutModifier`    | (no additional state) | (none)                                                    | GLSL `column_major`, parented on the _row_-major base for the same reason; no parse path constructs it.                                        |

Both the `row_major` and `column_major` keywords parse into the _HLSL_
...
| `EnumClassModifier`               | `Modifier`  | (no additional state)                           | (none)                                                                   | Marker the parser adds for the `enum class` spelling; the conflicting shape it exists to detect is an explicit `[UnscopedEnum]` on an `enum class`, since the implicit attachment above happens only for an enum _not_ written as `enum class`.                                                                       |
...
The split is by syntax: a _modifier_ is a bare keyword that the
parser knows by name and immediately constructs the appropriate
`Modifier` subclass for; an _attribute_ is an `[ident(args)]`
...
These modifiers are how a parameter's binding is _represented_ in the
...
describe _that_ a function is differentiable,
