...
is _not_ declared in the expression header: it lives in
...
`MatrixCoord` declared in the same header are _not_ `Expr` subclasses
...
significand. All three are _warnings_, so the compile continues and the
...
folds the sign into a _copy_ of the literal node; the
