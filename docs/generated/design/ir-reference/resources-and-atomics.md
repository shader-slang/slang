...
This page is the per-opcode reference for the IR opcodes that _operate on_
...
The resource _types_ these opcodes operate on are not documented here. They
...
that column. The distinction that matters is _where the struct is
declared_:
...
_not_ always the Lua key. In this family the differing pairs are
...
Only two entries on this page are Lua _grouping_ parents, and neither has
...
| `CombinedTextureSamplerGetTexture`     | `IRCombinedTextureSamplerGetTexture`     | `sampler`                        |       | `__intrinsic_op`, [hlsl.meta.slang](../../../../source/slang/hlsl.meta.slang) lines 1185, 2294                                                                                                                                                                                      | Projects the texture half. Note the Lua operand is named `sampler` even though it holds the _combined_ value.                                                                    |
...
means they _do_ appear at the lowering snapshot, with no inlining step
...
There is no memory-_scope_ operand. The SPIR-V backend hard-codes
...
Only the three _mask_ opcodes live here; the rest of the wave intrinsics
...
| `getOptiXHitAttribute`               | `IRGetOptiXHitAttribute`               | `[type], [index]`            |       | [slang-ir-legalize-varying-params.cpp](../../../../source/slang/slang-ir-legalize-varying-params.cpp) line 2151         | Reads one hit attribute. Operand 0 is the _type_ inst to fetch and operand 1 the attribute index — an unusual shape, since the type is normally the result type rather than an operand. |
...
What produces `getOptiXSbtDataPointer` is _where_ a uniform is
...
nearby shapes do _not_ produce it: a module-scope `uniform` global
...
opcodes are _core-module_ intrinsics (lines 27685 and 27688) that are
...
for `image[coord]`. It is not the load itself — it models a _pointer to a
texel_, which is what makes `image[coord] = v` and `image[coord] += v` type
...
Note also that one `atomicAdd` in the IR corresponds to one _statement_,
...
opcode is _declared_, not the earliest IR in which you can observe it.
...
The single operand is the heap index; the _result type_ carries the
...
- [types.md](types.md) — the resource, sampler, and buffer _types_ these
