...
key survives as the *mnemonic* printed by `-dump-ir`. Second, the
...
3534) or by the generated `getArrayType` / `getUnsizedArrayType`.
...
supplied as the single generic argument. The node *attributes*
...
*Existentials and interfaces*, and as a container in
...
lives in the *value*, not in the type, and is moved in and out by the
