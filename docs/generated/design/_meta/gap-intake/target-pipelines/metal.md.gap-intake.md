...
`74f4ac69bb7b` claimed Metal has _no_ surface for
...
| d0badd8620c9 | deferred             | The column-count question is decided by `MetalBufferElementTypeLoweringPolicy::needsPackedVectorStorage` / `usesPackedVectorStorage` in `source/slang/slang-ir-lower-buffer-element-type.cpp:2984-3010`, which is **not in `watched_paths`**; nothing in the watched set states the rule. (For the operator: the answer is _every_ vector of more than one element, so a `float4x4` device-buffer field does become `packed_float4`; and only for `Natural`-layout buffers in the `StorageBuffer` / `UserPointer` address spaces — constant and argument buffers keep native MSL layout.) Follow-up: add `source/slang/slang-ir-lower-buffer-element-type.cpp` to `watched_paths`, then state the rule with one example.   | —                                                                                                                                                                                                                              |
