...
| Struct field read/write and vector rgba swizzle emission on Metal compute target. | gpu-other      | [#member-expression](../../../../language-reference/expressions-member-access.md#member-expression)       | `printf` is not in Metal's compute capability set (E36107); the existing test shaders use printf for value escape. Dedicated buffer-output shaders could enable Metal emission testing.               |
| Struct field read/write and vector rgba swizzle emission on WGSL compute target.  | gpu-other      | [#member-expression](../../../../language-reference/expressions-member-access.md#member-expression)       | `printf` is not in WGSL's compute capability set (E36107); the existing test shaders use printf for value escape. Dedicated storage-buffer shaders could enable WGSL emission testing.                |
