...
| C7 — Static data member as a named module-scope variable on the cuda target.               | implementation-detail | [#description](../../../../language-reference/types-struct.md#description)                                               | For `struct-static-member-functional.slang`, CUDA folds the static to `int(5)` directly in the kernel body with no named variable. The spirv-asm half of this row was wrong and has been removed: at `-O0 -g2` SPIR-V does emit `%b = OpVariable %_ptr_Private_int Private`, and the test now pins it. |
...
| -------------------------------------------------------------------------------------------------------- | ----------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [#description](../../../../language-reference/types-struct.md#description)                               | drift-from-source | The doc says the no-body declaration "specifies an existence of a structure type ... This enables its use in type expressions without the member declarations", which reads as a forward declaration that a later with-members declaration completes. The compiler does not merge them: `struct ForwardDeclared;` followed by `struct ForwardDeclared { int x; }` reports `error[E30200] conflicting declaration` and then `error[E39999] ambiguous reference`, identically on hlsl, glsl, spirv-asm, metal and wgsl. A no-body declaration used on its own does compile. | State explicitly whether a no-body declaration may be completed by a later with-members declaration of the same name in the same module. If it may not, say so in the _no-body_ bullet and show the accepted use (declare, then use in a type expression, with the definition supplied by another module via `extern struct`). Tracked as `_meta/findings/types-struct-no-body-forward-decl-conflicting-declaration.yaml`. |
