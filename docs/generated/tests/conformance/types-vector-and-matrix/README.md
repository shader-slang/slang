...
| Emission of swizzle/operator claims on Metal target.                      | gpu-metal-toolchain   | [#element-access](../../../../language-reference/types-vector-and-matrix.md#element-access)                   | `printf` is unavailable in Metal compute entry points (E36107); the test bodies use `printf` and cannot compile for Metal without a full rewrite to buffer-based I/O. |
| Emission of swizzle/operator claims on WGSL target.                       | gpu-wgsl-tint         | [#element-access](../../../../language-reference/types-vector-and-matrix.md#element-access)                   | `printf` is unavailable in WGSL compute entry points (E36107); same constraint as Metal.                                                                              |
| Swizzle-extract (`v.xy`, `v.xww`) visible in any emitted text.            | implementation-detail | [#element-access](../../../../language-reference/types-vector-and-matrix.md#element-access)                   | All text-emit targets constant-fold the extract away when operands are literals; no swizzle-extract opcode survives in HLSL/GLSL/SPIRV/CUDA/CPP emission.             |
| Matrix subscript `m[r][c]` and row `m[r]` visible in GLSL/SPIRV emission. | implementation-detail | [#row-and-element-access-](../../../../language-reference/types-vector-and-matrix.md#row-and-element-access-) | GLSL constant-folds matrix literals entirely; SPIRV has no matrix subscript opcode (values become OpConstant). Both are covered on HLSL/CUDA/CPP.                     |
