...
| C22 — Variable declared in for-loop init is scoped to the for statement; can be redeclared in successive for loops, and each loop keeps its own bound on every shader text target. | functional           | [#for-statement](../../../../language-reference/statements.md#for-statement)                           | [`for-init-scope-functional.slang`](for-init-scope-functional.slang)                               |
...
| C39 — `discard` in a fragment shader emits a discard/kill instruction on every shader text target (HLSL/GLSL/WGSL `discard`, Metal `discard_fragment()`, SPIR-V `OpKill`).         | functional           | [#discard-statement](../../../../language-reference/statements.md#discard-statement)                   | [`discard-emission.slang`](discard-emission.slang)                                                 |
...
| C44 — `$for` is fully unrolled on every shader text target; repeated constant-index body, no loop structure remains.                                                               | functional           | [#compile-time-for-statement](../../../../language-reference/statements.md#compile-time-for-statement) | [`compile-time-for-emission.slang`](compile-time-for-emission.slang)                               |
