...
> behind a finding (or behind a decision *not* to file one) is recoverable.
...
| 3 | Delete the stale entry for `pointer-arithmetic-spirv-emission` (01) | open |
| 4 | Fix the `classdecl-reference-type` test, then drop its entry (03) | open |
