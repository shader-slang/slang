...
| Standard type aliases (`floatN`, `int32_tN`, `boolN`, etc.). | (unclassified) | [#standard-type-aliases](../../../../language-reference/types-vector-and-matrix.md#standard-type-aliases) | Each alias is exercised implicitly by every test in this bundle that uses `int4` / `vector<int, 4>` etc. A dedicated "alias works" test would be a no-op. |
| Matrix type dimension range `[1, 4]` for rows/columns. | (unclassified) | [#matrix-types](../../../../language-reference/types-vector-and-matrix.md#matrix-types) | Boundary tests for the range would need `matrix<T, 5, 5>` to reject and `matrix<T, 1, 1>` to accept; deferred — focus on the more-bug-prone `mul`-vs-`*` distinction. |
