# Prompt: docs/generated/tests/regression/ir-reference/differentiation/
...
`docs/generated/tests/regression/ir-reference/differentiation/`, anchored to
...
- `ir-reference/types` — the type-level half (`DifferentialPairType`,
  ...
  _type_ appears here only as the result/operand type of a
...
- `ir-reference/structure` — `func`, `key`, `interface`,
  ...
  this bundle observes the differentiation opcodes that _use_ those
...
%remat, %ctx_t)`** at LOWER-TO-IR, _not_ to `BackwardDifferentiate`.
