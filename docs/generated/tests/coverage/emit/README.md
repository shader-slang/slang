...
feasible text target the divergence is visible on — text emission runs
locally for every one of those targets, so they all validate here as
well as in CI. The value-level
...
- **`MakeMatrixFromScalar` / `MatrixReshape` with a single-element
  consumer is invisible on SPIR-V.** Both tests' original entry points
  read only one or two elements out of the resulting matrix
  (`m[1][1]`, `small[0][0] + small[2][2]`), and the SPIR-V path folds
  those reads back into their sources so the matrix never materializes —
  the emitted function contains no matrix instruction at all. Rather
  than leave SPIR-V unpinned, each of those two tests carries a second
  entry point (`computeMainMul`) that consumes the whole matrix with a
  `mul`, forcing it to materialize; the SPIR-V directives target that
  entry point. SPIR-V has no constructor syntax, so the same shapes show
  up as `OpCompositeConstruct` (scalar fan-out) and three
  `OpVectorShuffle`s selecting components `0 1 2` (row truncation).
