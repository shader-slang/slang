...
`SIMPLE` emit whose FileCheck finds the force-inlined runtime-tag switch in
the entry point and calls to both concrete implementations.
...
| [`dynamic-dispatch-two-conformers-emit.slang`](dynamic-dispatch-two-conformers-emit.slang)                       | An interface value assigned one of two concrete conformers on different paths lowers cleanly to CUDA dynamic-dispatch code: the runtime-tag `switch` is force-inlined into `computeMain`, with calls to both concrete implementations.                                                                                                                                              | source/slang/slang-ir-lower-dynamic-dispatch-insts.cpp |
