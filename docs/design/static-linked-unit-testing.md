...
| `addStraightLineMarkerRun`            | a run of line-coverage markers in one block                                   |
| `addMarkerRunAroundCall`              | two line markers separated by a call to a given callee                        |
| `addMarkerRunSplitByAbort`            | two line markers separated by a mid-block `Abort`                             |
| `addFunctionEndingInGenericAsm`       | a function whose sole terminator is a `GenericAsm`                            |
| `addMutuallyRecursiveFunctions`       | an asymmetric mutually-recursive pair (one abandons via `Abort`, one returns) |
| `addLineMarkersAroundFunctionMarker`  | two line markers around a function-entry marker                               |

The last six exist for the coverage counter-coalescing tests
