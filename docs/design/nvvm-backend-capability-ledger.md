...
| `tests/compute/non-square-column-major.slang` | NVRTC + direct NVVM runtime, research247 | Original graphics oracle mismatch retained | All three modes consume compact column stride12 and return `11,1`; unchanged graphics input/oracle requires stride16 and `11,22`. Eighteen independent CUDA controls pass; no old cell is resolved. |
