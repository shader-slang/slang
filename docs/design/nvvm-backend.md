...
### Slice 48 UInt wave read-lane-at and intrinsic arguments

Slice 48 adds a kernel that computes `laneIndex = WaveGetLaneIndex()`, passes that UInt value to
`WaveMaskReadLaneAt(mask, laneIndex, sourceLane)`, and stores the selected lane's index through
`destination[laneIndex]`. CUDA target selection retains a specialized one-block
`Func(UInt, UInt, UInt, Int)` helper whose terminator is exact
`GenericAsm("__shfl_sync($0, $1, $2)")`. The three semantic arguments are the helper parameters;
the GenericAsm instruction itself still has only its assembly-string operand.

The GenericAsm descriptor now includes an exact structural signature. Preflight accepts the
shuffle spelling only when its helper returns UInt and has `(UInt mask, UInt value, Int lane)`
parameters. Direct emission lowers those existing parameters from the helper value map and passes
their provider handles in declaration order. It does not parse placeholders, match helper names,
or construct a second representation of the arguments. The two zero-argument wave helpers retain
their established descriptor signature.

Feature 36, `WAVE_READ_LANE_AT_UINT`, and intrinsic operation 2 extend the unchanged generic
callback. The V3 table remains 528 bytes on x64 and 308 bytes on x86; exact Slice 47 tables remain
valid with feature 36 clear. The facade maps operation 2 to its feature before dispatch, and the
provider requires exactly three available same-function i32 arguments before any module mutation.

The LLVM 14 provider emits
`llvm.nvvm.shfl.sync.idx.i32(mask, value, lane, 31)`. The final constant is CUDA's default 32-lane
clamp and is provider policy rather than a fourth Slang semantic argument. LLVM 14 declares the
intrinsic as `i32(i32, i32, i32, i32)` with exact
`convergent inaccessiblememonly nounwind` attributes. That declaration is already accepted by the
LLVM-7-era NVVM reader, so the legacy writer validates it but leaves it unchanged. Both LLVM and
negotiated NVVM text contain one lane-id call, one shuffle call with clamp 31, two UInt helper
calls/returns, one pointer offset, and one store.

The fake graph records all three shuffle-helper parameter references and the kernel call's
`mask`, lane-index call result, and `sourceLane` arguments. NVVM and NVRTC agree on the
`[64, 32, 32]` launch ABI, one global 32-bit store, no global load, and
`shfl.sync.idx.b32`. CUDA 12.9 `ptxas` accepts both routes. On the RTX 5090, one full 32-thread warp
selects source lanes 0 and 7 and writes the selected lane index to all 32 destinations through both
compilers.

Seven independently registered evidence layers add 479 physical lines across the five measured
test/support files, from 23,732 to 24,211. The focused Slice 47/48 matrix passes 14/14 and the
Release NVVM prefix passes 319/319. Its exact sorted LF-terminated name set has SHA-256
...
### Slice 59 public wave-active-any-true and Boolean helper parameters

Slice 59 adds feature 44 `WAVE_MASK_ANY_TRUE` and intrinsic operation 10 through the existing
generic callback. CUDA specializes public `WaveActiveAnyTrue(condition)` to
`WaveMaskAnyTrue(WaveGetActiveMask(), condition)`; final linked IR retains one exact
`Func(Bool, UInt, Bool)` helper ending in `(__any_sync($0, $1) != 0)`. Complete assembly and
signature matching selects the provider semantic without helper names or graph rediscovery. V3
remains 528/308 bytes, and an exact Slice 58 provider remains valid with feature 44 clear.

This graph exposes the first ordinary helper with a Bool parameter. Type lowering, signature
preflight, and scalar-call argument validation now admit canonical Bool in that role and preserve it
as native i1. Bool entry-point parameters and block phis remain unsupported; the accepted block-phi
set is still signed i32/float. The fake provider records Bool as a distinct function-parameter kind,
so both the helper declaration and its call prove that no i32 surrogate is introduced.

The provider validates an available i32 mask and i1 condition before mutation, then calls
`llvm.nvvm.vote.any.sync(i32, i1) -> i1`. LLVM 7 and the provider LLVM describe that exact
signature with the same convergent/inaccessible-memory/nounwind semantic attributes. The existing
legacy writer now audits synchronized ballot and any-vote declarations together; their normal and
LLVM-7-compatible spellings are already identical, so this slice adds no text rewrite, callback, or
marker.
...
ballot, and masked any-true; clearing any one independently returns E52016 before provider module
construction.

NVVM and NVRTC agree on a `[64, 64]` entry with one 32-bit global load, two
`vote.sync.ballot.b32` instructions, one `vote.sync.any.pred`, signed inequality, and one global
32-bit store. CUDA 12.9 `ptxas` accepts both. The RTX 5090 runtime fixture supplies zero to 31 lanes
and one to lane seven; all 32 active lanes store one through both routes.

Seven independently registered evidence names add 368 physical lines across the five measured
test/support files, from 26,982 to 27,350. The complete Slice 46-59 wave matrix passes 92/92 and
the Release NVVM prefix passes 390/390. Its exact sorted LF-terminated name set has SHA-256
...
### Slice 63 Float wave-active-all-equal and native b32 adaptation

Slice 63 adds feature 48 `WAVE_MASK_ALL_EQUAL_FLOAT` and intrinsic operation 14 through the
generic callback. CUDA specializes public `WaveActiveAllEqual(floatValue)` to
`WaveMaskAllEqual(WaveGetActiveMask(), floatValue)`; final linked IR retains the exact
`Func(Bool, UInt, Float)` helper ending in `_waveAllEqual($0, $1)`. Complete assembly and signature
matching preserves Float through the emitter and independently negotiates the new feature. V3
remains 528/308 bytes, and an exact Slice 62 provider remains valid with feature 48 clear.

LLVM 7 and the provider LLVM expose only i32 and i64 match-all intrinsics, while NVRTC lowers the
Float CUDA template directly to `match.all.sync.b32`. The provider therefore validates the i32
mask, f32 value, ownership, insertion point, and availability before creating one
`bitcast float to i32`. It feeds that bit pattern to the established
`llvm.nvvm.match.all.sync.i32p(i32, i32) -> {i32, i1}` call and returns aggregate element 1. This
adaptation sits at the native representation boundary: the emitter and facade never misclassify
the semantic Float, and the integer bit pattern and matching mask remain provider-private. The
existing legacy intrinsic declaration audit applies unchanged and no text rewrite is added.

The predicate fixture now names its genuine Boolean/Integer/Float payload dimension explicitly
instead of encoding Boolean-vs-integer in a flag. The real-provider assembly verifier covers all
three all-equal overloads and requires exactly one Float bitcast only for the Float row. Each row
retains independent source, exact signature, feature, operation, PTX, and runtime assertions.

NVVM and NVRTC agree on a `[64, 64]` entry with one 32-bit global load, two
`vote.sync.ballot.b32` instructions, one `match.all.sync.b32`, and one global 32-bit store. CUDA
12.9 `ptxas` accepts both. On an RTX 5090, distinct ordinary Float lane values make all 32 lanes
store zero, while uniform `3.25` makes all lanes store one through both routes. This row makes no
new policy claim for NaNs or differently encoded equal Float values.

Seven names add 155 measured lines across the five test/support files, from 27,795 to 27,950. The
complete Slice 46-63 wave/ABI matrix passes 120/120 and the Release NVVM prefix passes 418/418. Its
exact sorted LF-terminated name set has SHA-256
...
- wave/subgroup operations beyond lane index, lane count, canonical masked UInt/Int/Float
  ...
  all-equal, including other scalar types, other votes, reductions, and their convergence
  contracts;
