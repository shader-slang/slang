...
An `if` statement defines a conditional branch in the program. If *`cond-expr`* evaluates to `true`,
*`then-stmt`* is executed. Otherwise, *`else-stmt`* is executed if one is provided. After
*`then-stmt`*/*`else-stmt`* has been executed, the execution of the `if` statement completes.
...
`if` statements that have an *`else-stmt`* in [block statements](statements-block.md) to avoid potential
confusion.
