...
**Terminal symbol**
> **`'terminal'`** --- terminal string, spelled exactly as is within quotes<br>
> **\<regex\>** --- terminal symbol expressed using a regular expression

[Terminal symbols](https://en.wikipedia.org/wiki/Terminal_and_nonterminal_symbols#Terminal_symbols) are those
that can appear in the language. In Slang language reference manual two forms are used: literal strings that
appear exactly as defined and symbols defined by regular expressions. White spaces between terminal symbols
...
A production rule defines how the left-hand-side non-terminal *`lhs`* may be substituted by the
right-hand-side grammatical expression \<expr\>. The expression consist of non-terminals and terminals using
the expression components described below.

Sometimes more than one production rule definitions are provided for the left-hand-side non-terminal. This
means that the non-terminal may be substituted using any of the definitions, which is grammatically equivalent
to alternation. Multiple production rules are used to associate semantics for individual production rules.

In case more than one production rules may be successfully used, the semantics of the rule introduced earlier
apply unless explicit precedence (*i.e.*, priority) is provided.

Note that a definition is not explicitly terminated with a semi-colon (;).
...
Grouping is used to group a sub-expression. It is used to denote the order of production.
