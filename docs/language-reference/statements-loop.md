...
A `while` loop evaluates the loop condition *`cond-expr`*. If the loop condition evaluates to `false`, the
...
A `do-while` loop begins by executing the *`loop-stmt`*. Then the loop condition *`cond-expr`* is evaluated to
...
A loop statement is a [breakable statement](statements-break-and-continue.md), and it can be prefixed by a
