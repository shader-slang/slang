...
A pointer of type `T` represents an address to an object of type `T`.

> 📝 **Warning:** Pointers are not yet fully implemented in `slangc`.

Current limitations include:
- Pointers to local memory are not supported.
...
- Slang currently does not support pointers to immutable values such as `const T *`.
...
> 📝 **Remark:** Pointers can also be declared using [variable declarations](declarations.md). In this case, a
> variable is declared as a pointer to a type, rather than the type being a pointer type.
...
## Generic Pointer Type {#generic-pointer}

A named type [Ptr<T, AccessMode, AddressSpace>](../../../core-module-reference/types/ptr-0/index.html) provided
by the Slang standard library.
...
For details, refer to the Slang standard library documentation.


## Description

A pointer declaration makes a type a pointer type. The base type may be any
[addressable](types-traits.md) type including pointer and [array](types-array.md) types.

To obtain the address of an object, the *address-of* operator `&` is used. The address may be assigned to a
pointer variable with matching type. Alternatively, `__getAddress(obj)` may be used.

To access the *pointed-to* object, a pointer dereference operator `*` is used. In case the pointed-to type is a
[structure](types-struct.md) or a [class](types-class.md) type, member access operator `.` or `->` may be
used to dereference the pointer and access a member.

When a pointer points to an array element, it may be added or subtracted by an integer value. The resulting
pointer points to an element offset by that value.

A value of a pointer belongs to one of the following classes:
...
> array (which may not be dereferenced). Other results are
> [undefined behavior](basics-behavior.md#classification).

> 📝 **Remark 1:** Currently, there are no immutable `const` pointers in Slang.
...
Pointers for other address spaces and access modes may be declared by using
[Ptr<T, AccessMode, AddressSpace>](../../../core-module-reference/types/ptr-0/index.html) type alias provided
...
```hlsl
RWStructuredBuffer<uint> inputData;
...
{
    // Get the length of inputData in elements
    uint dataLen = inputData.getCount();

    // Calculate sum of elements 0, 1, ..., 9 provided
    // the input data buffer is big enough.
    outputBuffer[id.x] +=
        sumOfValues(
            &inputData[0], &inputData[min(dataLen, 10)]);
}
```

> 📝 **Remark:** This example does not currently work properly on SPIR-V target
> \[[#9061](https://github.com/shader-slang/slang/issues/9061)\].
