...
> ⚠️ **Warning:** When an extension and the base type (or two `extension`s of the same type) declare a
> member with the same name (and, for functions, an overlapping signature), it is currently undefined
> which member takes effect. The compiler now emits a diagnostic flagging this conflict, but it does
> not decide which declaration is used. ([Issue #9660](https://github.com/shader-slang/slang/issues/9660))
