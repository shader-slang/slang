...
Thee HOST / SHADER split from the [CPU target](cpu-target.md) applies here as well.
...
* `llvm-shader-ir` / `SLANG_LLVM_SHADER_ASSEMBLY` generates LLVM IR compute shader entry points.
* `llvm-shader-obj` / `SLANG_LLVM_SHADER_OBJECT_CODE` generates object code compute shader entry points.
...
All things are stored in memory according to an LLVM-style layout by default.
This means that vectors are aligned to the next power of two, structures are
aligned and padded up to the largest alignment among their fields, and booleans
are a single byte.

If you specify a layout with flags like `-fvk-use-c-layout` or
`-fvk-use-scalar-layout`, all structure and array types on the stack and heap
will follow the specified layout rules.
