...
3. Spawns `test-server.exe` once and sends it a JSON-RPC `tool` request per shader, equivalent to
   ...
   ```
   slangc -lang glsl test.slang.<ext> -target spirv -stage <stage> -entry main -matrix-layout-row-major
   ```
...
```
slangc.exe -lang glsl test.slang.comp -target spirv -stage compute -entry main -matrix-layout-row-major -o out.spv
```
