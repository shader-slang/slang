...
| GLSLForceScalarLayout | Specifies the `-force-glsl-scalar-layout` option. When set will use `scalar` layout for all buffers when generating SPIR-V. Note that Slang's `scalar` layout is not C/C++-compatible (no struct-size round-up; `bool` is 4 bytes) and differs from DXC, which changed its scalar-layout option to add C-style struct-size rounding; use `ForceCLayout` (`-fvk-use-c-layout` on the CLI) for C-compatible layout (which targets standard C/C++ rules and may not match DXC byte-for-byte). `intValue0` specifies a bool value for the setting. |
...
| ForceCLayout | When set forces the compiler to lay out buffer struct fields the way a typical C/C++ compiler would. Relative to Slang's natural/scalar layout this rounds each struct's overall size up to its alignment and represents `bool` as a single byte (natural/scalar layout uses 4 bytes), so the two are not interchangeable. `intValue0` specifies a bool value for the setting. |
