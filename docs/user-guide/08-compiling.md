...
When these options are used the compiler will _ignore_ all `[shader(...)]` attributes and only use the explicitly-specified entry points instead.
...
> #### Note
>
> Currently, the global session type is _not_ thread-safe.
