...
| `SLANG_DXC_BUILD_FROM_SOURCE`     | Unset (auto on native Linux x86_64, no custom URL) | `ON`: always build DXC from source; `OFF`: always use prebuilt; unset: auto-detect on native (non-cross-compiling) Linux x86_64 (see [DXC GLIBC auto-detection](#dxc-glibc-auto-detection)) |
...
When `SLANG_DXC_BUILD_FROM_SOURCE` is unset on Linux x86_64 (and
`SLANG_DXC_BINARY_URL` is not set and cross-compilation is not active), CMake
automatically downloads the prebuilt DXC binary at configure time and inspects
the GLIBC requirements of both `libdxcompiler.so` and `libdxil.so`. If either
library requires a newer GLIBC than the system provides, DXC is built from
source instead. Detection results are cached in stamp files so subsequent
reconfigures are fast.

- `ON`: always build DXC from source (Windows and Linux only; fatal error on other platforms).
- `OFF`: skip auto-detection and always use the prebuilt binary (no GLIBC check is performed).
- unset on non-x86_64 Linux (e.g. ARM64): no prebuilt binary is available — set `ON` to build from source,
  or set `SLANG_DXC_BINARY_URL` to point to a custom prebuilt binary.
