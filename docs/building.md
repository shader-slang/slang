...
| `SLANG_ENABLE_SPIRV_OPT_MERGE_RETURN` | `TRUE`                             | **Will be removed.** Register SPIRV-Tools `MergeReturnPass` + `InlineExhaustivePass` in the slang-glslang SPIR-V optimizer pipeline; set to `FALSE` to work around [SPIRV-Tools#6711](https://github.com/KhronosGroup/SPIRV-Tools/issues/6711), a UAF observed with debug-info SPIR-V |
