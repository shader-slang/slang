...
| `SLANG_USE_SYSTEM_DXC`                | `FALSE`                                                                | Use a system-installed DXC via `find_package(DXC)`; requires `SLANG_ENABLE_DXIL=ON`. Takes precedence over the other DXC selectors (see [DXC acquisition](#dxc-acquisition)).                                                              |
| `DXC_ROOT_DIR`                        | ``                                                                     | Hint for `find_package(DXC)`: a DXC install prefix containing `include/dxc/dxcapi.h` and `lib/` (or `bin/` on Windows).                                                                                                                    |
