...
| `SLANG_DXC_BUILD_FROM_SOURCE`     | Auto-detected              | `ON`: always build DXC from source; `OFF`: always use prebuilt; unset: auto-detect on native (non-cross-compiling) Linux x86_64 (see [DXC GLIBC auto-detection](#dxc-glibc-auto-detection)) |
