...
| `SLANG_DXC_BUILD_FROM_SOURCE`     | Auto-detected              | `ON`: always build DXC from source (Windows and Linux only; fatal error on other platforms); `OFF`: skip auto-detection and always use prebuilt binaries; unset: on Linux x86_64, automatically detect by downloading the prebuilt binary at configure time and inspecting the GLIBC requirements of both `libdxcompiler.so` and `libdxil.so` — if either library requires a newer GLIBC than the system provides, DXC is built from source; auto-detection is also skipped when cross-compiling or when `SLANG_DXC_BINARY_URL` is explicitly set; on non-x86_64 Linux (e.g. ARM64) no prebuilt binary is available — set to `ON` to build DXC from source on those architectures |
