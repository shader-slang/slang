...
| `SLANG_ENABLE_SPIRV_OPT_MERGE_RETURN` | `TRUE`                             | **Temporary; will be removed.** Registers the SPIRV-Tools `MergeReturnPass` + `InlineExhaustivePass` pair in the slang-glslang SPIR-V optimizer pipeline. Default `TRUE` preserves existing SPIR-V output. Set `FALSE` to work around the use-after-free in [SPIRV-Tools#6711](https://github.com/KhronosGroup/SPIRV-Tools/issues/6711) (Slang tracking [#11146](https://github.com/shader-slang/slang/issues/11146)); SPIR-V-only, no effect on other targets. |
