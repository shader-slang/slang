...
| `SLANG_ENABLE_SPIRV_OPT_MERGE_RETURN` | `TRUE`                             | **Will be removed.** Register the paired SPIRV-Tools `MergeReturnPass` + `InlineExhaustivePass` sequence in the slang-glslang SPIR-V optimizer pipeline; set to `FALSE` to work around [SPIRV-Tools#6711](https://github.com/KhronosGroup/SPIRV-Tools/issues/6711), a UAF observed with debug-info SPIR-V. Disabling skips both passes and can produce larger or slower SPIR-V because later local/SROA/DCE passes see less inlined code |
