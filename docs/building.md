...
| `SLANG_DXC_BUILD_FROM_SOURCE`     | Auto-detected              | `ON`: always build DXC from source (Windows and Linux only; fatal error on other platforms); `OFF`: skip auto-detection and always use prebuilt binaries; unset: on Linux, automatically build from source when the system GLIBC is below 2.29 (targeting very-old release containers such as Ubuntu 18.04 / GLIBC 2.27–2.28); systems with GLIBC 2.29–2.37 download prebuilt binaries that require GLIBC 2.38 at runtime and may silently fail to load DXC — set `ON` explicitly if DXC is needed on those systems; auto-detection is also skipped when cross-compiling or when `SLANG_DXC_BINARY_URL` is explicitly set |
