...
* `-load-repro [filename]` loads the repro and compiles using it's options. Note this must be the last arg on the command line. Malformed repro files are rejected before use; `-extract-repro` uses the same validation.
