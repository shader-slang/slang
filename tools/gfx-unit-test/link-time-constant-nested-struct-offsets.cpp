    ...
    const char* testPhase)
{
    ...
    SLANG_CHECK_MSG(globalTypeLayout->getKind() == slang::TypeReflection::Kind::Struct, "Global scope is not a struct type");
    ...
    SLANG_CHECK_MSG(elementTypeLayout->getKind() == slang::TypeReflection::Kind::Struct, "Buffer element is not a struct type");
    ...
    for (unsigned int j = 0; j < structFieldCount; j++)
    {
        ...
        SLANG_CHECK_MSG(
            ...
            "Field offset incorrect");
        SLANG_CHECK_MSG(
            fieldSize == expectedSizes[j],
            "Field size incorrect");
        
        // If this is the nested struct field, check its behavior
        if (fieldKind == slang::TypeReflection::Kind::Struct && strcmp(structFieldName, "nested") == 0)
        {
...
void linkTimeConstantNestedStructOffsetsTestImpl(IDevice* device, UnitTestContext* context)
{
    ...
    slang::IModule* mainModule = slangSession->loadModule("link-time-constant-nested-struct-offsets-main", diagnosticsBlob.writeRef());
    ...
    // Check struct offsets with ONLY main module (nested struct with link-time array should be unknown)
    const char* expectedFieldNames[] = {"a", "b", "nested", "c", "d"};
    size_t beforeLinkingOffsets[] = {0, 4, 8, SLANG_UNKNOWN_SIZE, SLANG_UNKNOWN_SIZE};
    size_t beforeLinkingSizes[] = {4, 4, SLANG_UNKNOWN_SIZE, 4, 4}; // int, float, unknown nested struct, int, float
    ...
    slang::IModule* libModule = slangSession->loadModule("link-time-constant-nested-struct-offsets-lib", diagnosticsBlob.writeRef());
    diagnoseIfNeeded(diagnosticsBlob);
