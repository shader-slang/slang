...
// Clears one append-only feature suffix so historical provider fixtures stay exact as V3 grows.
static void _clearNVVMBuilderFeaturesFrom(
    SlangNVVMBuilderAPI_V3& api,
    SlangNVVMBuilderFeature_3 firstFeature)
{
    for (uint32_t feature = firstFeature; feature < SLANG_NVVM_BUILDER_FEATURE_COUNT_3; ++feature)
    {
...
SLANG_UNIT_TEST(nvvmIRBuilderNegotiatesV3Features)
{
    ...
    const uint32_t floatingUnaryOffset = sizeof(void*) == 8 ? 464u : 276u;
    const uint32_t floatingUnaryMinimumSize = sizeof(void*) == 8 ? 472u : 280u;
    const uint32_t floatingCompareOffset = sizeof(void*) == 8 ? 472u : 280u;
    ...
    const uint32_t callOffset = sizeof(void*) == 8 ? 504u : 296u;
    const uint32_t valueReturnOffset = sizeof(void*) == 8 ? 512u : 300u;
    const uint32_t scalarFunctionMinimumSize = sizeof(void*) == 8 ? 520u : 304u;
    const uint32_t intrinsicOffset = sizeof(void*) == 8 ? 520u : 304u;
    const uint32_t intrinsicMinimumSize = sizeof(void*) == 8 ? 528u : 308u;
    const uint32_t completeSize = intrinsicMinimumSize;
    ...
    SLANG_CHECK(offsetof(SlangNVVMBuilderAPI_V3, emitFloatingUnary) == floatingUnaryOffset);
    ...
    SLANG_CHECK(offsetof(SlangNVVMBuilderAPI_V3, emitCall) == callOffset);
    SLANG_CHECK(offsetof(SlangNVVMBuilderAPI_V3, emitValueReturn) == valueReturnOffset);
    SLANG_CHECK(offsetof(SlangNVVMBuilderAPI_V3, emitIntrinsic) == intrinsicOffset);
    ...
    SLANG_CHECK(
        SLANG_NVVM_BUILDER_API_V3_SCALAR_FLOAT32_NEGATE_MIN_SIZE == floatingUnaryMinimumSize);
    SLANG_CHECK(
        SLANG_NVVM_BUILDER_API_V3_SCALAR_FLOAT32_EQUAL_MIN_SIZE == floatingCompareMinimumSize);
    SLANG_CHECK(SLANG_NVVM_BUILDER_API_V3_FLOATING_COMPARE_MIN_SIZE == floatingCompareMinimumSize);
    SLANG_CHECK(
    ...
    SLANG_CHECK(
        SLANG_NVVM_BUILDER_API_V3_GENERIC_SCALAR_FUNCTIONS_MIN_SIZE == scalarFunctionMinimumSize);
    ...
    SLANG_CHECK(SLANG_NVVM_BUILDER_API_V3_WAVE_READ_LANE_AT_FLOAT_MIN_SIZE == intrinsicMinimumSize);
    SLANG_CHECK(SLANG_NVVM_BUILDER_API_V3_WAVE_MASK_BALLOT_MIN_SIZE == intrinsicMinimumSize);
    SLANG_CHECK(
        SLANG_NVVM_BUILDER_API_V3_WAVE_READ_LANE_FIRST_UINT_MIN_SIZE == intrinsicMinimumSize);
    SLANG_CHECK(
        SLANG_NVVM_BUILDER_API_V3_WAVE_READ_LANE_FIRST_INT_MIN_SIZE == intrinsicMinimumSize);
    SLANG_CHECK(
        SLANG_NVVM_BUILDER_API_V3_WAVE_READ_LANE_FIRST_FLOAT_MIN_SIZE == intrinsicMinimumSize);
    SLANG_CHECK(SLANG_NVVM_BUILDER_API_V3_WAVE_MASK_IS_FIRST_LANE_MIN_SIZE == intrinsicMinimumSize);
...
SLANG_UNIT_TEST(nvvmIRBuilderNegotiatesFloat32AddAPI)
{
    ...
    gFakeNVVMBuilder.reset();
    {
        ...
        api.features.words[SLANG_NVVM_BUILDER_FEATURE_SCALAR_FLOAT32_NEGATE / 64u] &=
            ~(uint64_t(1) << (SLANG_NVVM_BUILDER_FEATURE_SCALAR_FLOAT32_NEGATE % 64u));
        api.features.words[SLANG_NVVM_BUILDER_FEATURE_SCALAR_FLOAT32_EQUAL / 64u] &=
            ~(uint64_t(1) << (SLANG_NVVM_BUILDER_FEATURE_SCALAR_FLOAT32_EQUAL % 64u));
        api.features.words[SLANG_NVVM_BUILDER_FEATURE_SCALAR_FLOAT32_NOT_EQUAL / 64u] &=
            ~(uint64_t(1) << (SLANG_NVVM_BUILDER_FEATURE_SCALAR_FLOAT32_NOT_EQUAL % 64u));
        api.features.words[SLANG_NVVM_BUILDER_FEATURE_SCALAR_FLOAT32_ORDERED_GREATER_THAN / 64u] &=
            ~(uint64_t(1)
              << (SLANG_NVVM_BUILDER_FEATURE_SCALAR_FLOAT32_ORDERED_GREATER_THAN % 64u));
        api.features.words[SLANG_NVVM_BUILDER_FEATURE_SCALAR_FLOAT32_ORDERED_LESS_EQUAL / 64u] &=
            ~(uint64_t(1) << (SLANG_NVVM_BUILDER_FEATURE_SCALAR_FLOAT32_ORDERED_LESS_EQUAL % 64u));
        api.features.words[SLANG_NVVM_BUILDER_FEATURE_SCALAR_FLOAT32_ORDERED_GREATER_EQUAL / 64u] &=
            ~(uint64_t(1)
              << (SLANG_NVVM_BUILDER_FEATURE_SCALAR_FLOAT32_ORDERED_GREATER_EQUAL % 64u));
        api.features.words[SLANG_NVVM_BUILDER_FEATURE_SCALAR_FLOAT32_ORDERED_LESS_THAN / 64u] &=
        ...
        api.features.words[SLANG_NVVM_BUILDER_FEATURE_GENERIC_SCALAR_FUNCTIONS / 64u] &=
            ~(uint64_t(1) << (SLANG_NVVM_BUILDER_FEATURE_GENERIC_SCALAR_FUNCTIONS % 64u));
        api.features.words[SLANG_NVVM_BUILDER_FEATURE_WAVE_LANE_INDEX / 64u] &=
            ~(uint64_t(1) << (SLANG_NVVM_BUILDER_FEATURE_WAVE_LANE_INDEX % 64u));
        api.features.words[SLANG_NVVM_BUILDER_FEATURE_WAVE_LANE_COUNT / 64u] &=
        ...
        api.features.words[SLANG_NVVM_BUILDER_FEATURE_WAVE_READ_LANE_AT_FLOAT / 64u] &=
            ~(uint64_t(1) << (SLANG_NVVM_BUILDER_FEATURE_WAVE_READ_LANE_AT_FLOAT % 64u));
        api.features.words[SLANG_NVVM_BUILDER_FEATURE_WAVE_MASK_BALLOT / 64u] &=
            ~(uint64_t(1) << (SLANG_NVVM_BUILDER_FEATURE_WAVE_MASK_BALLOT % 64u));
        _clearNVVMBuilderFeaturesFrom(api, SLANG_NVVM_BUILDER_FEATURE_WAVE_READ_LANE_FIRST_UINT);
    ...
    NVVMFloat32ArithmeticTestOperation testOperation)
{
    ...
    SLANG_RELEASE_ASSERT(testCase.operandCount == 2);
...
{
    _runNVVMIRBuilderNegotiatesFloat32BinaryAPI(NVVMFloat32ArithmeticTestOperation::Subtract);
}

SLANG_UNIT_TEST(nvvmIRBuilderNegotiatesFloat32MultiplyAPI)
{
    _runNVVMIRBuilderNegotiatesFloat32BinaryAPI(NVVMFloat32ArithmeticTestOperation::Multiply);
}

SLANG_UNIT_TEST(nvvmIRBuilderNegotiatesFloat32DivideAPI)
{
    _runNVVMIRBuilderNegotiatesFloat32BinaryAPI(NVVMFloat32ArithmeticTestOperation::Divide);
}

SLANG_UNIT_TEST(nvvmIRBuilderNegotiatesFloat32NegateAPI)
{
    // An exact Slice 35 table remains valid when it does not advertise the appended semantic.
    gFakeNVVMBuilder.reset();
    {
        ComPtr<ISlangSharedLibrary> library(new FakeNVVMBuilderLibrary);
        SlangNVVMBuilderAPI_V3 api = _makeFakeNVVMBuilderAPIV3();
        ...
        NVVMIRBuilder builder;
        SLANG_CHECK_ABORT(SLANG_SUCCEEDED(NVVMIRBuilder::initialize(api, library, builder)));
        SLANG_CHECK(!builder.supportsFeature(SLANG_NVVM_BUILDER_FEATURE_SCALAR_FLOAT32_NEGATE));
        SlangNVVMValueHandle_1 result = _getFakeNVVMBuilderFunction();
        SLANG_CHECK(
    ...
    gFakeNVVMBuilder.reset();
    {
        ComPtr<ISlangSharedLibrary> library(new FakeNVVMBuilderLibrary);
        SlangNVVMBuilderAPI_V3 api = _makeFakeNVVMBuilderAPIV3();
        api.structureSize = uint32_t(offsetof(SlangNVVMBuilderAPI_V3, emitFloatingUnary) + 1);
        NVVMIRBuilder builder;
        SLANG_CHECK(NVVMIRBuilder::initialize(api, library, builder) == SLANG_E_NO_INTERFACE);
        SLANG_CHECK(!builder.isInitialized());
    }
    ...
    gFakeNVVMBuilder.reset();
    {
        ...
        SLANG_CHECK_ABORT(SLANG_SUCCEEDED(builder.declareFunction(
            _getFakeNVVMBuilderModule(),
            functionType,
            toSlice("fakeFloatNegate"),
            function)));
        SlangNVVMValueHandle_1 value = nullptr;
        SLANG_CHECK_ABORT(SLANG_SUCCEEDED(
            builder.getFunctionParameter(_getFakeNVVMBuilderModule(), function, 0, value)));
        SlangNVVMBlockHandle_1 block = nullptr;
        SLANG_CHECK_ABORT(SLANG_SUCCEEDED(
            builder.createBlock(_getFakeNVVMBuilderModule(), function, toSlice("entry"), block)));
        SLANG_CHECK_ABORT(
        ...
        SlangNVVMValueHandle_1 result = nullptr;
        SLANG_CHECK_ABORT(SLANG_SUCCEEDED(builder.emitFloatingUnary(
            _getFakeNVVMBuilderModule(),
            SLANG_NVVM_FLOATING_UNARY_OP_NEGATE,
            value,
            result)));
        ...
        SLANG_CHECK(
            builder.emitFloatingUnary(
                _getFakeNVVMBuilderModule(),
                SlangNVVMFloatingUnaryOp_3(99),
                value,
                result) == SLANG_E_INVALID_ARG);
        SLANG_CHECK(result == nullptr);
        SLANG_CHECK(
            gFakeNVVMBuilder
                .scalarV3FamilyCallCounts[Index(FakeNVVMBuilderScalarFamily::FloatingUnary)] == 1);
...
SLANG_UNIT_TEST(nvvmIRBuilderNegotiatesFloat32EqualAPI)
{
    // An exact Slice 36 table remains valid when it does not advertise the appended semantic.
    gFakeNVVMBuilder.reset();
    {
        ComPtr<ISlangSharedLibrary> library(new FakeNVVMBuilderLibrary);
        SlangNVVMBuilderAPI_V3 api = _makeFakeNVVMBuilderAPIV3();
        ...
        NVVMIRBuilder builder;
        SLANG_CHECK_ABORT(SLANG_SUCCEEDED(NVVMIRBuilder::initialize(api, library, builder)));
        SLANG_CHECK(!builder.supportsFeature(SLANG_NVVM_BUILDER_FEATURE_SCALAR_FLOAT32_EQUAL));
        SlangNVVMValueHandle_1 result = _getFakeNVVMBuilderFunction();
        SLANG_CHECK(
    ...
    const uint32_t floatingCompareOffset =
        uint32_t(offsetof(SlangNVVMBuilderAPI_V3, emitFloatingCompare));
    for (uint32_t partialSize = floatingCompareOffset;
         partialSize < SLANG_NVVM_BUILDER_API_V3_SCALAR_FLOAT32_EQUAL_MIN_SIZE;
         ++partialSize)
    {
        gFakeNVVMBuilder.reset();
        ComPtr<ISlangSharedLibrary> library(new FakeNVVMBuilderLibrary);
    ...
    gFakeNVVMBuilder.reset();
    {
        ComPtr<ISlangSharedLibrary> library(new FakeNVVMBuilderLibrary);
        SlangNVVMBuilderAPI_V3 api = _makeFakeNVVMBuilderAPIV3();
        api.emitFloatingCompare = nullptr;
        NVVMIRBuilder builder;
        SLANG_CHECK(NVVMIRBuilder::initialize(api, library, builder) == SLANG_E_NO_INTERFACE);
        SLANG_CHECK(!builder.isInitialized());
    }
    ...
    gFakeNVVMBuilder.reset();
    {
        ...
        SlangNVVMValueHandle_1 result = nullptr;
        SLANG_CHECK_ABORT(SLANG_SUCCEEDED(builder.emitFloatingCompare(
            _getFakeNVVMBuilderModule(),
            SLANG_NVVM_FLOATING_COMPARE_OP_ORDERED_EQUAL,
            left,
            right,
...
static void _runNVVMIRBuilderNegotiatesFloat32ComparisonAPI(
    NVVMFloat32ComparisonTestOperation testOperation)
{
    ...
    // Earlier feature sets remain loadable and reject this semantic before provider dispatch.
    gFakeNVVMBuilder.reset();
    {
        ComPtr<ISlangSharedLibrary> library(new FakeNVVMBuilderLibrary);
        SlangNVVMBuilderAPI_V3 api = _makeFakeNVVMBuilderAPIV3();
        ...
        NVVMIRBuilder builder;
        SLANG_CHECK_ABORT(SLANG_SUCCEEDED(NVVMIRBuilder::initialize(api, library, builder)));
        SLANG_CHECK(!builder.supportsFeature(testCase.feature));
        SlangNVVMValueHandle_1 result = _getFakeNVVMBuilderFunction();
        SLANG_CHECK(
            builder.emitFloatingCompare(
                _getFakeNVVMBuilderModule(),
    ...
    gFakeNVVMBuilder.reset();
    {
        ...
        SLANG_CHECK(
            gFakeNVVMBuilder
                .scalarV3FamilyCallCounts[Index(FakeNVVMBuilderScalarFamily::FloatingCompare)] ==
            1);
        SLANG_CHECK(gFakeNVVMBuilder.scalarV3Operations.getLast().operation == testCase.operation);
    }
    SLANG_CHECK(gFakeNVVMBuilder.liveLibraryCount == 0);
}

#define NVVM_FLOAT32_COMPARISON_NEGOTIATION_TEST(NAME, OPERATION) \
    SLANG_UNIT_TEST(NAME)                                         \
    {                                                             \
        _runNVVMIRBuilderNegotiatesFloat32ComparisonAPI(          \
            NVVMFloat32ComparisonTestOperation::OPERATION);       \
    }

NVVM_FLOAT32_COMPARISON_NEGOTIATION_TEST(
    nvvmIRBuilderNegotiatesFloat32NotEqualAPI,
    UnorderedNotEqual)
NVVM_FLOAT32_COMPARISON_NEGOTIATION_TEST(
    nvvmIRBuilderNegotiatesFloat32GreaterThanAPI,
    OrderedGreaterThan)
NVVM_FLOAT32_COMPARISON_NEGOTIATION_TEST(
    nvvmIRBuilderNegotiatesFloat32LessEqualAPI,
    OrderedLessEqual)
NVVM_FLOAT32_COMPARISON_NEGOTIATION_TEST(
    nvvmIRBuilderNegotiatesFloat32GreaterEqualAPI,
    OrderedGreaterEqual)
NVVM_FLOAT32_COMPARISON_NEGOTIATION_TEST(nvvmIRBuilderNegotiatesFloat32LessThanAPI, OrderedLessThan)

#undef NVVM_FLOAT32_COMPARISON_NEGOTIATION_TEST

SLANG_UNIT_TEST(nvvmIRBuilderNegotiatesFloat32ConstantAPI)
{
    // The exact Slice 42 table remains valid when it does not advertise the appended semantic.
    gFakeNVVMBuilder.reset();
    {
        ComPtr<ISlangSharedLibrary> library(new FakeNVVMBuilderLibrary);
        SlangNVVMBuilderAPI_V3 api = _makeFakeNVVMBuilderAPIV3();
    ...
    const uint32_t constantOffset =
        uint32_t(offsetof(SlangNVVMBuilderAPI_V3, getFloatingPointConstant));
    for (uint32_t partialSize = constantOffset;
         partialSize < SLANG_NVVM_BUILDER_API_V3_SCALAR_FLOAT32_CONSTANT_MIN_SIZE;
         ++partialSize)
    {
        gFakeNVVMBuilder.reset();
        ComPtr<ISlangSharedLibrary> library(new FakeNVVMBuilderLibrary);
    ...
    gFakeNVVMBuilder.reset();
    {
        ComPtr<ISlangSharedLibrary> library(new FakeNVVMBuilderLibrary);
        SlangNVVMBuilderAPI_V3 api = _makeFakeNVVMBuilderAPIV3();
        api.getFloatingPointConstant = nullptr;
        NVVMIRBuilder builder;
        SLANG_CHECK(NVVMIRBuilder::initialize(api, library, builder) == SLANG_E_NO_INTERFACE);
        SLANG_CHECK(!builder.isInitialized());
    }
    ...
    gFakeNVVMBuilder.reset();
    {
        ...
        SLANG_CHECK_ABORT(SLANG_SUCCEEDED(builder.getFloatingPointConstant(
            ...
            UINT64_C(0x3fc00000),
            value)));
        SLANG_CHECK(value == _getFakeNVVMBuilderFloatingPointConstant());
        ...
        gFakeNVVMBuilder.failFloatingPointConstantAfterWrite = true;
        value = _getFakeNVVMBuilderFunction();
        SLANG_CHECK(
            builder.getFloatingPointConstant(
                _getFakeNVVMBuilderModule(),
...
SLANG_UNIT_TEST(nvvmIRBuilderNegotiatesGenericScalarFunctionAPI)
{
    // The exact Slice 44 table remains valid when it does not advertise the appended pair.
    gFakeNVVMBuilder.reset();
    {
        ComPtr<ISlangSharedLibrary> library(new FakeNVVMBuilderLibrary);
        SlangNVVMBuilderAPI_V3 api = _makeFakeNVVMBuilderAPIV3();
        ...
        SlangNVVMValueHandle_1 value = _getFakeNVVMBuilderFunction();
        SLANG_CHECK(
            builder.emitCall(_getFakeNVVMBuilderModule(), nullptr, nullptr, 0, value) ==
            SLANG_E_NOT_AVAILABLE);
        SLANG_CHECK(value == nullptr);
        SLANG_CHECK(
            builder.emitValueReturn(_getFakeNVVMBuilderModule(), nullptr) == SLANG_E_NOT_AVAILABLE);
        SLANG_CHECK(gFakeNVVMBuilder.emitCallCallCount == 0);
        SLANG_CHECK(gFakeNVVMBuilder.emitValueReturnCallCount == 0);
    }
    SLANG_CHECK(gFakeNVVMBuilder.liveLibraryCount == 0);

    const uint32_t callOffset = uint32_t(offsetof(SlangNVVMBuilderAPI_V3, emitCall));
    for (uint32_t partialSize = callOffset;
         partialSize < SLANG_NVVM_BUILDER_API_V3_GENERIC_SCALAR_FUNCTIONS_MIN_SIZE;
         ++partialSize)
    {
        gFakeNVVMBuilder.reset();
        ComPtr<ISlangSharedLibrary> library(new FakeNVVMBuilderLibrary);
    ...
    for (Index callbackIndex = 0; callbackIndex < 3; ++callbackIndex)
    {
        gFakeNVVMBuilder.reset();
        ComPtr<ISlangSharedLibrary> library(new FakeNVVMBuilderLibrary);
        SlangNVVMBuilderAPI_V3 api = _makeFakeNVVMBuilderAPIV3();
        if (callbackIndex == 0)
            ...
            api.emitCall = nullptr;
        else
            api.emitValueReturn = nullptr;
        NVVMIRBuilder builder;
        SLANG_CHECK(NVVMIRBuilder::initialize(api, library, builder) == SLANG_E_NO_INTERFACE);
        SLANG_CHECK(!builder.isInitialized());
    }
    ...
    gFakeNVVMBuilder.reset();
    {
        ...
        const SlangNVVMTypeHandle_1 helperParameterTypes[] = {
            _getFakeNVVMBuilderFloatType(),
            _getFakeNVVMBuilderFloatType(),
        };
        ...
        SLANG_CHECK_ABORT(SLANG_SUCCEEDED(builder.declareFunction(
            _getFakeNVVMBuilderModule(),
            helperType,
            toSlice("fakeFloatHelper"),
            helper)));
        SlangNVVMValueHandle_1 helperLeft = nullptr;
        SlangNVVMValueHandle_1 helperRight = nullptr;
        SLANG_CHECK_ABORT(SLANG_SUCCEEDED(
            builder.getFunctionParameter(_getFakeNVVMBuilderModule(), helper, 0, helperLeft)));
        SLANG_CHECK_ABORT(SLANG_SUCCEEDED(
            builder.getFunctionParameter(_getFakeNVVMBuilderModule(), helper, 1, helperRight)));
        ...
        SLANG_CHECK_ABORT(SLANG_SUCCEEDED(builder.declareFunction(
            _getFakeNVVMBuilderModule(),
            callerType,
            toSlice("fakeFloatCaller"),
            caller)));
        SlangNVVMValueHandle_1 callerLeft = nullptr;
        SlangNVVMValueHandle_1 callerRight = nullptr;
        SLANG_CHECK_ABORT(SLANG_SUCCEEDED(
            builder.getFunctionParameter(_getFakeNVVMBuilderModule(), caller, 0, callerLeft)));
        SLANG_CHECK_ABORT(SLANG_SUCCEEDED(
            builder.getFunctionParameter(_getFakeNVVMBuilderModule(), caller, 1, callerRight)));

        SlangNVVMBlockHandle_1 helperBlock = nullptr;
        SlangNVVMBlockHandle_1 callerBlock = nullptr;
        SLANG_CHECK_ABORT(SLANG_SUCCEEDED(builder.createBlock(
            _getFakeNVVMBuilderModule(),
            helper,
            toSlice("helper.entry"),
            helperBlock)));
        SLANG_CHECK_ABORT(SLANG_SUCCEEDED(builder.createBlock(
            _getFakeNVVMBuilderModule(),
            caller,
            toSlice("caller.entry"),
            callerBlock)));

        SLANG_CHECK_ABORT(
            SLANG_SUCCEEDED(builder.setInsertBlock(_getFakeNVVMBuilderModule(), helperBlock)));
        ...
        SLANG_CHECK_ABORT(SLANG_SUCCEEDED(builder.emitFloatingBinary(
            ...
            sum)));
        SLANG_CHECK_ABORT(
            SLANG_SUCCEEDED(builder.emitValueReturn(_getFakeNVVMBuilderModule(), sum)));

        SLANG_CHECK_ABORT(
            SLANG_SUCCEEDED(builder.setInsertBlock(_getFakeNVVMBuilderModule(), callerBlock)));
        const SlangNVVMValueHandle_1 arguments[] = {callerLeft, callerRight};
        SlangNVVMValueHandle_1 callResult = nullptr;
        SLANG_CHECK_ABORT(SLANG_SUCCEEDED(builder.emitCall(
            _getFakeNVVMBuilderModule(),
            helper,
            arguments,
            SLANG_COUNT_OF(arguments),
            callResult)));
        SLANG_CHECK(callResult == _getFakeNVVMBuilderCall());
        SLANG_CHECK(_isFakeNVVMBuilderFloatValue(callResult));
        SLANG_CHECK(gFakeNVVMBuilder.callArgumentValueRefs.getCount() == 2);
        SLANG_CHECK(gFakeNVVMBuilder.callArgumentValueRefs[0].functionIndex == 1);
        SLANG_CHECK(gFakeNVVMBuilder.callArgumentValueRefs[0].index == 0);
        SLANG_CHECK(gFakeNVVMBuilder.callArgumentValueRefs[1].functionIndex == 1);
        SLANG_CHECK(gFakeNVVMBuilder.callArgumentValueRefs[1].index == 1);
        ...
        callResult = _getFakeNVVMBuilderFunction();
        SLANG_CHECK(
            builder.emitCall(_getFakeNVVMBuilderModule(), helper, nullptr, 2, callResult) ==
            SLANG_E_INVALID_ARG);
        SLANG_CHECK(callResult == nullptr);

        gFakeNVVMBuilder.failCallAfterWrite = true;
        callResult = _getFakeNVVMBuilderFunction();
        SLANG_CHECK(
            builder.emitCall(
                _getFakeNVVMBuilderModule(),
                ...
                arguments,
                SLANG_COUNT_OF(arguments),
                callResult) == SLANG_FAIL);
        SLANG_CHECK(callResult == nullptr);

        SLANG_CHECK_ABORT(
            SLANG_SUCCEEDED(builder.setInsertBlock(_getFakeNVVMBuilderModule(), helperBlock)));
        gFakeNVVMBuilder.failValueReturn = true;
        SLANG_CHECK(builder.emitValueReturn(_getFakeNVVMBuilderModule(), sum) == SLANG_FAIL);
        SLANG_CHECK(gFakeNVVMBuilder.scalarReturnValueRefs.getCount() == 1);
    }
    SLANG_CHECK(gFakeNVVMBuilder.liveLibraryCount == 0);
}

SLANG_UNIT_TEST(nvvmIRBuilderNegotiatesIntrinsicAPI)
{
    // The exact Slice 45 table remains valid without the appended intrinsic family.
    gFakeNVVMBuilder.reset();
    {
        ComPtr<ISlangSharedLibrary> library(new FakeNVVMBuilderLibrary);
        SlangNVVMBuilderAPI_V3 api = _makeFakeNVVMBuilderAPIV3();
    ...
    const uint32_t intrinsicOffset = uint32_t(offsetof(SlangNVVMBuilderAPI_V3, emitIntrinsic));
    for (uint32_t partialSize = intrinsicOffset;
         partialSize < SLANG_NVVM_BUILDER_API_V3_WAVE_LANE_INDEX_MIN_SIZE;
         ++partialSize)
    {
        gFakeNVVMBuilder.reset();
        ComPtr<ISlangSharedLibrary> library(new FakeNVVMBuilderLibrary);
    ...
    gFakeNVVMBuilder.reset();
    {
        ...
        gFakeNVVMBuilder.failIntrinsicAfterWrite = true;
        value = _getFakeNVVMBuilderFunction();
        SLANG_CHECK(
            builder.emitIntrinsic(
                _getFakeNVVMBuilderModule(),
                ...
                value) == SLANG_FAIL);
        SLANG_CHECK(value == nullptr);
        gFakeNVVMBuilder.failIntrinsicAfterWrite = false;

        gFakeNVVMBuilder.returnNullIntrinsic = true;
        value = _getFakeNVVMBuilderFunction();
        SLANG_CHECK(
            builder.emitIntrinsic(
                _getFakeNVVMBuilderModule(),
...
SLANG_UNIT_TEST(nvvmIRBuilderNegotiatesWaveLaneCountOperation)
{
    // The exact Slice 46 feature set and table remain valid without lane count.
    gFakeNVVMBuilder.reset();
    {
        ComPtr<ISlangSharedLibrary> library(new FakeNVVMBuilderLibrary);
        SlangNVVMBuilderAPI_V3 api = _makeFakeNVVMBuilderAPIV3();
        ...
        SLANG_CHECK(!builder.supportsFeature(SLANG_NVVM_BUILDER_FEATURE_WAVE_LANE_COUNT));
        SlangNVVMValueHandle_1 value = _getFakeNVVMBuilderFunction();
        SLANG_CHECK(
            builder.emitIntrinsic(
                _getFakeNVVMBuilderModule(),
    ...
    gFakeNVVMBuilder.reset();
    {
        ...
        SLANG_CHECK_ABORT(SLANG_SUCCEEDED(_populateWaveLaneCountKernel(
            builder,
            _getFakeNVVMBuilderModule(),
            toSlice("fakeWaveLaneCount"),
            toSlice("fakeReadLaneIndex"),
            toSlice("fakeReadLaneCount"))));
...
SLANG_UNIT_TEST(nvvmIRBuilderNegotiatesWaveReadLaneAtUIntOperation)
{
    // The exact Slice 47 feature set and table remain valid without UInt lane shuffle.
    gFakeNVVMBuilder.reset();
    {
        ComPtr<ISlangSharedLibrary> library(new FakeNVVMBuilderLibrary);
        SlangNVVMBuilderAPI_V3 api = _makeFakeNVVMBuilderAPIV3();
    ...
    gFakeNVVMBuilder.reset();
    {
        ...
        SLANG_CHECK_ABORT(SLANG_SUCCEEDED(_populateWaveReadLaneAtUIntKernel(
            builder,
            _getFakeNVVMBuilderModule(),
            toSlice("fakeWaveReadLaneAtUInt"),
            toSlice("fakeReadLaneIndex"),
            toSlice("fakeReadLaneAtUInt"))));
        SLANG_CHECK(gFakeNVVMBuilder.emitIntrinsicCallCount == 2);
        SLANG_CHECK(gFakeNVVMBuilder.intrinsicOperations.getCount() == 2);
        SLANG_CHECK(
            gFakeNVVMBuilder.intrinsicOperations[0] == SLANG_NVVM_INTRINSIC_OP_WAVE_LANE_INDEX);
...
SLANG_UNIT_TEST(nvvmIRBuilderNegotiatesWaveReadLaneAtFloatOperation)
{
    // The exact Slice 49 feature set and table remain valid without float32 lane shuffle.
    gFakeNVVMBuilder.reset();
    {
        ComPtr<ISlangSharedLibrary> library(new FakeNVVMBuilderLibrary);
        SlangNVVMBuilderAPI_V3 api = _makeFakeNVVMBuilderAPIV3();
        ...
        SLANG_CHECK(builder.supportsFeature(SLANG_NVVM_BUILDER_FEATURE_WAVE_READ_LANE_AT_UINT));
        SLANG_CHECK(builder.supportsFeature(SLANG_NVVM_BUILDER_FEATURE_WAVE_READ_LANE_AT_INT));
        SLANG_CHECK(!builder.supportsFeature(SLANG_NVVM_BUILDER_FEATURE_WAVE_READ_LANE_AT_FLOAT));
        SlangNVVMValueHandle_1 value = _getFakeNVVMBuilderFunction();
        SlangNVVMValueHandle_1 arguments[3] = {
            _getFakeNVVMBuilderParameter(0),
            _getFakeNVVMBuilderParameter(1),
    ...
    gFakeNVVMBuilder.reset();
    {
        ...
        SLANG_CHECK_ABORT(SLANG_SUCCEEDED(_populateWaveReadLaneAtFloatKernel(
            builder,
            _getFakeNVVMBuilderModule(),
            toSlice("fakeWaveReadLaneAtFloat"),
            toSlice("fakeReadLaneIndex"),
            toSlice("fakeReadLaneAtFloat"))));
        SLANG_CHECK(gFakeNVVMBuilder.emitIntrinsicCallCount == 2);
        SLANG_CHECK(gFakeNVVMBuilder.intrinsicOperations.getCount() == 2);
        SLANG_CHECK(
            gFakeNVVMBuilder.intrinsicOperations[0] == SLANG_NVVM_INTRINSIC_OP_WAVE_LANE_INDEX);
...
SLANG_UNIT_TEST(nvvmIRBuilderNegotiatesWaveMaskBallotOperation)
{
    // The exact Slice 50 feature set and table remain valid without synchronized ballot support.
    gFakeNVVMBuilder.reset();
    {
        ComPtr<ISlangSharedLibrary> library(new FakeNVVMBuilderLibrary);
        SlangNVVMBuilderAPI_V3 api = _makeFakeNVVMBuilderAPIV3();
        ...
        SLANG_CHECK(builder.supportsFeature(SLANG_NVVM_BUILDER_FEATURE_WAVE_LANE_INDEX));
        SLANG_CHECK(builder.supportsFeature(SLANG_NVVM_BUILDER_FEATURE_WAVE_READ_LANE_AT_FLOAT));
        SLANG_CHECK(!builder.supportsFeature(SLANG_NVVM_BUILDER_FEATURE_WAVE_MASK_BALLOT));
        SlangNVVMValueHandle_1 value = _getFakeNVVMBuilderFunction();
    ...
    gFakeNVVMBuilder.reset();
    {
        ...
        SLANG_CHECK_ABORT(SLANG_SUCCEEDED(
            NVVMIRBuilder::initialize(_makeFakeNVVMBuilderAPIV3(), library, builder)));
        SLANG_CHECK_ABORT(SLANG_SUCCEEDED(_populateWaveActiveMaskKernel(
            builder,
            _getFakeNVVMBuilderModule(),
            toSlice("fakeWaveActiveMask"))));
        SLANG_CHECK(gFakeNVVMBuilder.emitIntrinsicCallCount == 2);
        SLANG_CHECK(gFakeNVVMBuilder.intrinsicOperations.getCount() == 2);
        SLANG_CHECK(
            gFakeNVVMBuilder.intrinsicOperations[0] == SLANG_NVVM_INTRINSIC_OP_WAVE_LANE_INDEX);
        SLANG_CHECK(
            gFakeNVVMBuilder.intrinsicOperations[1] == SLANG_NVVM_INTRINSIC_OP_WAVE_MASK_BALLOT);
        SLANG_CHECK(gFakeNVVMBuilder.intrinsicArgumentCounts[0] == 0);
        SLANG_CHECK(gFakeNVVMBuilder.intrinsicArgumentCounts[1] == 2);
        ...
        SLANG_CHECK(gFakeNVVMBuilder.intrinsicArgumentValueRefs.getCount() == 2);
        SLANG_CHECK(
            gFakeNVVMBuilder.intrinsicArgumentValueRefs[0].kind ==
            FakeNVVMBuilderValueKind::IntegerConstant);
        SLANG_CHECK(gFakeNVVMBuilder.intrinsicArgumentValueRefs[0].index == 0);
        SLANG_CHECK(
            gFakeNVVMBuilder.intrinsicArgumentValueRefs[1].kind ==
            FakeNVVMBuilderValueKind::IntegerConstant);
        SLANG_CHECK(gFakeNVVMBuilder.intrinsicArgumentValueRefs[1].index == 1);
        SLANG_CHECK(gFakeNVVMBuilder.emitPointerOffsetCallCount == 1);
        SLANG_CHECK(gFakeNVVMBuilder.emitStoreCallCount == 1);
        SLANG_CHECK(gFakeNVVMBuilder.storeValueRefs[0].kind == FakeNVVMBuilderValueKind::Intrinsic);
    }
    SLANG_CHECK(gFakeNVVMBuilder.liveLibraryCount == 0);
}

// Proves one append-only read-first operation against the exact preceding feature set and through
// the shared two-i32 fake-provider graph.
static void _checkNVVMIRBuilderNegotiatesWaveReadLaneFirstOperation(
    SlangNVVMBuilderFeature_3 requiredFeature,
    ...
    bool usesFloatValue)
{
    gFakeNVVMBuilder.reset();
    {
        ComPtr<ISlangSharedLibrary> library(new FakeNVVMBuilderLibrary);
        SlangNVVMBuilderAPI_V3 api = _makeFakeNVVMBuilderAPIV3();
        for (uint32_t feature = requiredFeature; feature < SLANG_NVVM_BUILDER_FEATURE_COUNT_3;
             ++feature)
        {
            api.features.words[feature / 64u] &= ~(uint64_t(1) << (feature % 64u));
        }
        ...
        SLANG_CHECK_ABORT(SLANG_SUCCEEDED(NVVMIRBuilder::initialize(api, library, builder)));
        SLANG_CHECK(builder.supportsFeature(SLANG_NVVM_BUILDER_FEATURE_WAVE_MASK_BALLOT));
        SLANG_CHECK(!builder.supportsFeature(requiredFeature));
        const SlangNVVMValueHandle_1 arguments[] = {
            _getFakeNVVMBuilderIntegerConstant(0),
            _getFakeNVVMBuilderIntegerConstant(1),
        };
    ...
    gFakeNVVMBuilder.reset();
    {
        ...
        SLANG_CHECK(gFakeNVVMBuilder.intrinsicArgumentCounts[0] == 2);
        SLANG_CHECK(gFakeNVVMBuilder.intrinsicArgumentValueRefs.getCount() == 2);
        SLANG_CHECK(
            gFakeNVVMBuilder.intrinsicArgumentValueRefs[0].kind ==
            FakeNVVMBuilderValueKind::Parameter);
...
SLANG_UNIT_TEST(nvvmIRBuilderNegotiatesWaveReadLaneFirstUIntOperation)
{
    _checkNVVMIRBuilderNegotiatesWaveReadLaneFirstOperation(
        SLANG_NVVM_BUILDER_FEATURE_WAVE_READ_LANE_FIRST_UINT,
        SLANG_NVVM_INTRINSIC_OP_WAVE_READ_LANE_FIRST_UINT,
        toSlice("fakeWaveReadLaneFirstUInt"),
        toSlice("fakeReadLaneFirstUInt"),
        false);
}

SLANG_UNIT_TEST(nvvmIRBuilderNegotiatesWaveReadLaneFirstIntOperation)
{
    _checkNVVMIRBuilderNegotiatesWaveReadLaneFirstOperation(
        SLANG_NVVM_BUILDER_FEATURE_WAVE_READ_LANE_FIRST_INT,
        SLANG_NVVM_INTRINSIC_OP_WAVE_READ_LANE_FIRST_INT,
        toSlice("fakeWaveReadLaneFirstInt"),
        toSlice("fakeReadLaneFirstInt"),
        false);
}

SLANG_UNIT_TEST(nvvmIRBuilderNegotiatesWaveReadLaneFirstFloatOperation)
{
    _checkNVVMIRBuilderNegotiatesWaveReadLaneFirstOperation(
        SLANG_NVVM_BUILDER_FEATURE_WAVE_READ_LANE_FIRST_FLOAT,
        SLANG_NVVM_INTRINSIC_OP_WAVE_READ_LANE_FIRST_FLOAT,
        toSlice("fakeWaveReadLaneFirstFloat"),
        toSlice("fakeReadLaneFirstFloat"),
        true);
}

SLANG_UNIT_TEST(nvvmIRBuilderNegotiatesWaveMaskIsFirstLaneOperation)
{
    // The exact Slice 57 feature set remains valid without masked-is-first-lane support.
    gFakeNVVMBuilder.reset();
    {
        ComPtr<ISlangSharedLibrary> library(new FakeNVVMBuilderLibrary);
        SlangNVVMBuilderAPI_V3 api = _makeFakeNVVMBuilderAPIV3();
        _clearNVVMBuilderFeaturesFrom(api, SLANG_NVVM_BUILDER_FEATURE_WAVE_MASK_IS_FIRST_LANE);
        NVVMIRBuilder builder;
        SLANG_CHECK_ABORT(SLANG_SUCCEEDED(NVVMIRBuilder::initialize(api, library, builder)));
        SLANG_CHECK(builder.supportsFeature(SLANG_NVVM_BUILDER_FEATURE_WAVE_MASK_BALLOT));
        SLANG_CHECK(builder.supportsFeature(SLANG_NVVM_BUILDER_FEATURE_WAVE_READ_LANE_FIRST_FLOAT));
        SLANG_CHECK(!builder.supportsFeature(SLANG_NVVM_BUILDER_FEATURE_WAVE_MASK_IS_FIRST_LANE));
        SlangNVVMValueHandle_1 value = _getFakeNVVMBuilderFunction();
        const SlangNVVMValueHandle_1 mask = _getFakeNVVMBuilderIntegerConstant(0);
        SLANG_CHECK(
            builder.emitIntrinsic(
                _getFakeNVVMBuilderModule(),
    ...
    gFakeNVVMBuilder.reset();
    {
        ...
        SLANG_CHECK_ABORT(SLANG_SUCCEEDED(_populateWaveIsFirstLaneKernel(
            builder,
            _getFakeNVVMBuilderModule(),
            toSlice("fakeWaveIsFirstLane"),
            toSlice("fakeWaveMaskIsFirstLane"))));
        SLANG_CHECK(
            gFakeNVVMBuilder.functionTypeResultKinds[0] == FakeNVVMBuilderResultTypeKind::Boolean);
        SLANG_CHECK(gFakeNVVMBuilder.emitIntrinsicCallCount == 2);
        SLANG_CHECK(gFakeNVVMBuilder.intrinsicOperations.getCount() == 2);
        SLANG_CHECK(
            gFakeNVVMBuilder.intrinsicOperations[0] ==
            SLANG_NVVM_INTRINSIC_OP_WAVE_MASK_IS_FIRST_LANE);
        SLANG_CHECK(
            gFakeNVVMBuilder.intrinsicOperations[1] == SLANG_NVVM_INTRINSIC_OP_WAVE_MASK_BALLOT);
        ...
        SLANG_CHECK(
            gFakeNVVMBuilder.intrinsicArgumentValueRefs[0].kind ==
            FakeNVVMBuilderValueKind::Parameter);
        SLANG_CHECK(gFakeNVVMBuilder.intrinsicArgumentValueRefs[0].functionIndex == 0);
        SLANG_CHECK(gFakeNVVMBuilder.intrinsicArgumentValueRefs[0].index == 0);
        const Index ballotArgumentOffset = gFakeNVVMBuilder.intrinsicArgumentOffsets[1];
        SLANG_CHECK(
            gFakeNVVMBuilder.intrinsicArgumentValueRefs[ballotArgumentOffset].kind ==
            FakeNVVMBuilderValueKind::Parameter);
        SLANG_CHECK(
        ...
        SLANG_CHECK(gFakeNVVMBuilder.emitCallCallCount == 1);
        SLANG_CHECK(
            gFakeNVVMBuilder.callResultTypeKinds[0] == FakeNVVMBuilderScalarTypeKind::Boolean);
        SLANG_CHECK(gFakeNVVMBuilder.emitStoreCallCount == 1);
        SLANG_CHECK(gFakeNVVMBuilder.storeValueRefs[0].kind == FakeNVVMBuilderValueKind::Intrinsic);
    }
    SLANG_CHECK(gFakeNVVMBuilder.liveLibraryCount == 0);
...
SLANG_UNIT_TEST(nvvmIRBuilderNegotiatesWaveMaskAnyTrueOperation)
{
    // The exact Slice 58 feature set remains valid without masked any-true support.
    gFakeNVVMBuilder.reset();
    {
        ComPtr<ISlangSharedLibrary> library(new FakeNVVMBuilderLibrary);
        SlangNVVMBuilderAPI_V3 api = _makeFakeNVVMBuilderAPIV3();
        _clearNVVMBuilderFeaturesFrom(api, SLANG_NVVM_BUILDER_FEATURE_WAVE_MASK_ANY_TRUE);
        NVVMIRBuilder builder;
        SLANG_CHECK_ABORT(SLANG_SUCCEEDED(NVVMIRBuilder::initialize(api, library, builder)));
        SLANG_CHECK(builder.supportsFeature(SLANG_NVVM_BUILDER_FEATURE_WAVE_MASK_BALLOT));
        SLANG_CHECK(builder.supportsFeature(SLANG_NVVM_BUILDER_FEATURE_WAVE_MASK_IS_FIRST_LANE));
        SLANG_CHECK(!builder.supportsFeature(SLANG_NVVM_BUILDER_FEATURE_WAVE_MASK_ANY_TRUE));
        SlangNVVMValueHandle_1 value = _getFakeNVVMBuilderFunction();
        const SlangNVVMValueHandle_1 arguments[] = {
            _getFakeNVVMBuilderIntegerConstant(0),
            _getFakeNVVMBuilderIntegerConstant(1),
    ...
    gFakeNVVMBuilder.reset();
    {
        ...
        SLANG_CHECK_ABORT(SLANG_SUCCEEDED(
            NVVMIRBuilder::initialize(_makeFakeNVVMBuilderAPIV3(), library, builder)));
        SLANG_CHECK_ABORT(SLANG_SUCCEEDED(_populateWaveActiveAnyTrueKernel(
            builder,
            _getFakeNVVMBuilderModule(),
        ...
        SLANG_CHECK(gFakeNVVMBuilder.intrinsicOperations.getCount() == 2);
        SLANG_CHECK(
            gFakeNVVMBuilder.intrinsicOperations[0] == SLANG_NVVM_INTRINSIC_OP_WAVE_MASK_ANY_TRUE);
        SLANG_CHECK(
            gFakeNVVMBuilder.intrinsicOperations[1] == SLANG_NVVM_INTRINSIC_OP_WAVE_MASK_BALLOT);
        SLANG_CHECK(gFakeNVVMBuilder.intrinsicArgumentCounts[0] == 2);
        SLANG_CHECK(gFakeNVVMBuilder.intrinsicArgumentCounts[1] == 2);
        ...
        SLANG_CHECK(
            gFakeNVVMBuilder.intrinsicArgumentValueRefs[1].kind ==
            FakeNVVMBuilderValueKind::Parameter);
        SLANG_CHECK(gFakeNVVMBuilder.intrinsicArgumentValueRefs[1].functionIndex == 0);
        SLANG_CHECK(gFakeNVVMBuilder.intrinsicArgumentValueRefs[1].index == 1);
        const Index ballotArgumentOffset = gFakeNVVMBuilder.intrinsicArgumentOffsets[1];
        SLANG_CHECK(
            gFakeNVVMBuilder.intrinsicArgumentValueRefs[ballotArgumentOffset].kind ==
...
SLANG_UNIT_TEST(nvvmIRBuilderRejectsInvalidFloat32Operations)
{
    ...
    const SlangNVVMValueHandle_1 invalidUnaryOperands[] = {
        integer,
        foreignValue,
        laterValue,
        nullptr,
    };
    for (SlangNVVMValueHandle_1 operand : invalidUnaryOperands)
    {
        SlangNVVMValueHandle_1 output = reinterpret_cast<SlangNVVMValueHandle_1>(uintptr_t(1));
        SLANG_CHECK(
    ...
    SLANG_CHECK_ABORT(SLANG_SUCCEEDED(builder.emitFloatingBinary(
        ...
        right,
        sum)));
    SlangNVVMValueHandle_1 negated = nullptr;
    SLANG_CHECK_ABORT(SLANG_SUCCEEDED(
        builder
            .emitFloatingUnary(scope.module, SLANG_NVVM_FLOATING_UNARY_OP_NEGATE, left, negated)));
    SlangNVVMValueHandle_1 equal = nullptr;
    SLANG_CHECK_ABORT(SLANG_SUCCEEDED(builder.emitFloatingCompare(
        scope.module,
        SLANG_NVVM_FLOATING_COMPARE_OP_ORDERED_EQUAL,
        left,
...
SLANG_UNIT_TEST(nvvmIRBuilderBuildsFloat32ConstantKernel)
{
    NVVMIRBuilder builder;
    _requireRealNVVMBuilder(unitTestContext, builder);
    SLANG_CHECK_ABORT(builder.supportsFeature(SLANG_NVVM_BUILDER_FEATURE_SCALAR_FLOAT32_CONSTANT));
...
SLANG_UNIT_TEST(nvvmIRBuilderBuildsFloat32FunctionKernel)
{
    NVVMIRBuilder builder;
    _requireRealNVVMBuilder(unitTestContext, builder);
    SLANG_CHECK_ABORT(builder.supportsFeature(SLANG_NVVM_BUILDER_FEATURE_GENERIC_SCALAR_FUNCTIONS));
...
SLANG_UNIT_TEST(nvvmIRBuilderBuildsWaveLaneCountKernel)
{
    ...
    _requireRealNVVMBuilder(unitTestContext, builder);
    SLANG_CHECK_ABORT(builder.supportsFeature(SLANG_NVVM_BUILDER_FEATURE_WAVE_LANE_INDEX));
    SLANG_CHECK_ABORT(builder.supportsFeature(SLANG_NVVM_BUILDER_FEATURE_WAVE_LANE_COUNT));
...
SLANG_UNIT_TEST(nvvmIRBuilderBuildsWaveReadLaneAtUIntKernel)
{
    ...
    _requireRealNVVMBuilder(unitTestContext, builder);
    SLANG_CHECK_ABORT(builder.supportsFeature(SLANG_NVVM_BUILDER_FEATURE_WAVE_LANE_INDEX));
    SLANG_CHECK_ABORT(builder.supportsFeature(SLANG_NVVM_BUILDER_FEATURE_WAVE_READ_LANE_AT_UINT));
...
SLANG_UNIT_TEST(nvvmIRBuilderBuildsWaveReadLaneAtFloatKernel)
{
    ...
    _requireRealNVVMBuilder(unitTestContext, builder);
    SLANG_CHECK_ABORT(builder.supportsFeature(SLANG_NVVM_BUILDER_FEATURE_WAVE_LANE_INDEX));
    SLANG_CHECK_ABORT(builder.supportsFeature(SLANG_NVVM_BUILDER_FEATURE_WAVE_READ_LANE_AT_FLOAT));
    ...
    bool usesFloatValue)
{
    NVVMIRBuilder builder;
    _requireRealNVVMBuilder(unitTestContext, builder);
    SLANG_CHECK_ABORT(builder.supportsFeature(requiredFeature));
...
SLANG_UNIT_TEST(nvvmIRBuilderBuildsWaveMaskIsFirstLaneKernel)
{
    NVVMIRBuilder builder;
    _requireRealNVVMBuilder(unitTestContext, builder);
    SLANG_CHECK_ABORT(builder.supportsFeature(SLANG_NVVM_BUILDER_FEATURE_WAVE_MASK_IS_FIRST_LANE));
    SLANG_CHECK_ABORT(builder.supportsFeature(SLANG_NVVM_BUILDER_FEATURE_WAVE_MASK_BALLOT));
...
SLANG_UNIT_TEST(nvvmIRBuilderBuildsWaveMaskAnyTrueKernel)
{
    NVVMIRBuilder builder;
    _requireRealNVVMBuilder(unitTestContext, builder);
    SLANG_CHECK_ABORT(builder.supportsFeature(SLANG_NVVM_BUILDER_FEATURE_WAVE_MASK_ANY_TRUE));
    SLANG_CHECK_ABORT(builder.supportsFeature(SLANG_NVVM_BUILDER_FEATURE_WAVE_MASK_BALLOT));

    ScopedNVVMBuilderModule scope;
    scope.builder = &builder;
    SLANG_CHECK_ABORT(
        SLANG_SUCCEEDED(builder.createModule(toSlice("wave-mask-any-true-module"), scope.module)));
    SLANG_CHECK_ABORT(SLANG_SUCCEEDED(_populateWaveActiveAnyTrueKernel(
        builder,
        scope.module,
    ...
    for (SlangNVVMSerializationFormat_1 format : formats)
    {
        ...
        const UnownedStringSlice text(
            static_cast<const char*>(assembly->getBufferPointer()),
            assembly->getBufferSize());
        SLANG_CHECK(text.indexOf(toSlice("define i1 @waveMaskAnyTrue(i32")) >= 0);
        SLANG_CHECK(text.indexOf(toSlice(", i1")) >= 0);
        SLANG_CHECK(_countOccurrences(text, toSlice("call i1 @llvm.nvvm.vote.any.sync(i32")) == 1);
        SLANG_CHECK(_countOccurrences(text, toSlice("ret i1")) == 1);
        SLANG_CHECK(_countOccurrences(text, toSlice("call i1 @waveMaskAnyTrue")) == 1);
        SLANG_CHECK(_countOccurrences(text, toSlice("call i32 @llvm.nvvm.vote.ballot.sync")) == 1);
        SLANG_CHECK(_countOccurrences(text, toSlice("store i32")) == 1);
        SLANG_CHECK(
            _countOccurrences(text, toSlice("declare i1 @llvm.nvvm.vote.any.sync(i32, i1)")) == 1);
    }
}
