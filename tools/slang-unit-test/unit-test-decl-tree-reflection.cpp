...
SLANG_UNIT_TEST(declTreeReflection)
{
    ...
    SLANG_CHECK(
        secondDecl->getChildrenCount() == 2
    ); // Parameter declarations are children (return type is not)
...

    {
        ...
        SLANG_CHECK(
            getTypeFullName(funcReflection->getParameterByIndex(0)->getType()) == "vector<float,4>"
        );
    }
    ...
    // Check type-lookup-by-name for generic type
    {
        ...
        SLANG_CHECK(
            getTypeFullName(specializationInfo->getConcreteType(typeParam)) == "float"
        ); // specialized type name under the context in which the generic is obtained
        ...
        SLANG_CHECK(
            getTypeFullName(specializationInfo->getConcreteType(typeParam)) == "half"
        ); // specialized type name under the context in which the generic is obtained
