...
SLANG_UNIT_TEST(getDownstreamCompilerPath)
{
    ...
    // Gate the NVVM path on availability, so this test passes both on machines that have NVVM and
    // those that do not (no GPU is required merely to load the NVVM library and recover its path).
    const bool nvvmAvailable =
        SLANG_SUCCEEDED(globalSession->checkPassThroughSupport(SLANG_PASS_THROUGH_NVVM));

    ComPtr<ISlangBlob> nvvmPath;
    const SlangResult nvvmResult =
        globalSession->getDownstreamCompilerPath(SLANG_PASS_THROUGH_NVVM, nvvmPath.writeRef());

    if (nvvmAvailable)
    {
        // NVVM is a shared library, so its path is recoverable from a held symbol.
        SLANG_CHECK(nvvmResult == SLANG_OK);
        SLANG_CHECK_ABORT(nvvmPath != nullptr); // abort rather than deref null below on regression
        SLANG_CHECK(nvvmPath->getBufferSize() > 0);

        // The central contract is that this is the exact loadable library Slang uses, not merely a
        // non-empty string. Prove it: reload the returned path by its platform path, resolve the
        // nvvmVersion entry point, and call it. A path pointing at the wrong library (or a stale
        // string) would fail to load or would not expose nvvmVersion; a real NVVM loads and
        // reports a plausible, non-zero major version.
        const char* nvvmPathStr = static_cast<const char*>(nvvmPath->getBufferPointer());
        SharedLibrary::Handle handle = nullptr;
        SLANG_CHECK(SLANG_SUCCEEDED(SharedLibrary::loadWithPlatformPath(nvvmPathStr, handle)));
        if (handle)
        {
            // nvvmVersion(int* major, int* minor) returns nvvmResult (0 == NVVM_SUCCESS).
            using NvvmVersionFunc = int (*)(int*, int*);
            auto nvvmVersion = reinterpret_cast<NvvmVersionFunc>(
                SharedLibrary::findSymbolAddressByName(handle, "nvvmVersion"));
            SLANG_CHECK(nvvmVersion != nullptr);
            if (nvvmVersion)
            {
                int libMajor = 0;
                int libMinor = 0;
                SLANG_CHECK(nvvmVersion(&libMajor, &libMinor) == 0);
                SLANG_CHECK(libMajor > 0);
            }
            SharedLibrary::unload(handle);
        }
    ...
    {
        SLANG_CHECK(nvvmResult == SLANG_E_NOT_FOUND);
    }
