...
// Set by the fake validator so the tests can tell "returned false" from "was never called".
...
// A shared library that exists only as a symbol table. `validateAvailable` decides whether
// `glslang_validateSPIRV` resolves, which is the whole point of the test.
class FakeGlslangLibrary : public RefObject, public ISlangSharedLibrary
{
public:
    SLANG_REF_OBJECT_IUNKNOWN_ALL

    explicit FakeGlslangLibrary(bool validateAvailable)
        : m_validateAvailable(validateAvailable)
    {
    }
        ...
        SLANG_OVERRIDE
    {
        ...
        {
            return m_validateAvailable ? (void*)fakeValidateReturningFalse : nullptr;
        }
        return nullptr;
    }
...
public:
    ...
    explicit FakeLoader(bool validateAvailable)
        : m_validateAvailable(validateAvailable)
    {
    }
    ...
    loadSharedLibrary(const char* path, ISlangSharedLibrary** outLibrary) SLANG_OVERRIDE
    {
        ...
        ComPtr<ISlangSharedLibrary> library(new FakeGlslangLibrary(m_validateAvailable));
...
{
    ComPtr<ISlangSharedLibraryLoader> loader(new FakeLoader(false));
...
{
    ComPtr<ISlangSharedLibraryLoader> loader(new FakeLoader(true));
    RefPtr<DownstreamCompilerSet> set;
    IDownstreamCompiler* compiler = getFakeSpirvOptCompiler(set, loader);
    SLANG_CHECK_ABORT(compiler != nullptr);
...
// `slang-glslang` whose validator is present or absent per `validateAvailable`, and return whatever
// diagnostics the compile produced.
String compileWithFakeGlslangAndCaptureDiagnostics(bool validateAvailable)
{
    ...
    ComPtr<ISlangSharedLibraryLoader> loader(new FakeLoader(validateAvailable));
...
// capability rather than accuse the shader.
SLANG_UNIT_TEST(spirvValidationUnavailableDiagnosesMissingValidator)
{
    const String diagnosticText = compileWithFakeGlslangAndCaptureDiagnostics(false);
    const UnownedStringSlice diagnosticSlice = diagnosticText.getUnownedSlice();

    // Assert something was captured before asserting what it lacks: the two negative checks below
...
{
    const String diagnosticText = compileWithFakeGlslangAndCaptureDiagnostics(true);
    const UnownedStringSlice diagnosticSlice = diagnosticText.getUnownedSlice();
