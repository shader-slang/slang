    ...
    ComPtr<slang::IComponentType>& outLinkedProgram)
{
    ...
    loadedLib1 = slang_loadModuleFromIRBlob(
        ...
        "MathLibrary",
        "MathLibrary.slang",
        lib1Blob->getBufferPointer(),
        lib1Blob->getBufferSize(),
        nullptr);
    if (!loadedLib1)
        return SLANG_FAIL;
    ...
    loadedLib2 = slang_loadModuleFromIRBlob(
        ...
        "ColorLibrary",
        "ColorLibrary.slang",
        lib2Blob->getBufferPointer(),
        lib2Blob->getBufferSize(),
        nullptr);
    if (!loadedLib2)
        return SLANG_FAIL;
    ...
    slang::IComponentType* components[] =
        {finalModule, entryPoint, inlineModule, loadedLib1, loadedLib2};
    ComPtr<slang::IComponentType> compositeProgram;
    SLANG_RETURN_ON_FAIL(
        session->createCompositeComponentType(components, 5, compositeProgram.writeRef(), nullptr));
...
SLANG_UNIT_TEST(obfuscationWithSeparateDebug)
{
    ...
    slang::CompilerOptionEntry optionsWithSepDebug[] = {
        debugInfoOption,
        obfuscationOption,
        separateDebugOption};
    ...
    sessionDesc1.compilerOptionEntryCount =
