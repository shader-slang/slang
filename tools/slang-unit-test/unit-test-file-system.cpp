...
static SlangResult
_checkFile(ISlangMutableFileSystem* fileSystem, const char* path, const char* contents)
{
...
static SlangResult
_createAndCheckFile(ISlangMutableFileSystem* fileSystem, const char* path, const char* contents)
{
...
static SlangResult
_checkCanonical(ISlangMutableFileSystem* fileSystem, const char* const* paths, Count count)
{
    if (count <= 0)
    {
        return SLANG_FAIL;
...
static SlangResult
_checkSimplifiedPath(ISlangFileSystemExt* fileSystem, const char* path, const char* normalPath)
{
...
SlangResult
_appendPathEntries(ISlangFileSystemExt* fileSystem, const char* inBasePath, List<Entry>& outEntries)
{
...
static SlangResult
_getAllEntries(ISlangFileSystemExt* fileSystem, const char* inBasePath, List<Entry>& outEntries)
{
    outEntries.clear();
...
static SlangResult
_createFileSystem(FileSystemType type, ComPtr<ISlangMutableFileSystem>& outFileSystem)
{
