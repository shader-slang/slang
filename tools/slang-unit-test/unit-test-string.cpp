...
static bool
_areApproximatelyEqual(double a, double b, double fixedEpsilon = 1e-10, int ulpsEpsilon = 100)
{
...
{
    {
        ...
        SLANG_CHECK(
            _checkLines(UnownedStringSlice::fromLiteral(""), checkLines, SLANG_COUNT_OF(checkLines))
        );
    }
    {
        // Will emit no lines
    ...
    }
    {
        Int value;
        SLANG_CHECK(
            SLANG_SUCCEEDED(StringUtil::parseInt(UnownedStringSlice("-10"), value)) && value == -10
        );
        SLANG_CHECK(
...

    {
        UnownedStringSlice values[] =
            {UnownedStringSlice("hello"), UnownedStringSlice("world"), UnownedStringSlice("!")};
