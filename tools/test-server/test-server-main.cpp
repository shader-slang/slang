...
)
{
    ...
    // If so we *don't* use the sharedSession, and create a new stdlib-less session just for this
    // compilation.
    if (TestToolUtil::hasDeferredStdLib(Index(argc - 1), argv + 1))
    {
        SLANG_RETURN_ON_FAIL(
            slang_createGlobalSessionWithoutStdLib(SLANG_API_VERSION, session.writeRef())
        );
    }
