...
def output_artifact_bytes(timed):
    ...
    A load-independent size signal to complement the noisy downstream (nvrtc/dxc)
    wall time: for a fixed input, toolchain, and output path it is deterministic,
    where the wall time is not. (Emitted #line/include paths make the raw byte
    count path-sensitive, so it is comparable within a runner's stable layout, not
    across differing scratch paths.) Bytes rather than line count so it is
