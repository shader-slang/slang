...
| **vector_ops_ptx**                            | dense fixed-width vector operator traffic, no transcendentals               | the **downstream** cost of the prelude's fixed-width vector operators. `codegen_ptx` shares `gen_codegen`, whose body is scalar `float` math, so it instantiates almost none of that operator surface no matter what filler it uses; this workload exists to give the bucket a signal that is actually about those operators (#12621). win32-only and `downstream_required`, as above | `compileInner` (+ downstream wall clock) |
