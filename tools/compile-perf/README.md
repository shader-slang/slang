...
Each `api*` timer wraps exactly **one public API call** (wall clock, measured
in the driver). These names are user-facing (site charts, trend alerts); a
timer's meaning must not drift from this table.

| Timer                    | API call                                                  | Notes                                                                                                                                                    |
| ------------------------ | --------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `apiTotal`               | whole timed section of the mode                           | excludes setup that runs before its scope opens (e.g. module-graph-bin's source load / `writeToFile`, timed as `apiLoadModuleSource` / `apiWriteModule`) |
