...
| **vector_ops_ptx**                            | dense fixed-width vector operator traffic, no transcendentals               | the **downstream** cost of the prelude's fixed-width vector operators. The other downstream workloads use `sin`/`cos` as body filler, which expands to hundreds of PTX instructions each and swamps everything else — so this one exists to give that bucket a signal that is actually about the feature under test (#12621). win32-only and `downstream_required`, as above | `compileInner` (+ downstream wall clock) |
