...
- **`nightly-mdl-perf-test.yml`** — builds tip-of-tree, sweeps into
  ...
  backfilling historical daily points), `samples`, `sweep` (default `true`:
  every night also collects the multi-size scaling ladders), `only`, and `publish`
...
- **`compile-perf-release-sweep.yml`** (`workflow_dispatch`) — downloads prebuilt
  ...
  `since`, `until`, `samples`, `sweep` (default `true`), `force`.

**Sweep publication policy — one page set, all the data.** Both workflows
collect multi-size sweep data by default, but only the LATEST nightly sweep is
rendered and served (`analysis/sweep/`, regenerated by whichever workflow
deploys pages last via `sweep_report.py --latest`). Rendering a page set per
release and per day would multiply the site by ~24 pages per label for little
benefit: the served sweep answers "where is the scaling cost on current
master", while historical scaling questions are answered from the archived
per-release/per-day sweep data on demand — `sweep_report.py --label <tag>`
locally, or `ladder_scaling.py` for the cross-release floor/slope fit table.
