...
# the full n=1 body line (so a broken f-string fails at import, not as a lost data
# point) plus the sin/cos-free invariant anywhere in the source (the #12621 intent).
_cg_src = gen_codegen(1)["codegen.slang"]
assert "    acc = fma(acc, 1.0009, 0.0) + fma(acc, 0.5, 1.0) * 0.25 + " \
    "sqrt(abs(acc) + 1.0);\n" in _cg_src
