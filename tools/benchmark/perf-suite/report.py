...
def main():
    ...
    inline = {p: open(p).read() for p in (svg_norm, svg_abs, svg_pw)}
    ...
    PW = ['<!doctype html><meta charset="utf-8">',
          ...
          f'metric: {args.metric} (ms) · {n_wl} workloads · absolute, linear, '
          'each on its own zero-based y-axis (× = first→last ratio).</p>',
          f'<div class="chart">{inline[svg_pw]}</div>',
          '<p class="small">Generated by perf-suite/report.py. Companion to '
