...
- Build Slang with coverage instrumentation enabled
...
- `LLVM_PROFDATA` - Path to llvm-profdata tool (Linux/Unix only)
- `LLVM_COV` - Path to llvm-cov tool (Linux/Unix only)
...
The LCOV format output can be used with various CI/CD coverage tools:

- **Codecov**: Upload `coverage.lcov`
- **Coveralls**: Upload `coverage.lcov`
- **SonarQube**: Use LLVM coverage format or convert from LCOV
