...
/* static */ SlangResult
D3DUtil::createFactory(DeviceCheckFlags flags, ComPtr<IDXGIFactory>& outFactory)
{
    ...
    {
        auto createFactory2 = (PFN_DXGI_CREATE_FACTORY_2
        )SharedLibrary::findSymbolAddressByName(dxgiModule, "CreateDXGIFactory2");
        if (createFactory2)
        {
            UINT dxgiFlags = 0;
    ...
    {
        auto createFactory = (PFN_DXGI_CREATE_FACTORY
        )SharedLibrary::findSymbolAddressByName(dxgiModule, "CreateDXGIFactory");
        if (!createFactory)
        {
        ...
        for (Index i = 0; i < geomDescs.getCount(); i++)
        {
            ...
            switch (inputGeomDesc.type)
            {
                case IAccelerationStructure::GeometryType::Triangles:
                    ...
                    geomDescs[i].Triangles.Transform3x4 =
                        inputGeomDesc.content.triangles.transform3x4;
                    ...
                    geomDescs[i].Triangles.VertexCount =
                        inputGeomDesc.content.triangles.vertexCount;
                ...
                case IAccelerationStructure::GeometryType::ProcedurePrimitives:
                    ...
                    geomDescs[i].AABBs.AABBs.StartAddress =
                        inputGeomDesc.content.proceduralAABBs.data;
