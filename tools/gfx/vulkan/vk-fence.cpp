...
Result FenceImpl::init(const IFence::Desc& desc)
{
    ...
    SLANG_VK_RETURN_ON_FAIL(
        m_device->m_api
            .vkCreateSemaphore(m_device->m_api.m_device, &createInfo, nullptr, &m_semaphore)
    );
...
{
    SLANG_VK_RETURN_ON_FAIL(
        m_device->m_api.vkGetSemaphoreCounterValue(m_device->m_api.m_device, m_semaphore, outValue)
    );
    return SLANG_OK;
}
