...
struct SimpleBindingOffset
{
    ...
    SimpleBindingOffset(slang::VariableLayoutReflection* varLayout)
    {
        ...
        {
            bindingSet = (uint32_t
            )varLayout->getBindingSpace(SLANG_PARAMETER_CATEGORY_DESCRIPTOR_TABLE_SLOT);
            binding =
...
VkImageUsageFlags
_calcImageUsageFlags(ResourceStateSet states, MemoryType memoryType, const void* initData);
