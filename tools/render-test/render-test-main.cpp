...
struct AssignValsFromLayoutContext
{
    ...
    SlangResult assignTexture(ShaderCursor const& dstCursor, ShaderInputLayout::TextureVal* srcVal)
    {
        ...
        SLANG_RETURN_ON_FAIL(
            ShaderRendererUtil::generateTexture(srcVal->textureDesc, defaultState, device, texture)
        );
...
{
    auto slangReflection =
        (slang::ProgramLayout*)spGetReflection(m_compilationOutput.output.getRequestForReflection()
        );
...
static SlangResult
_setSessionPrelude(const Options& options, const char* exePath, SlangSession* session)
{
