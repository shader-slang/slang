    ...
    ShaderCompilerUtil::Output& out)
{
    ...
    // We skip adding the module reference options, and instead record the referenced module paths
    // in a list so that we can load them later.
    List<slang::CompilerOptionEntry> sessionOptionEntries;
    List<const char*> referencedModulePaths;
    for (int optionIndex = 0; optionIndex < sessionDesc.compilerOptionEntryCount; optionIndex++)
    {
        ...
        if (option.name == slang::CompilerOptionName::ReferenceModule)
        {
            ...
            referencedModulePaths.add(option.value.stringValue0);
        }
        else
        {
    ...
    // Argument parsing should not have added targets when we don't have a repro.
    ...
    for (const char* path : referencedModulePaths)
    {
        char const* name = nullptr;
        ComPtr<slang::IBlob> irBlob;
        SlangResult result = osFileSystem->loadFile(path, irBlob.writeRef());
        if (SLANG_FAILED(result))
        {
            fprintf(stderr, "error: Failed to read referenced module file: %s\n", path);
            return SLANG_FAIL;
        }
        ComPtr<slang::IModule> module = ComPtr<slang::IModule>(
            slangSession->loadModuleFromIRBlob(name, path, irBlob, diagnostics.writeRef()));
        if (!module)
        {
            fprintf(
                stderr,
