...
Set `SLANG_TEST_DIAGNOSTICS` to enable opt-in diagnostic logging from `slang-test`,
`test-server`, and related process/pipe helpers. Categories are comma-separated:

- `timing`: Per-test execution timing and summary output
- `timing-phases`: Detailed phase timings; also enables `timing`
- `rpc`: JSON-RPC wait/write timing and slow-operation warnings
- `fd`: File descriptor open/close tracking
- `pipe`: Pipe read/write diagnostics and buffer occupancy when available
...
SLANG_TEST_DIAGNOSTICS=timing-phases ./build/Debug/bin/slang-test -use-test-server -server-count 4 slang-unit-test-tool/JSONNative
```

Diagnostics are written to stderr. In test-server mode, `slang-test` drains server
stderr and forwards it to the parent process output, so server phase timing lines
remain visible without changing the test-server JSON-RPC protocol. The final
timing summary is runner-side wall-clock timing; detailed server phase timings are
