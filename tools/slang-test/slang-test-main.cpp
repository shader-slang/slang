...
static SlangResult
_parseCategories(TestCategorySet* categorySet, char const** ioCursor, TestOptions& out)
{
    char const* cursor = *ioCursor;
...
static SlangResult
_gatherTestOptions(TestCategorySet* categorySet, char const** ioCursor, TestOptions& outOptions)
{
...
static SlangResult
_gatherTestsForFile(TestCategorySet* categorySet, String filePath, FileTestList* outTestList)
{
...
)
{
    ...
    if (options.shouldBeVerbose)
    {
        ...
        context->getTestReporter()
            ->messageFormat(TestMessageType::Info, "%s\n", commandLine.begin());
    }
...
static SlangResult
_extractRenderTestRequirements(const CommandLine& cmdLine, TestRequirements* ioRequirements)
{
...
static SlangResult
_extractSlangCTestRequirements(const CommandLine& cmdLine, TestRequirements* ioRequirements)
{
...
static SlangResult
_extractReflectionTestRequirements(const CommandLine& cmdLine, TestRequirements* ioRequirements)
{
...
TestResult runLanguageServerTest(TestContext* context, TestInput& input)
{
    ...
    {
        if (SLANG_FAILED(
                context->createLanguageServerJSONRPCConnection(context->m_languageServerConnection)
            ))
        {
            return TestResult::Fail;
        }
...
static TestResult
generateExpectedOutput(TestContext* const context, const TestInput& input, String& expectedOutput)
{
    auto filePath = input.filePath;
    auto outputStem = input.outputStem;
...
// requirements, writes the output into the `expectedOutput` parameter
TestResult
generateActualOutput(TestContext* const context, const TestInput& input, String& actualOutput)
{
    auto filePath = input.filePath;
