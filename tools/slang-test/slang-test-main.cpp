...
static SlangResult _splitPathLocation(const UnownedStringSlice& pathLocation, DownstreamDiagnostic& outDiagnostic)
{
...
static SlangResult _parseFXCLine(const UnownedStringSlice& line,  List<UnownedStringSlice>& lineSlices, DownstreamDiagnostic& outDiagnostic)
{
    SLANG_RETURN_ON_FAIL(_splitPathLocation(lineSlices[1], outDiagnostic));
...
static SlangResult _parseDXCLine(const UnownedStringSlice& line,  List<UnownedStringSlice>& lineSlices, DownstreamDiagnostic& outDiagnostic)
{
...
static SlangResult _parseGlslangLine(const UnownedStringSlice& line, List<UnownedStringSlice>& lineSlices, DownstreamDiagnostic& outDiagnostic)
{
...
static SlangResult _parseGenericLine(const UnownedStringSlice& line, List<UnownedStringSlice>& lineSlices, DownstreamDiagnostic& outDiagnostic)
{
...
static SlangResult _getDiagnostics(const UnownedStringSlice& inText, List<DownstreamDiagnostic>& outDiagnostics)
{
    // TODO(JS):
    ...
    while (StringUtil::extractLine(text, line))
    {
        ...
        switch (downstreamCompiler)
        {
            ...
            {
                parseRes = _parseFXCLine(line, splitLine, diagnostic);
                break;
            }
            case SLANG_PASS_THROUGH_DXC:
            {
                parseRes = _parseDXCLine(line, splitLine, diagnostic);
                break;
            }
            case SLANG_PASS_THROUGH_GLSLANG:
            {
                parseRes = _parseGlslangLine(line, splitLine, diagnostic);
                break;
            }
            default:
            {
                parseRes = _parseGenericLine(line, splitLine, diagnostic);
                break;
            }
        }
...
TestResult runSimpleLineTest(TestContext* context, TestInput& input)
{
    ...
    if (SLANG_FAILED(_getDiagnostics(exeRes.standardError.getUnownedSlice(), diagnostics)) || diagnostics.getCount() <= 0)
    {
        return TestResult::Fail;
    }

    // We only care that the first line with an error matches

    String actualOutput = getOutput(exeRes);

    // We only want to extract the line number info, to compare
