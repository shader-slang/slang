...
#ifdef _WIN32
/* static */ SlangResult
File::generateTemporary(const UnownedStringSlice& inPrefix, Slang::String& outFileName)
{
...
/* static */ SlangResult
Path::simplify(const UnownedStringSlice& path, SimplifyStyle style, StringBuilder& outPath)
{
    if (path.getLength() == 0)
    {
        return SLANG_FAIL;
...
#else
    auto unlink_cb = [](const char* fpath, const struct stat* sb, int typeflag, struct FTW* ftwbuf
                     ) -> int
    {
        SLANG_UNUSED(sb)
...
#if defined(_WIN32)
/* static */ SlangResult
Path::find(const String& directoryPath, const char* pattern, Visitor* visitor)
{
