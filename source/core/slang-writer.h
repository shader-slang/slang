...
public:
    ...
    static SlangResult
    createBinary(const char* filePath, WriterFlags flags, ComPtr<ISlangWriter>& outWriter)
    {
        return create(filePath, "wb", flags, outWriter);
    }
    static SlangResult
    createText(const char* filePath, WriterFlags flags, ComPtr<ISlangWriter>& outWriter)
    {
