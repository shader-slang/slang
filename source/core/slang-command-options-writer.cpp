...
)
{
    if (linkFlags)
    {
        ...
        if (trimmedWord.getLength())
        {
            ...
            {
                index = m_commandOptions
                            ->findTargetIndexByName(LookupKind::Option, trimmedWord, &nameKey);
            }
...
UnownedStringSlice
MarkdownCommandOptionsWriter::_getLinkName(CommandOptions::LookupKind kind, Index index)
{
    auto& options = *m_commandOptions;
