    ...
    ISlangBlob** outBlob)
{
    ...
    // it to `ScopedAllocation` (which always reclaims via `StandardAllocator`). This keeps
    // allocation and deallocation paired if miniz is ever built with a custom allocator (e.g.
    // redefined `MZ_MALLOC`/`MZ_FREE`).
