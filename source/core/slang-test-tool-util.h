...
struct TestToolUtil
{
    ...
    static SlangResult
    setSessionDefaultPreludeFromRootPath(const String& rootPath, slang::IGlobalSession* session);
    ...
    static SlangResult
    getIncludePath(const String& parentPath, const char* relPath, String& outIncludePath);


    /// Sets the default preludes on the session based on the executable path
    static SlangResult
    setSessionDefaultPreludeFromExePath(const char* exePath, slang::IGlobalSession* session);

    /// Returns true if the StdLib should not be initialized immediately (eg when doing a
    /// -load-stdlib).
