...
/// Avoid host-specific x86_64 codegen on the JIT TargetMachine before LLJIT
/// construction, **only when the SLANG_DISABLE_AVX512 environment variable
/// is set to "1"**. Default is a no-op so production builds keep native
/// codegen on capable hosts. CI workflows that hit #11062 set the env var on
/// the test step.
///
/// On x86_64 with the var set, builds an explicit JITTargetMachineBuilder
/// via detectHost(), then overrides the CPU and feature string to baseline
/// x86-64 before handing it to the LLJITBuilder. On non-x86_64 hosts (or
...
/// Construct an LLJIT with host-specific features conditionally disabled in its
/// JIT TargetMachine. Equivalent to:
///
///   LLJITBuilder b;
///   disableHostSpecificFeaturesForJIT(b);
///   return b.create();
///
/// `disableHostSpecificFeaturesForJIT` only fires when SLANG_DISABLE_AVX512=1
/// is set in the environment; otherwise this is a plain `LLJITBuilder().create()`.
