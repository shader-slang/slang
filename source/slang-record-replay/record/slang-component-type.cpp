...
SLANG_NO_THROW slang::ProgramLayout*
IComponentTypeRecorder::getLayout(SlangInt targetIndex, slang::IBlob** outDiagnostics)
{
    slangRecordLog(LogLevel::Verbose, "%s\n", __PRETTY_FUNCTION__);
...
)
{
    ...
    SlangResult res =
        m_actualComponentType
            ->getEntryPointCode(entryPointIndex, targetIndex, outCode, outDiagnostics);
...
IComponentTypeRecorder::renameEntryPoint(const char* newName, IComponentType** outEntryPoint)
{
    ...
    ApiCallId callId =
        static_cast<ApiCallId>(makeApiCallId(getClassId(), IComponentTypeMethodId::renameEntryPoint)
        );
    ParameterRecorder* recorder{};
    {
        recorder = m_recordManager->beginMethodRecord(callId, m_componentHandle);
...
)
{
    ...
    ApiCallId callId =
        static_cast<ApiCallId>(makeApiCallId(getClassId(), IComponentTypeMethodId::linkWithOptions)
        );
    ParameterRecorder* recorder{};
    {
        recorder = m_recordManager->beginMethodRecord(callId, m_componentHandle);
