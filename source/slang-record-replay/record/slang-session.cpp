...
SLANG_NO_THROW slang::IModule*
SessionRecorder::loadModule(const char* moduleName, slang::IBlob** outDiagnostics)
{
    slangRecordLog(LogLevel::Verbose, "%s\n", __PRETTY_FUNCTION__);
...
)
{
    ...
    slang::TypeReflection* pTypeReflection =
        m_actualSession
            ->specializeType(type, specializationArgs, specializationArgCount, outDiagnostics);
...
SLANG_NO_THROW bool
SessionRecorder::isBinaryModuleUpToDate(const char* modulePath, slang::IBlob* binaryModuleBlob)
{
...
)
{
    for (SlangInt i = 0; i < componentTypeCount; i++)
    {
        ...
        else if (componentType->queryInterface(IEntryPointRecorder::getTypeGuid(), &outObj) ==
                 SLANG_OK)
        {
        ...
        else if (componentType->queryInterface(
                     CompositeComponentTypeRecorder::getTypeGuid(),
                     &outObj
                 ) == SLANG_OK)
        {
            CompositeComponentTypeRecorder* compositeComponentTypeRecord =
        ...
        else if (componentType->queryInterface(ITypeConformanceRecorder::getTypeGuid(), &outObj) ==
                 SLANG_OK)
        {
