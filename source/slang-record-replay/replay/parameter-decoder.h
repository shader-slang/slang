...
public:
    ...
    static size_t
    decodeString(const uint8_t* buffer, int64_t bufferSize, PointerDecoder<char*>& typeDecoder);

    static size_t
    decodePointer(const uint8_t* buffer, int64_t bufferSize, PointerDecoder<void*>& pointerDecoder);

    static size_t
    decodeAddress(const uint8_t* buffer, int64_t bufferSize, SlangRecord::AddressFormat& address)
    {
    ...
    static size_t
    decodeStringArray(const uint8_t* buffer, int64_t bufferSize, char** outputArray, size_t count)
    {
        if (count == 0 && bufferSize == 0)
        {
            return 0;
    ...
    template<typename T>
    static size_t
    decodeStructArray(const uint8_t* buffer, int64_t bufferSize, T* outputArray, size_t count)
    {
        if (count == 0 && bufferSize == 0)
        {
            return 0;
