...
void ReplayConsumer::IGlobalSession_compileStdLib(
    ObjectID objectId,
...
)
{
    ...
    if (SLANG_FAILED(res))
    {
        slangRecordLog(
            LogLevel::Error,
            "IGlobalSession::compileStdLib fails, ret: 0x%X, this: 0x%X\n",
            res,
            objectId
        );
    }
...
void ReplayConsumer::IGlobalSession_loadStdLib(
    ObjectID objectId,
    const void* stdLib,
    size_t stdLibSizeInBytes
)
{
    InputObjectSanityCheck(objectId);
    ...
    if (SLANG_FAILED(res))
    {
        slangRecordLog(
            LogLevel::Error,
            "IGlobalSession::loadStdLib fails, ret: 0x%X, this: 0x%X\n",
            res,
            objectId
        );
    }
...
void ReplayConsumer::IGlobalSession_saveStdLib(
    ObjectID objectId,
...
)
{
    ...
    else
    {
        slangRecordLog(
            LogLevel::Error,
            "IGlobalSession::saveStdLib fails, ret: 0x%X, this: 0x%X\n",
            res,
            objectId
        );
    }
...
)
{
    SlangResult res =
        m_commonReplayer
            .getResultAsFileSystem(objectId, entryPointIndex, targetIndex, outFileSystemId);
