...
-- For notes within diagnostics, use the `note` function above
                ...
                if note.spans then
                    if type(note.spans) ~= "table" then
                        ...
                        for j, span in ipairs(note.spans) do
                            local span_errors =
                                validate_span(span, diagnostic_name .. ".notes[" .. i .. "].spans[" .. j .. "]")
                            for _, err in ipairs(span_errors) do
                                table.insert(errors, err)
                            end
                        end
                ...
                for _, span in ipairs(diag.secondary_spans) do
                    ...
                    if span.variadic and span.struct_name then
                        if not variadic_structs[span.struct_name] then
                            variadic_structs[span.struct_name] =
                                { params = {}, locations = {}, container_type = "span", container = span }
                ...
                for _, note in ipairs(diag.notes) do
                    ...
                    if note.variadic and note.struct_name then
                        if not variadic_structs[note.struct_name] then
                            variadic_structs[note.struct_name] =
                                { params = {}, locations = {}, container_type = "note", container = note }
                    ...
                    if part.type == "interpolation" then
                        ...
                        if not part.member_name then
                            if not param_contexts[param_name] then
                                param_contexts[param_name] =
                                    { variadic_structs = {}, non_variadic = false, is_location = false }
            ...
            if diag.notes then
                for j, note in ipairs(diag.notes) do
                    ...
                    if note.primary_span_message then
                        ...
                        local primary_span_parts = process_message_container(
                            primary_span_container,
                            diagnostic_name .. ".notes[" .. j .. "].primary_span"
                        )
