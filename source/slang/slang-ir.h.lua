...
[[ IR%s* %s(%s)
    {
        IRInst* operands[] = {%s};
        return (IR%s*)getType(kIROp_%s, %d, operands);
    }]],
                struct_name, methodName, paramList, operandList,
                struct_name, struct_name, #operands)
...
local function generateAllIRBuilderMethods()
        ...
        local function traverse(tbl)
                for _, i in ipairs(tbl) do
                        ...
                        if value.is_leaf and value.operands and #value.operands > 0 and not value.skipIRBuilderMethodGeneration then
