...
struct StoreContext
{
    ...
    const Offset32Array<ReproUtil::StringPair>
    calcDefines(const Dictionary<String, String>& srcDefines)
    {
...
)
{
...

    {
        ...
        dst->debugInfoLevel =
            linkage->m_optionSet.getEnumOption<DebugInfoLevel>(CompilerOptionName::DebugInformation
            );
    ...
    // Add the search paths
    {
        ...
        Offset32Array<Offset32Ptr<OffsetString>> dstPaths =
            inOutContainer.newArray<Offset32Ptr<OffsetString>>(srcPaths.searchDirectories.getCount()
            );
...
struct LoadContext
{
    ...
    void
    loadDefines(const Offset32Array<ReproUtil::StringPair>& in, Dictionary<String, String>& out)
    {
        out.clear();
...
/* static */ SlangResult
ReproUtil::saveState(EndToEndCompileRequest* request, const String& filename)
{
...
/* static */ SlangResult
ReproUtil::loadState(const String& filename, DiagnosticSink* sink, List<uint8_t>& outBuffer)
{
    RefPtr<FileStream> stream = new FileStream;
    SLANG_RETURN_ON_FAIL(
...
/* static */ SlangResult
ReproUtil::loadState(Stream* stream, DiagnosticSink* sink, List<uint8_t>& buffer)
{
    Header header;
...
/* static */ SlangResult
ReproUtil::calcDirectoryPathFromFilename(const String& filename, String& outPath)
{
    String absPath;
...
/* static */ SlangResult
ReproUtil::extractFilesToDirectory(const String& filename, DiagnosticSink* sink)
{
    List<uint8_t> buffer;
...
static SlangResult
_calcCommandLine(OffsetBase& base, ReproUtil::RequestState* requestState, CommandLine& cmd)
{
...
)
{
    ...
    for (int triesCount = 0; triesCount < maxTries; ++triesCount)
    {
        ...
        if (SLANG_SUCCEEDED(
                stream->init(builder, FileMode::CreateNew, FileAccess::Write, FileShare::WriteOnly)
            ))
        {
            outStream = stream;
            return SLANG_OK;
