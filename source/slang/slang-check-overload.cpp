...
namespace Slang
{
SemanticsVisitor::ParamCounts
SemanticsVisitor::CountParameters(FilteredMemberRefList<ParamDecl> params)
{
...
)
{
    ...
    switch (candidate.flavor)
    {
        case OverloadCandidate::Flavor::Func:
            paramCounts = CountParameters(
                getParameters(m_astBuilder, candidate.item.declRef.as<CallableDecl>())
            );
            break;
        ...
        switch (candidate.flavor)
        {
            ...
            default:
                SLANG_DIAGNOSE_UNEXPECTED(
                    getSink(),
                    context.loc,
                    "unknown overload candidate flavor"
                );
                break;
        }
    }
...
)
{
    ...
    if (auto leftSubscriptDecl = left.declRef.as<SubscriptDecl>())
    {
        if (auto rightSubscriptDecl = right.declRef.as<SubscriptDecl>())
        {
            ...
            else if (rightAccessorCount > leftAccessorCount && decl1IsSubsetOfDecl2(
                                                                   leftSubscriptDecl.getDecl(),
                                                                   rightSubscriptDecl.getDecl()
                                                               ))
            {
                return 1;
            }
...
Expr* SemanticsVisitor::checkGenericAppWithCheckedArgs(GenericAppExpr* genericAppExpr)
{
    ...
    if (context.bestCandidates.getCount() > 0)
    {
        ...
        if (context.bestCandidates[0].status != OverloadCandidate::Status::Applicable)
        {
            ...
            getSink()
                ->diagnose(genericAppExpr, Diagnostics::unimplemented, "no applicable generic");
