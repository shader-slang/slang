...
)
{
    ...
    if (!diff)
        return this;
    (*ioDiff)++;
    return getCurrentASTBuilder()
        ->getSubtypeWitnessPack(newSub, newSup, newWitnesses.getArrayView().arrayView);
}
...
)
{
    ...
    if (auto subTypePack = as<ConcreteTypePack>(newSub))
    {
        ...
        for (Index i = 0; i < subTypePack->getTypeCount(); i++)
        {
            ...
            auto newWitness = getCurrentASTBuilder()
                                  ->getExpandSubtypeWitness(elementType, newSup, elementWitness);
