...
RefPtr<PropagationInfo> PropagationInfo::makeConcrete(PropagationJudgment j, IRInst* value)
{
    auto info = new PropagationInfo(j);
    info->concreteValue = value;
    return info;
}

RefPtr<PropagationInfo> PropagationInfo::makeSet(
    PropagationJudgment j,
...
RefPtr<PropagationInfo> PropagationInfo::makeSetOfFuncs(
    const HashSet<IRInst*>& values,
    IRFuncType* dynFuncType)
{
    auto info = new PropagationInfo(PropagationJudgment::SetOfFuncs);
    info->possibleValues = values;
    info->dynFuncType = dynFuncType;
    return info;
}

RefPtr<PropagationInfo> PropagationInfo::makeExistential(const HashSet<IRInst*>& tables)
{
    auto info = new PropagationInfo(PropagationJudgment::Existential);
    info->possibleValues = tables;
    return info;
}

RefPtr<PropagationInfo> PropagationInfo::makeUnknown()
{
    return new PropagationInfo(PropagationJudgment::UnknownSet);
}

bool areInfosEqual(const RefPtr<PropagationInfo>& a, const RefPtr<PropagationInfo>& b)
{
    ...
    case PropagationJudgment::UnknownSet:
...
RefPtr<PropagationInfo> DynamicInstLoweringContext::tryGetInfo(IRInst* inst)
{
    // If this is a global instruction (parent is module), return concrete info
    if (as<IRModuleInst>(inst->getParent()))
    {
        // Create static info based on instruction type
        static RefPtr<PropagationInfo> typeInfo =
            PropagationInfo::makeConcrete(PropagationJudgment::ConcreteType, nullptr);
        static RefPtr<PropagationInfo> tableInfo =
            PropagationInfo::makeConcrete(PropagationJudgment::ConcreteTable, nullptr);
        static RefPtr<PropagationInfo> funcInfo =
            PropagationInfo::makeConcrete(PropagationJudgment::ConcreteFunc, nullptr);
        static RefPtr<PropagationInfo> valueInfo = PropagationInfo::makeValue();
...
void DynamicInstLoweringContext::performInformationPropagation()
{
...
IRInst* DynamicInstLoweringContext::maybeReinterpretArg(IRInst* arg, IRInst* param)
{
    /*auto argTypeInfo = tryGetInfo(arg->getDataType());
    auto paramTypeInfo = tryGetInfo(param->getDataType());

    if (argTypeInfo & paramTypeInfo)
        return arg;

    IRBuilder builder(module);
    builder.setInsertAfter(arg);

    if (argTypeInfo->judgment == PropagationJudgment::SetOfTypes &&
        paramTypeInfo->judgment == PropagationJudgment::SetOfTypes)
    {
        if (argTypeInfo->possibleValues.getCount() != paramTypeInfo->possibleValues.getCount())
        {
            SLANG_ASSERT("Unhandled");

            // If the sets are not equal, reinterpret to the parameter type
            // auto reinterpret = builder.emitReinterpret(param->getDataType(), arg);
            // propagationMap[reinterpret] = paramTypeInfo;
        }
    }*/
    ...
    if (!argInfo || !paramInfo)
        return arg;

    if (argInfo->judgment == PropagationJudgment::Existential &&
        paramInfo->judgment == PropagationJudgment::Existential)
    {
        if (argInfo->possibleValues.getCount() != paramInfo->possibleValues.getCount())
        {
            // If the sets of witness tables are not equal, reinterpret to the parameter type
            IRBuilder builder(module);
            builder.setInsertAfter(arg);
            auto reinterpret = builder.emitReinterpret(param->getDataType(), arg);
            propagationMap[reinterpret] = paramInfo;
...
void DynamicInstLoweringContext::insertReinterpretsForPhiParameters()
{
    // Process each function in the module
    for (auto inst : module->getGlobalInsts())
    {
                    ...
                    for (auto pred : block->getPredecessors())
                    {
                        ...
                        if (auto unconditionalBranch = as<IRUnconditionalBranch>(terminator))
                        {
                            ...
                            if (paramIndex < unconditionalBranch->getArgCount())
                            {
                                ...
                                auto newArg = maybeReinterpretArg(arg, param);
...
void DynamicInstLoweringContext::processInstForPropagation(IRInst* inst)
{
    RefPtr<PropagationInfo> info;
    ...
    case kIROp_ExtractExistentialWitnessTable:
        info = analyzeExtractExistentialWitnessTable(as<IRExtractExistentialWitnessTable>(inst));
        break;
    ...
    case kIROp_ExtractExistentialValue:
        ...
        break;
    case kIROp_Call:
        info = analyzeCall(as<IRCall>(inst));
        break;
    default:
...
RefPtr<PropagationInfo> DynamicInstLoweringContext::analyzeCreateExistentialObject(
    IRCreateExistentialObject* inst)
{
...
RefPtr<PropagationInfo> DynamicInstLoweringContext::analyzeMakeExistential(IRMakeExistential* inst)
{
    ...
    if (!witnessTableInfo || witnessTableInfo->judgment == PropagationJudgment::UnknownSet)
    {
        return PropagationInfo::makeUnknown();
    }
...
RefPtr<PropagationInfo> DynamicInstLoweringContext::analyzeLookupWitnessMethod(
    IRLookupWitnessMethod* inst)
{
    auto witnessTable = inst->getWitnessTable();
...
RefPtr<PropagationInfo> DynamicInstLoweringContext::analyzeExtractExistentialWitnessTable(
    IRExtractExistentialWitnessTable* inst)
{
    auto operand = inst->getOperand(0);
    auto operandInfo = tryGetInfo(operand);

    if (!operandInfo || operandInfo->judgment == PropagationJudgment::UnknownSet)
    {
        return PropagationInfo::makeUnknown();
    }
...
RefPtr<PropagationInfo> DynamicInstLoweringContext::analyzeExtractExistentialType(
    IRExtractExistentialType* inst)
{
    auto operand = inst->getOperand(0);
    auto operandInfo = tryGetInfo(operand);
    ...
    if (operandInfo->judgment == PropagationJudgment::Existential)
    {
        ...
        if (types.getCount() == 1)
        {
            return PropagationInfo::makeConcrete(PropagationJudgment::ConcreteType, *types.begin());
        }
        else
        {
...
RefPtr<PropagationInfo> DynamicInstLoweringContext::analyzeExtractExistentialValue(
    IRExtractExistentialValue* inst)
{
    // The value itself is just a regular value
    return PropagationInfo::makeValue();
}

RefPtr<PropagationInfo> DynamicInstLoweringContext::analyzeCall(IRCall* inst)
{
    auto callee = inst->getCallee();
    auto calleeInfo = tryGetInfo(callee);

    List<IRInst*> outputVals;
    outputVals.add(inst); // The call inst itself is an output.

    // Also add all OutTypeBase parameters
    ...
    if (funcType)
    {
        ...
        for (auto paramType : funcType->getParamTypes())
        {
            ...
            {
                // If this is an OutTypeBase, we consider it an output
                outputVals.add(inst->getArg(paramIndex));
            }
            paramIndex++;
        }
    }
    ...
    {
        if (as<IRVar>(outputVal))
        {
            // TODO: We need to set up infrastructure to track variable
            // assignments.
            // For now, we will just return a value judgment for variables.
            // (doesn't make much sense.. but its fine for now)
            //
            propagationMap[outputVal] = PropagationInfo::makeValue();
        }
        else
        {
            if (auto interfaceType = as<IRInterfaceType>(outputVal->getDataType()))
            {
                if (!interfaceType->findDecoration<IRComInterfaceDecoration>())
                {
                    // If this is an interface type, we need to propagate existential info
                    // based on the interface type.
                    propagationMap[outputVal] =
                        PropagationInfo::makeExistential(collectExistentialTables(interfaceType));
                }
                else
                {
                    // If this is a COM interface, we treat it as unknown
                    propagationMap[outputVal] = PropagationInfo::makeUnknown();
                }
            }
            else
            {
    ...
    return tryGetInfo(inst);
}


void DynamicInstLoweringContext::processFunction(IRFunc* func)
{
    // Initialize parameter info for the first block
    initializeFirstBlockParameters(func);

    // Initialize worklist with edges from successor blocks
    LinkedList<IREdge> edgeQueue;

    auto processBlock = [&](IRBlock* block)
    {
        ...
        for (auto inst : block->getChildren())
        {
            ...
            processInstForPropagation(inst);
            auto newInfo = tryGetInfo(inst);

            // Check if information changed
            if (!anyInfoChanged)
            {
                if (!oldInfo || (newInfo && !areInfosEqual(oldInfo, newInfo)))
                {
                    anyInfoChanged = true;
                }
            }
        ...
        // If any info changed, add successor edges back to queue
        if (anyInfoChanged)
        {
            auto successors = block->getSuccessors();
            for (auto succIter = successors.begin(); succIter != successors.end(); ++succIter)
            {
                edgeQueue.addLast(succIter.getEdge());
            }
        }
    };

    // Start processing from the first block
    processBlock(func->getFirstBlock());

    // Process until fixed point
    while (edgeQueue.getCount() > 0)
    {
        // Pop edge from front
        auto edge = edgeQueue.getFirst();
        edgeQueue.getFirstNode()->removeAndDelete();

        // Propagate along the edge
        propagateEdge(edge);

        // Process the successor block's instructions.
        // This will also add any new edges to the queue if info changed
        //
        processBlock(edge.getSuccessor());
    }
} // namespace Slang

void DynamicInstLoweringContext::propagateEdge(IREdge edge)
{
        ...
        for (auto param : successorBlock->getParams())
        {
            if (paramIndex < unconditionalBranch->getArgCount())
            {
                ...
                if (argInfo)
                {
                    ...
                    if (existingInfo)
                    {
                        List<RefPtr<PropagationInfo>> infos;
                        infos.add(existingInfo);
                        infos.add(argInfo);
                        propagationMap[param] = unionPropagationInfo(infos);
                    }
                    else
                    {
...
void DynamicInstLoweringContext::initializeFirstBlockParameters(IRFunc* func)
{
    ...
    for (auto param : firstBlock->getParams())
    {
        ...
        if (auto interfaceType = as<IRInterfaceType>(paramType))
        {
            if (!interfaceType->findDecoration<IRComInterfaceDecoration>())
                ...
                propagationMap[param] = PropagationInfo::makeUnknown();
        }
        else if (as<IRWitnessTableType>(paramType))
        {
            propagationMap[param] = PropagationInfo::makeUnknown();
        }
        else if (as<IRFuncType>(paramType))
        {
            propagationMap[param] = PropagationInfo::makeUnknown();
        }
        else if (as<IRTypeKind>(paramType) || as<IRTypeType>(paramType))
        {
            propagationMap[param] = PropagationInfo::makeUnknown();
        }
        else
...
RefPtr<PropagationInfo> DynamicInstLoweringContext::unionPropagationInfo(
    const List<RefPtr<PropagationInfo>>& infos)
{
    if (infos.getCount() == 0)
    {
        return PropagationInfo::makeValue();
    ...
    PropagationJudgment unionJudgment = PropagationJudgment::Value;
    ...
    for (auto info : infos)
    {
        ...
        case PropagationJudgment::SetOfFuncs:
            ...
            break;
        case PropagationJudgment::Value:
            // Value judgments don't contribute to the union
            break;
        case PropagationJudgment::Existential:
            ...
            return PropagationInfo::makeValue();
        case PropagationJudgment::UnknownSet:
            // If any info is unknown, the union is unknown
            return PropagationInfo::makeUnknown();
        }
    }
...
RefPtr<PropagationInfo> DynamicInstLoweringContext::analyzeDefault(IRInst* inst)
{
...
void DynamicInstLoweringContext::performDynamicInstLowering()
{
...
void DynamicInstLoweringContext::replaceType(IRInst* inst)
{
    ...
    auto tupleType = builder.getTupleType(List<IRType*>({builder.getUIntType(), anyValueType}));
    inst->setFullType(tupleType);
}

void DynamicInstLoweringContext::lowerInst(IRInst* inst)
{
    switch (inst->getOp())
    {
    case kIROp_LookupWitnessMethod:
...
void DynamicInstLoweringContext::lowerLookupWitnessMethod(IRLookupWitnessMethod* inst)
{
    auto info = tryGetInfo(inst);
    if (!info)
        return;
...
void DynamicInstLoweringContext::lowerExtractExistentialWitnessTable(
    IRExtractExistentialWitnessTable* inst)
{
    auto info = tryGetInfo(inst);
    if (!info)
...
void DynamicInstLoweringContext::lowerExtractExistentialValue(IRExtractExistentialValue* inst)
{
    IRBuilder builder(inst);
    builder.setInsertBefore(inst);
...
void DynamicInstLoweringContext::lowerExtractExistentialType(IRExtractExistentialType* inst)
{
    auto info = tryGetInfo(inst);
...
void DynamicInstLoweringContext::lowerCall(IRCall* inst)
{
    auto callee = inst->getCallee();
    auto calleeInfo = tryGetInfo(callee);
    ...
    auto dispatchFunc = createDispatchFunc(calleeInfo->possibleValues, calleeInfo->dynFuncType);
...
void DynamicInstLoweringContext::lowerMakeExistential(IRMakeExistential* inst)
{
    auto info = tryGetInfo(inst);
    if (!info || info->judgment != PropagationJudgment::Existential)
        return;
    ...
    auto tupleType =
        builder.getTupleType(List<IRType*>({builder.getUIntType(), packedValue->getDataType()}));
...
void DynamicInstLoweringContext::lowerCreateExistentialObject(IRCreateExistentialObject* inst)
{
...
UInt DynamicInstLoweringContext::getUniqueID(IRInst* funcOrTable)
{
...
IRFunc* DynamicInstLoweringContext::createKeyMappingFunc(
    IRInst* key,
...
IRFunc* DynamicInstLoweringContext::createDispatchFunc(
    const HashSet<IRInst*>& funcs,
    IRFuncType* expectedFuncType)
{
    ...
    for (auto funcInst : funcs)
    {
        ...
        {
            callArgs.add(
                maybeUnpackValue(&builder, originalParams[ii], concreteFuncType->getParamType(ii)));
        }

        // Call the specific function
        auto callResult = builder.emitCallInst(resultType, funcInst, callArgs);
...
IRAnyValueType* DynamicInstLoweringContext::createAnyValueType(const HashSet<IRType*>& types)
{
    IRBuilder builder(module);
...
IRAnyValueType* DynamicInstLoweringContext::createAnyValueTypeFromInsts(
    const HashSet<IRInst*>& typeInsts)
{
    HashSet<IRType*> types;
...
SlangInt DynamicInstLoweringContext::calculateAnyValueSize(const HashSet<IRType*>& types)
{
    SlangInt maxSize = 0;
    for (auto type : types)
    {
...
bool DynamicInstLoweringContext::needsReinterpret(
    RefPtr<PropagationInfo> sourceInfo,
    RefPtr<PropagationInfo> targetInfo)
{
...
bool DynamicInstLoweringContext::isExistentialType(IRType* type)
{
    return as<IRInterfaceType>(type) != nullptr;
}

bool DynamicInstLoweringContext::isInterfaceType(IRType* type)
{
    return as<IRInterfaceType>(type) != nullptr;
}

HashSet<IRInst*> DynamicInstLoweringContext::collectExistentialTables(
...
void DynamicInstLoweringContext::processModule()
{
    // Phase 1: Information Propagation
    performInformationPropagation();

    // Phase 1.5: Insert reinterprets for phi parameters where needed
    insertReinterpretsForPhiParameters();
