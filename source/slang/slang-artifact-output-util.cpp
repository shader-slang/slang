...
)
{
    ...
    if (ArtifactDescUtil::isDisassembly(desc, toDesc))
    {
        ...
        if (SLANG_SUCCEEDED(
                dissassembleWithDownstream(session, artifact, sink, disassemblyArtifact.writeRef())
            ))
        {
...
/* static */ SlangResult
ArtifactOutputUtil::write(const ArtifactDesc& desc, ISlangBlob* blob, ISlangWriter* writer)
{
...
static SlangResult
_requireBlob(IArtifact* artifact, DiagnosticSink* sink, ComPtr<ISlangBlob>& outBlob)
{
...
/* static */ SlangResult
ArtifactOutputUtil::writeToFile(const ArtifactDesc& desc, ISlangBlob* blob, const String& path)
{
...
/* static */ SlangResult
ArtifactOutputUtil::writeToFile(IArtifact* artifact, DiagnosticSink* sink, const String& path)
{
    if (!sink)
    {
