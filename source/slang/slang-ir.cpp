...
)
{
    return createInstImpl<
        T>(builder, op, type, fixedArgCount, fixedArgs, 1, &varArgCount, &varArgs);
}
...
template<typename T>
static T*
createInst(IRBuilder* builder, IROp op, IRType* type, IRInst* arg1, IRInst* arg2, IRInst* arg3)
{
...
IRExpandType*
IRBuilder::getExpandTypeOrVal(IRType* type, IRInst* pattern, ArrayView<IRInst*> capture)
{
...
IRPtrTypeBase*
IRBuilder::getPtrTypeWithAddressSpace(IRType* valueType, IRPtrTypeBase* ptrWithAddrSpace)
{
...
IRVectorType* IRBuilder::getVectorType(IRType* elementType, IRInst* elementCount)
{
    IRInst* operands[] = {elementType, elementCount};
    return (IRVectorType*)
        getType(kIROp_VectorType, sizeof(operands) / sizeof(operands[0]), operands);
}
...
IRMatrixType*
IRBuilder::getMatrixType(IRType* elementType, IRInst* rowCount, IRInst* columnCount, IRInst* layout)
{
    IRInst* operands[] = {elementType, rowCount, columnCount, layout};
    return (IRMatrixType*)
        getType(kIROp_MatrixType, sizeof(operands) / sizeof(operands[0]), operands);
}
...
IRDifferentialPtrPairType*
IRBuilder::getDifferentialPtrPairType(IRType* valueType, IRInst* witnessTable)
{
    IRInst* operands[] = {valueType, witnessTable};
...
IRDifferentialPairUserCodeType*
IRBuilder::getDifferentialPairUserCodeType(IRType* valueType, IRInst* witnessTable)
{
    IRInst* operands[] = {valueType, witnessTable};
...
IRBackwardDiffIntermediateContextType*
IRBuilder::getBackwardDiffIntermediateContextType(IRInst* func)
{
    if (!func)
        func = getVoidValue();
...
{
    IRInst* operands[] = {elementType};
    return (IRGLSLOutputParameterGroupType*)
        getType(kIROp_GLSLOutputParameterGroupType, 1, operands);
}
...
IRType*
IRBuilder::getBindExistentialsType(IRInst* baseType, UInt slotArgCount, IRInst* const* slotArgs)
{
    if (slotArgCount == 0)
...
IRType*
IRBuilder::getBindExistentialsType(IRInst* baseType, UInt slotArgCount, IRUse const* slotArgUses)
{
    if (slotArgCount == 0)
        return (IRType*)baseType;
...
IRType*
IRBuilder::getBoundInterfaceType(IRType* interfaceType, IRType* concreteType, IRInst* witnessTable)
{
...
IRType*
IRBuilder::getAttributedType(IRType* baseType, UInt attributeCount, IRAttr* const* attributes)
{
    List<IRInst*> operands;
...
IRInst*
IRBuilder::emitByteAddressBufferStore(IRInst* byteAddressBuffer, IRInst* offset, IRInst* value)
{
...
IRInst*
IRBuilder::emitDebugVar(IRType* type, IRInst* source, IRInst* line, IRInst* col, IRInst* argIndex)
{
    if (argIndex)
    {
...
IRInst*
IRBuilder::emitDebugValue(IRInst* debugVar, IRInst* debugValue, ArrayView<IRInst*> accessChain)
{
    List<IRInst*> args;
    args.add(debugVar);
    args.add(debugValue);
...
IRInst*
IRBuilder::emitMakeDifferentialPairUserCode(IRType* type, IRInst* primal, IRInst* differential)
{
...
IRInst*
IRBuilder::emitSpecializeInst(IRType* type, IRInst* genericVal, UInt argCount, IRInst* const* args)
{
    ...
    auto inst = createInstWithTrailingArgs<
        IRSpecialize>(this, kIROp_Specialize, type, 1, &genericVal, argCount, args);
...
{
    auto inst = createInstWithTrailingArgs<
        IRSpecialize>(this, kIROp_Expand, type, capturedArgCount, capturedArgs, 0, nullptr);
    addInst(inst);
    return inst;
}
...
)
{
    ...
    auto inst = createInstWithTrailingArgs<
        IRTryCall>(this, kIROp_TryCall, type, 3, fixedArgs, argCount, args);
    addInst(inst);
    return inst;
}
...
IRInst*
IRBuilder::emitTargetTupleGetElement(IRType* elementType, IRInst* targetTupleVal, IRInst* indexVal)
{
...
IRInst*
IRBuilder::addIntermediateContextFieldDifferentialTypeDecoration(IRInst* target, IRInst* witness)
{
...
IRInterfaceRequirementEntry*
IRBuilder::createInterfaceRequirementEntry(IRInst* requirementKey, IRInst* requirementVal)
{
...
IRGLSLShaderStorageBufferType*
IRBuilder::createGLSLShaderStorableBufferType(UInt operandCount, IRInst* const* operands)
{
    IRGLSLShaderStorageBufferType* ssboType = createInst<IRGLSLShaderStorageBufferType>(
        this,
        kIROp_GLSLShaderStorageBufferType,
...
IRStructField*
IRBuilder::createStructField(IRType* aggType, IRStructKey* fieldKey, IRType* fieldType)
{
    IRInst* operands[] = {fieldKey, fieldType};
    IRStructField* field = (IRStructField*)createInstWithTrailingArgs<
        IRInst>(this, kIROp_StructField, nullptr, 0, nullptr, 2, operands);
...
IRInst*
IRBuilder::emitIsType(IRInst* value, IRInst* witness, IRInst* typeOperand, IRInst* targetWitness)
{
...
IRInst*
IRBuilder::emitUpdateElement(IRInst* base, ArrayView<IRInst*> accessChain, IRInst* newElement)
{
    List<IRInst*> args;
    args.add(base);
    args.add(newElement);
...
IRInst*
IRBuilder::emitSwizzle(IRType* type, IRInst* base, UInt elementCount, IRInst* const* elementIndices)
{
...
IRInst*
IRBuilder::emitSwizzle(IRType* type, IRInst* base, UInt elementCount, UInt const* elementIndices)
{
...
IRIfElse*
IRBuilder::emitIfElse(IRInst* val, IRBlock* trueBlock, IRBlock* falseBlock, IRBlock* afterBlock)
{
...
)
{
    ...
    auto inst = createInstWithTrailingArgs<
        IRSwitch>(this, kIROp_Switch, nullptr, fixedArgCount, fixedArgs, caseArgCount, caseArgs);
    addInst(inst);
    return inst;
}
...
IRDecoration*
IRBuilder::addBindExistentialSlotsDecoration(IRInst* value, UInt argCount, IRInst* const* args)
{
...
IRDecoration*
IRBuilder::addDecoration(IRInst* value, IROp op, IRInst* const* operands, Int operandCount)
{
...
IREntryPointLayout*
IRBuilder::getEntryPointLayout(IRVarLayout* paramsLayout, IRVarLayout* resultLayout)
{
    ...
    for (Index i = 0; i < workList.getCount(); i++)
    {
        ...
        for (;;)
        {
            ...
            if (userIsHoistable)
            {
                ...
                // value.
                IRInst* existingVal = nullptr;
                if (dedupContext->getGlobalValueNumberingMap()
                        .tryGetValue(IRInstKey{user}, existingVal))
                {
...
template<typename T>
IRTargetSpecificDecoration*
findBestTargetDecoration(IRInst* inInst, CapabilitySet const& targetCaps)
{
...
template<typename T>
IRTargetSpecificDecoration*
findBestTargetDecoration(IRInst* val, CapabilityName targetCapabilityAtom)
{
