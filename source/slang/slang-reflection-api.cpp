...
static SlangReflectionUserAttribute*
findUserAttributeByName(Session* session, Decl* decl, const char* name)
{
...
static SlangParameterCategory
maybeRemapParameterCategory(TypeLayout* typeLayout, SlangParameterCategory category)
{
...
SlangReflectionType*
spReflectionUserAttribute_GetArgumentType(SlangReflectionUserAttribute* attrib, unsigned int index)
{
    auto userAttr = convert(attrib);
    if (!userAttr)
        return nullptr;
...
SLANG_API unsigned int spReflectionType_GetFieldCount(SlangReflectionType* inType)
{
    ...
    if (auto declRefType = as<DeclRefType>(type))
    {
        ...
        {
            return (unsigned int
            )getFields(getModule(declRef.getDecl())->getLinkage()->getASTBuilder(),
                       structDeclRef,
...
spReflectionType_GetFieldByIndex(SlangReflectionType* inType, unsigned index)
{
    auto type = convert(inType);
    if (!type)
        return nullptr;
...
spReflectionType_GetUserAttribute(SlangReflectionType* inType, unsigned int index)
{
    auto type = convert(inType);
    if (!type)
        return 0;
...
SLANG_API SlangReflectionUserAttribute*
spReflectionType_FindUserAttributeByName(SlangReflectionType* inType, char const* name)
{
    auto type = convert(inType);
    if (!type)
        return 0;
...
SLANG_API SlangReflectionType*
spReflectionTypeLayout_GetType(SlangReflectionTypeLayout* inTypeLayout)
{
...
spReflectionTypeLayout_GetFieldByIndex(SlangReflectionTypeLayout* inTypeLayout, unsigned index)
{
    auto typeLayout = convert(inTypeLayout);
    if (!typeLayout)
        return nullptr;
...
SLANG_API SlangReflectionVariableLayout*
spReflectionTypeLayout_GetExplicitCounter(SlangReflectionTypeLayout* inTypeLayout)
{
...
spReflectionTypeLayout_GetElementTypeLayout(SlangReflectionTypeLayout* inTypeLayout)
{
    auto typeLayout = convert(inTypeLayout);
    if (!typeLayout)
        return nullptr;
...
SLANG_API SlangReflectionVariableLayout*
spReflectionTypeLayout_GetElementVarLayout(SlangReflectionTypeLayout* inTypeLayout)
{
    auto typeLayout = convert(inTypeLayout);
    if (!typeLayout)
        return nullptr;
...
SLANG_API SlangReflectionVariableLayout*
spReflectionTypeLayout_getContainerVarLayout(SlangReflectionTypeLayout* inTypeLayout)
{
    auto typeLayout = convert(inTypeLayout);
    if (!typeLayout)
        return nullptr;
...
SLANG_API SlangReflectionTypeLayout*
spReflectionTypeLayout_getPendingDataTypeLayout(SlangReflectionTypeLayout* inTypeLayout)
{
    auto typeLayout = convert(inTypeLayout);
    if (!typeLayout)
        return nullptr;
...
SLANG_API SlangReflectionVariableLayout*
spReflectionVariableLayout_getPendingDataLayout(SlangReflectionVariableLayout* inVarLayout)
{
...
SLANG_API SlangReflectionType*
spReflectionType_getSpecializedTypeArgType(SlangReflectionType* inType, SlangInt index)
{
    auto type = convert(inType);
    if (!type)
        return nullptr;
...
struct ExtendedTypeLayoutContext
{
    ...
    RefPtr<VarLayout>
    _createSimpleOffsetVarLayout(TypeLayout* typeLayout, BindingRangePathLink* path)
    {
...
spReflectionVariable_FindModifier(SlangReflectionVariable* inVar, SlangModifierID modifierID)
{
    auto var = convert(inVar).getDecl();
...
SLANG_API SlangReflectionUserAttribute*
spReflectionVariable_GetUserAttribute(SlangReflectionVariable* inVar, unsigned int index)
{
    auto varDecl = convert(inVar).getDecl();
    if (!varDecl)
        return 0;
...
SLANG_API SlangReflectionVariable*
spReflectionVariableLayout_GetVariable(SlangReflectionVariableLayout* inVarLayout)
{
    auto varLayout = convert(inVarLayout);
    if (!varLayout)
        return nullptr;
...
SLANG_API SlangReflectionTypeLayout*
spReflectionVariableLayout_GetTypeLayout(SlangReflectionVariableLayout* inVarLayout)
{
    auto varLayout = convert(inVarLayout);
    if (!varLayout)
        return nullptr;
...
spReflectionVariableLayout_GetSemanticName(SlangReflectionVariableLayout* inVarLayout)
{
    auto varLayout = convert(inVarLayout);
    if (!varLayout)
        return 0;
...
SLANG_API SlangReflectionModifier*
spReflectionFunction_FindModifier(SlangReflectionFunction* inFunc, SlangModifierID modifierID)
{
...
SLANG_API SlangReflectionUserAttribute*
spReflectionFunction_GetUserAttribute(SlangReflectionFunction* inFunc, unsigned int index)
{
    auto func = convertToFunc(inFunc);
    if (!func)
        return nullptr;
...
SLANG_API SlangReflectionVariable*
spReflectionFunction_GetParameter(SlangReflectionFunction* inFunc, unsigned int index)
{
    auto func = convertToFunc(inFunc);
    if (!func)
        return nullptr;
...
SLANG_API unsigned int spReflectionGeneric_GetTypeParameterCount(SlangReflectionGeneric* generic)
{
    ...
    return (unsigned int
    )getMembersOfType<GenericTypeParamDecl>(astBuilder, slangGeneric.getDecl()->parentDecl)
        .getCount();
}
...
SLANG_API unsigned int spReflectionGeneric_GetValueParameterCount(SlangReflectionGeneric* generic)
{
    ...
    return (unsigned int
    )getMembersOfType<GenericValueParamDecl>(astBuilder, slangGeneric.getDecl()->parentDecl)
        .getCount();
}
...
SLANG_API SlangReflectionFunction*
spReflectionEntryPoint_getFunction(SlangReflectionEntryPoint* inEntryPoint)
{
    auto entryPointLayout = convert(inEntryPoint);
    if (entryPointLayout)
    {
...
SLANG_API SlangReflectionVariableLayout*
spReflectionEntryPoint_getParameterByIndex(SlangReflectionEntryPoint* inEntryPoint, unsigned index)
{
    auto entryPointLayout = convert(inEntryPoint);
    if (!entryPointLayout)
        return 0;
...
SLANG_API SlangReflectionVariableLayout*
spReflectionEntryPoint_getVarLayout(SlangReflectionEntryPoint* inEntryPoint)
{
    auto entryPointLayout = convert(inEntryPoint);
    if (!entryPointLayout)
        return nullptr;
...
SLANG_API SlangReflectionVariableLayout*
spReflectionEntryPoint_getResultVarLayout(SlangReflectionEntryPoint* inEntryPoint)
{
    auto entryPointLayout = convert(inEntryPoint);
    if (!entryPointLayout)
        return nullptr;
...
SLANG_API int
spReflectionEntryPoint_hasDefaultConstantBuffer(SlangReflectionEntryPoint* inEntryPoint)
{
    auto entryPointLayout = convert(inEntryPoint);
    if (!entryPointLayout)
        return 0;
...
SLANG_API unsigned int
spReflectionTypeParameter_GetConstraintCount(SlangReflectionTypeParameter* inTypeParam)
{
    auto specializationParam = convert(inTypeParam);
    if (auto genericParamLayout = as<GenericSpecializationParamLayout>(specializationParam))
    {
...
spReflection_GetParameterByIndex(SlangReflection* inProgram, unsigned index)
{
    auto program = convert(inProgram);
    if (!program)
        return nullptr;
...
SLANG_API SlangReflectionVariableLayout*
spReflection_getGlobalParamsVarLayout(SlangReflection* inProgram)
{
    auto program = convert(inProgram);
    if (!program)
        return nullptr;
...
SLANG_API SlangReflectionTypeParameter*
spReflection_FindTypeParameter(SlangReflection* inProgram, char const* name)
{
    auto program = convert(inProgram);
    if (!program)
        return nullptr;
...
spReflection_getEntryPointByIndex(SlangReflection* inProgram, SlangUInt index)
{
    auto program = convert(inProgram);
    if (!program)
        return 0;
...
SLANG_API SlangReflectionEntryPoint*
spReflection_findEntryPointByName(SlangReflection* inProgram, char const* name)
{
    auto program = convert(inProgram);
    if (!program)
        return 0;
