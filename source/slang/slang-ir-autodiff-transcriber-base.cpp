    ...
    if (kind == DiffConformanceKind::Any)
    {
        if (auto valueWitness =
                differentiableTypeConformanceContext
                    .tryGetDifferentiableWitness(builder, originalType, DiffConformanceKind::Value))
            return valueWitness;
        if (auto ptrWitness =
                differentiableTypeConformanceContext
                    .tryGetDifferentiableWitness(builder, originalType, DiffConformanceKind::Ptr))
            return ptrWitness;
    }
    else
    {
        return differentiableTypeConformanceContext
            .tryGetDifferentiableWitness(builder, originalType, kind);
    }
    return nullptr;
}
...
IRType* AutoDiffTranscriberBase::differentiateType(IRBuilder* builder, IRType* origType)
{
    ...
    {
        if (differentiableTypeConformanceContext
                .lookUpConformanceForType(origType, DiffConformanceKind::Value))
            return autoDiffSharedContext->differentiableInterfaceType;
        else if (differentiableTypeConformanceContext
                     .lookUpConformanceForType(origType, DiffConformanceKind::Ptr))
    ...
    switch (primalType->getOp())
    {
        ...
        default:
            return (IRType*)maybeCloneForPrimalInst(
                builder,
                differentiableTypeConformanceContext
                    .getDifferentialForType(builder, (IRType*)origType)
            );
    }
}
...
)
{
    ...
    if (auto specialize = as<IRSpecialize>(derivative))
    {
        ...
        migrationContext
            .init(derivativeGeneric, cast<IRGeneric>(findOuterGeneric(origFunc)), origFunc);
...
)
{
    ...
    auto primal =
        (IRSpecialize*)
            builder->emitLookupInterfaceMethodInst((IRType*)primalType, primalWt, primalKey);
    ...
    else if (auto returnWitnessType = as<IRWitnessTableTypeBase>(lookupInst->getDataType()))
    {
        ...
        else if (returnWitnessType->getConformanceType() ==
                 autoDiffSharedContext->differentiablePtrInterfaceType)
        {
            auto primalDiffType = builder->emitLookupInterfaceMethodInst(
                builder->getTypeKind(),
...
static void
_markGenericChildrenWithoutRelaventUse(IRGeneric* origGeneric, HashSet<IRInst*>& outInstsToSkip)
{
    for (;;)
    {
        bool changed = false;
