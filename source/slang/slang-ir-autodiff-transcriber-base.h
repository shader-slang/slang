...
struct AutoDiffTranscriberBase
{
    ...
    void
    maybeMigrateDifferentiableDictionaryFromDerivativeFunc(IRBuilder* builder, IRInst* origFunc);
    ...
    virtual InstPair
    transcribeFuncParam(IRBuilder* builder, IRParam* origParam, IRInst* primalType) = 0;

    InstPair transcribeLookupInterfaceMethod(IRBuilder* builder, IRLookupWitnessMethod* lookupInst);

    InstPair
    transcribeBlockImpl(IRBuilder* builder, IRBlock* origBlock, HashSet<IRInst*>& instsToSkip);
    ...
    void
    _markInstAsDifferential(IRBuilder* builder, IRInst* diffInst, IRInst* primalInst = nullptr);

    void copyOriginalDecorations(IRInst* origFunc, IRInst* diffFunc);

    virtual IRFuncType*
    differentiateFunctionType(IRBuilder* builder, IRInst* func, IRFuncType* funcType) = 0;
