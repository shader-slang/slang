...
struct SerialStringTableUtil
{
    ...
    static void
    encodeStringTable(const ConstArrayView<UnownedStringSlice>& slices, List<char>& stringTable);
    ...
    /// StringSlicePool)
    static void
    decodeStringTable(const char* table, size_t tableSize, List<UnownedStringSlice>& slicesOut);
...
struct SerialParseUtil
{
    ...
    static SlangResult
    parseCompressionType(const UnownedStringSlice& text, SerialCompressionType& outType);
...
struct SerialRiffUtil
{
    ...
    template<typename T>
    static Result
    writeArrayUncompressedChunk(FourCC chunkId, const List<T>& array, RiffContainer* container)
    {
        return writeArrayChunk(
