...
CapabilityAtom getLatestSpirvAtom()
{
    ...
    if (result == CapabilityAtom::Invalid)
    {
        ...
        result = asAtom(latestSpirvCapSetElements[latestSpirvCapSetElements.getCount() - 2]
        ); //-1 gets shader stage
    }
    return result;
}
...
CapabilityAtom getLatestMetalAtom()
{
    ...
    if (result == CapabilityAtom::Invalid)
    {
        ...
        result = asAtom(latestMetalCapSetElements[latestMetalCapSetElements.getCount() - 2]
        ); //-1 gets shader stage
    }
    return result;
}
...
CapabilitySet::_implies(CapabilitySet const& otherSet, ImpliesFlags flags) const
{
...
CapabilitySet::ImpliesReturnFlags
CapabilitySet::atLeastOneSetImpliedInOther(CapabilitySet const& other) const
{
...
{
    return capSet.getCapabilityTargetSets()[target].shaderStageSets[stage].atomSet->contains((UInt
    )atom);
}
