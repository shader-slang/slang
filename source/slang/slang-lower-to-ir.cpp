    ...
    ParamPassingMode defaultModeFromContext = kDefaultModeForImplicitThisParam)
{
    ...
    // `[ref]`, `[nonmutating]`) and declaration kinds (e.g. a `set` accessor)
    ...
    // declaration.
    //
    // At this point we read the mode stored on the declaration during semantic
    // checking (the single source of truth for the declared `this` mode), which
    // for a declaration with no explicit modifier is the default mode. Reading it
    // here is also where a future context-dependent default (computed at the store
    // site, where the source language and options are in scope) would flow through.
    //
    if (auto storedModifier =
            declWithImplicitThisParam->findModifier<ThisParamPassingModeModifier>())
    {
        return storedModifier->mode;
    }
