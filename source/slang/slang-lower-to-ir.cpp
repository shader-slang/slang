...
bool isStdLibMemberFuncDecl(Decl* decl);
        ...
        default:
            switch (tryEnv.clauseType)
            {
                case TryClauseType::None:
                {
                    ...
                    else
                    {
                        return LoweredValInfo::simple(
                            builder
                                ->emitCallInst(type, getSimpleVal(context, funcVal), argCount, args)
                        );
                    }
                }
                ...
                case TryClauseType::Standard:
                {
                    ...
                    builder
                        ->emitTryCallInst(voidType, succBlock, failBlock, callee, argCount, args);
...
LoweredValInfo
extractField(IRGenContext* context, IRType* fieldType, LoweredValInfo base, DeclRef<Decl> field)
{
...
IRInst*
getAddress(IRGenContext* context, LoweredValInfo const& inVal, SourceLoc diagnosticLocation);
...
static void addLinkageDecoration(IRGenContext* context, IRInst* inst, Decl* decl)
{
    ...
    if (context->shared->m_obfuscateCode && !isFromStdLib(decl))
    {
...
struct ValLoweringVisitor : ValVisitor<ValLoweringVisitor, LoweredValInfo, LoweredValInfo>
{
    ...
    LoweredValInfo visitEachType(EachType* eachType)
    {
        ...
        return LoweredValInfo::simple(getBuilder()->emitEachInst(getBuilder()->getTypeKind(), type)
        );
    }
    ...
    LoweredValInfo
    visitBackwardDifferentiateIntermediateTypeVal(BackwardDifferentiateIntermediateTypeVal* val)
    {
        auto funcVal = emitDeclRef(context, val->getFunc(), context->irBuilder->getTypeKind());
        SLANG_RELEASE_ASSERT(funcVal.flavor == LoweredValInfo::Flavor::Simple);
    ...
    LoweredValInfo
    visitExtractFromConjunctionSubtypeWitness(ExtractFromConjunctionSubtypeWitness* val)
    {
    ...
    IRType* visitDeclRefType(DeclRefType* type)
    {
        ...
        return (IRType*)
            getSimpleVal(context, emitDeclRef(context, declRef, context->irBuilder->getTypeKind()));
    }
    ...
    IRType* lowerSimpleIntrinsicType(DeclRefType* type)
    {
        ...
        return getBuilder()
            ->getType(op, static_cast<UInt>(operands.getCount()), operands.getBuffer());
    }
...
LoweredValInfo
tryGetAddress(IRGenContext* context, LoweredValInfo const& inVal, TryGetAddressMode mode);
...
void addArg(
    ...
    SourceLoc loc
) //< A location to use if we need to report an error
{
...
DeclRef<Decl>
createDefaultSpecializedDeclRefImpl(IRGenContext* context, SemanticsVisitor* semantics, Decl* decl)
{
    DeclRef<Decl> declRef =
...
template<typename D>
DeclRef<D>
createDefaultSpecializedDeclRef(IRGenContext* context, SemanticsVisitor* semantics, D* decl)
{
...
static Type* _findReplacementThisParamType(IRGenContext* context, DeclRef<Decl> parentDeclRef)
{
    ...
    if (auto interfaceDeclRef = parentDeclRef.as<InterfaceDecl>())
    {
        auto thisType = DeclRefType::create(
            context->astBuilder,
            context->astBuilder
                ->getMemberDeclRef(interfaceDeclRef, interfaceDeclRef.getDecl()->getThisTypeDecl())
        );
        return thisType;
    }
...
getParameterInfo(IRGenContext* context, DeclRef<VarDeclBase> const& paramDecl)
{
...
struct ExprLoweringVisitorBase : public ExprVisitor<Derived, LoweredValInfo>
{
    ...
    LoweredValInfo visitForwardDifferentiateExpr(ForwardDifferentiateExpr* expr)
    {
        ...
        return LoweredValInfo::simple(
            getBuilder()->emitForwardDifferentiateInst(lowerType(context, expr->type), baseVal.val)
        );
    }
    ...
    LoweredValInfo visitBackwardDifferentiateExpr(BackwardDifferentiateExpr* expr)
    {
        ...
        return LoweredValInfo::simple(
            getBuilder()->emitBackwardDifferentiateInst(lowerType(context, expr->type), baseVal.val)
        );
    }
    ...
    {
        return sharedLoweringContext
            .visitInvokeExprImpl(expr, LoweredValInfo(), TryClauseEnvironment());
    }
    ...
    : ExprVisitor<DestinationDrivenRValueExprLoweringVisitor>
{
    ...
    void visitInvokeExpr(InvokeExpr* expr)
    {
        LoweredValInfo resultRVal;
        {
            ...
            resultRVal = sharedLoweringContext
                             .visitInvokeExprImpl(expr, destination, TryClauseEnvironment{});
        }
...
void maybeEmitDebugLine(IRGenContext* context, StmtLoweringVisitor& visitor, Stmt* stmt)
{
    ...
    auto humaneLoc =
        context->getLinkage()->getSourceManager()->getHumaneLoc(stmt->loc, SourceLocType::Emit);
...
#undef IGNORED_CASE
        ...
        for (auto entry : astWitnessTable->getRequirementDictionary())
        {
            ...
            switch (satisfyingWitness.getFlavor())
            {
                ...
                case RequirementWitness::Flavor::witnessTable:
                {
                    ...
                    if (!mapASTToIRWitnessTable
                             .tryGetValue(astReqWitnessTable, irSatisfyingWitnessTable))
                    {
    ...
    LoweredValInfo lowerGlobalShaderParam(VarDecl* decl)
    {
        ...
        for (auto modifier : decl->modifiers)
        {
            ...
            else if (as<SpecializationConstantAttribute>(modifier) ||
    ...
    LoweredValInfo visitAggTypeDecl(AggTypeDecl* decl)
    {
        ...
        {
            getSink()
                ->diagnose(decl->loc, Diagnostics::unimplemented, "lower unknown AggType to IR");
    ...
    void
    emitGenericConstraintDecl(IRGenContext* subContext, GenericTypeConstraintDecl* constraintDecl)
    {
    ...
    /// declaration?
    bool isStdLibMemberFuncDecl(Decl* inDecl)
    {
    ...
    )
    {
        ...
        for (auto modifier : decl->modifiers)
        {
            ...
            else if (as<UserDefinedDerivativeAttribute>(modifier) ||
            ...
            else if (auto attr = as<PreferRecomputeAttribute>(modifier))
            {
                getBuilder()->addDecoration(
                    ...
                    getBuilder()->getIntValue(getBuilder()->getIntType(), attr->sideEffectBehavior)
                );
            }
...
LoweredValInfo emitDeclRef(IRGenContext* context, Decl* decl, DeclRefBase* subst, IRType* type)
{
    ...
    if (auto genericSubst = as<GenericAppDeclRef>(subst))
    {
        ...
        auto irSpecializedVal =
            context->irBuilder
                ->emitSpecializeInst(type, irGenericVal, irArgs.getCount(), irArgs.getBuffer());
...
static void
lowerFrontEndEntryPointToIR(IRGenContext* context, EntryPoint* entryPoint, String moduleName)
{
...
RefPtr<IRModule>
generateIRForTranslationUnit(ASTBuilder* astBuilder, TranslationUnitRequest* translationUnit)
{
    SLANG_PROFILE;
...
RefPtr<IRModule>
generateIRForSpecializedComponentType(SpecializedComponentType* componentType, DiagnosticSink* sink)
{
...
struct TypeConformanceIRGenContext
{
    ...
    RefPtr<IRModule>
    process(TypeConformance* typeConformance, Int conformanceIdOverride, DiagnosticSink* inSink)
    {
        sink = inSink;
...
IRTypeLayout* lowerTypeLayout(IRLayoutGenContext* context, TypeLayout* typeLayout)
{
    ...
    if (auto paramGroupTypeLayout = as<ParameterGroupTypeLayout>(typeLayout))
    {
        ...
        builder.setElementVarLayout(lowerVarLayout(context, paramGroupTypeLayout->elementVarLayout)
        );
...
IRVarLayout*
lowerVarLayout(IRLayoutGenContext* context, VarLayout* varLayout, IRTypeLayout* irTypeLayout)
{
...
IREntryPointLayout*
lowerEntryPointLayout(IRLayoutGenContext* context, EntryPointLayout* entryPointLayout)
{
