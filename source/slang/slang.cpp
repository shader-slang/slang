...
SlangResult Session::compileStdLib(slang::CompileStdLibFlags compileFlags)
{
    ...
    stdLibSrcBuilder << (const char*)getCoreLibraryCode()->getBufferPointer()
    ...
    if (compileFlags & slang::CompileStdLibFlag::WriteDocumentation)
    {
        ...
        if (SLANG_FAILED(File::readAllText("config.txt", configText)))
        {
            fprintf(
                stderr,
                "Error writing documentation: config file not found on current working directory.\n"
            );
        }
        else
        {
...
SlangResult
Session::_readBuiltinModule(ISlangFileSystem* fileSystem, Scope* scope, String moduleName)
{
...
)
{
    ...
    {
        m_codeGenTransitionMap
            .addTransition(CodeGenTarget(source), CodeGenTarget(target), PassThroughMode(compiler));
    }
}
...
{
    PassThroughMode compilerType = (PassThroughMode
    )getDownstreamCompilerForTransition(SlangCompileTarget(source), SlangCompileTarget(target));
...
DeclRef<GenericDecl>
getGenericParentDeclRef(ASTBuilder* astBuilder, SemanticsVisitor* visitor, DeclRef<Decl> declRef)
{
...
DeclRef<Decl>
Linkage::specializeWithArgTypes(Expr* funcExpr, List<Type*> argTypes, DiagnosticSink* sink)
{
    SemanticsVisitor visitor(getSemanticsForReflection());
...
DeclRef<Decl>
Linkage::specializeGeneric(DeclRef<Decl> declRef, List<Expr*> argExprs, DiagnosticSink* sink)
{
    SLANG_AST_BUILDER_RAII(getASTBuilder());
...
static void
_calcViewInitiatingHierarchy(SourceManager* sourceManager, ViewInitiatingHierarchy& outHierarchy)
{
...
)
{
    ...
    else if (filePathInfo.type == PathInfo::Type::Normal ||
...
)
{
    ...
    for (auto checkBinaryModule : shouldCheckBinaryModuleSettings)
    {
        ...
        for (int translateUnderScore = 0; translateUnderScore <= 1; translateUnderScore++)
        {
            ...
            if (mapPathToLoadedModule
                    .tryGetValue(filePathInfo.getMostUniqueIdentity(), loadedModule))
                return loadedModule;
...
RefPtr<ComponentType>
CompositeComponentType::create(Linkage* linkage, List<RefPtr<ComponentType>> const& childComponents)
{
...
SlangResult
EndToEndCompileRequest::addTargetCapability(SlangInt targetIndex, SlangCapabilityID capability)
{
...
SlangResult
EndToEndCompileRequest::setGlobalGenericArgs(int genericArgCount, char const** genericArgs)
{
...
SlangResult EndToEndCompileRequest::compile()
{
    ...
    // Repro dump handling
    {
        ...
        else if (dumpReproOnError && SLANG_FAILED(res))
        {
            ...
            {
                getSink()
                    ->diagnose(SourceLoc(), Diagnostics::unableToWriteReproFile, reproFileName);
            }
        }
    }
...
SlangResult
EndToEndCompileRequest::getCompileTimeProfile(ISlangProfiler** compileTimeProfile, bool shouldClear)
{
...
static SlangResult
_getWholeProgramResult(EndToEndCompileRequest* req, int targetIndex, ComPtr<IArtifact>& outArtifact)
{
    auto linkage = req->getLinkage();
    auto program = req->getSpecializedGlobalAndEntryPointsComponentType();
...
SlangResult
EndToEndCompileRequest::loadRepro(ISlangFileSystem* fileSystem, const void* data, size_t size)
{
    List<uint8_t> buffer;
...
SlangResult
EndToEndCompileRequest::processCommandLineArguments(char const* const* args, int argCount)
{
...
SlangResult
EndToEndCompileRequest::getModule(SlangInt translationUnitIndex, slang::IModule** outModule)
{
