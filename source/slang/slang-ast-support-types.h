...
enum : ConversionCost
{
    ...
    kConversionCost_BoolToInt =
        120, // Converting bool to int has lower cost than other integer types to prevent ambiguity.
...
{
    SlangScalarType scalarType; ///< If image format is not made up of channels of set sizes this
                                ///< will be SLANG_SCALAR_TYPE_NONE
    uint8_t channelCount;       ///< The number of channels
    uint8_t sizeInBytes;        ///< Size in bytes
    UnownedStringSlice name;    ///< The name associated with this type. NOTE! Currently these names
                                ///< *are* the GLSL format names.
};
...
public:
    ...
    LookupResultItem_Breadcrumb(
        ...
        RefPtr<LookupResultItem_Breadcrumb> next,
        ThisParameterMode thisParameterMode = ThisParameterMode::Default)
        : kind(kind), thisParameterMode(thisParameterMode), declRef(declRef), val(val), next(next)
    {
    }
...
struct LookupRequest
{
    ...
    // A decl to exclude from the lookup, used to exclude the current decl being checked, such as in
    // typedef Foo Foo; to avoid finding itself.
