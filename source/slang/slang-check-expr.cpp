...
InvokeExpr*
SemanticsVisitor::constructUncheckedInvokeExpr(Expr* callee, const List<Expr*>& arguments)
{
...
)
{
    ...
    if (!allConst)
    {
        ...
        else if (opName == getName("/") || opName == getName("==") || opName == getName(">=") ||
                 ...
                 opName == getName("?:") || opName == getName("<<") || opName == getName(">>"))
        {
            auto result = m_astBuilder->getOrCreate<FuncCallIntVal>(
                ...
                funcDeclRef,
                as<Type>(funcDeclRefExpr.getExpr()
                             ->type->substitute(m_astBuilder, funcDeclRefExpr.getSubsts())),
...
void SemanticsVisitor::compareMemoryQualifierOfParamToArgument(ParamDecl* paramIn, Expr* argIn)
{
    ...
    if (argQualifiers & MemoryQualifierSetModifier::Flags::kWriteOnly &&
        ...
        getSink()
            ->diagnose(arg, Diagnostics::argumentHasMoreMemoryQualifiersThanParam, "writeonly");
                ...
                if (as<OutTypeBase>(paramType) || as<RefType>(paramType))
                {
                    ...
                    if (argExpr)
                    {
                        if (!argExpr->type.isLeftValue)
                        {
                            ...
                            {
                                getSink()
                                    ->diagnose(argExpr, Diagnostics::argumentExpectedLValue, pp);
        ...
        if (auto declRefExpr = as<DeclRefExpr>(funcExpr))
        {
            if (auto baseFuncGenericDeclRef = declRefExpr->declRef.as<GenericDecl>())
            {
                ...
                DeclRef<Decl> unspecializedInnerRef = createDefaultSubstitutionsIfNeeded(
                    ...
                    semantics,
                    astBuilder
                        ->getMemberDeclRef(baseFuncGenericDeclRef, getInner(baseFuncGenericDeclRef))
                );
...
struct ForwardDifferentiateExprCheckingActions : HigherOrderInvokeExprCheckingActions
{
    ...
    ) override
    {
        ...
        if (!baseFuncType)
        {
            ...
            semantics->getSink()
                ->diagnose(funcExpr, Diagnostics::expectedFunction, funcExpr->type.type);
            return;
        }
...
)
{
    ...
    getSink()
        ->diagnose(memberRefExpr, Diagnostics::unimplemented, "swizzle on matrix of unknown size");
    return CreateErrorExpr(memberRefExpr);
}
            ...
            if (auto matrixType = as<MatrixExpressionType>(baseType))
            {
                if (as<ColumnMajorLayoutModifier>(matrixLayoutModifier))
                {
                    baseType = m_astBuilder->getMatrixType(
                        ...
                        m_astBuilder
                            ->getIntVal(m_astBuilder->getIntType(), kMatrixLayoutMode_ColumnMajor)
                    );
                }
                else
                {
                    baseType = m_astBuilder->getMatrixType(
                        ...
                        matrixType->getColumnCount(),
                        m_astBuilder
                            ->getIntVal(m_astBuilder->getIntType(), kMatrixLayoutMode_RowMajor)
                    );
                }
