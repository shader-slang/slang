    ...
    Type* targetType)
{
    ...
    else if (auto invokeExpr = as<InvokeExpr>(baseExpr))
    {
        ...
        // `RWStructuredBuffer` or `RasterizerOrderedStructuredBuffer`. Both share this ref
        // accessor, and `&buf[i]` already compiles for them; #10280 makes `__getAddress`
        // equivalent to `&`, so accept the same set and produce the same pointer type.
        // The element lives in device memory (`AddressSpace.Device` == `UserPointer`), and
        // the layout is `DefaultDataLayout` to match `&buf[i]` (the element offset is
        // resolved from the buffer type's own layout at IR generation, so this pointer's
        // layout argument does not affect stride).
