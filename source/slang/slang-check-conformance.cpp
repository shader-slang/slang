...
SubtypeWitness*
SemanticsVisitor::isSubtype(Type* subType, Type* superType, IsSubTypeOptions isSubTypeOptions)
{
...
)
{
    ...
    else if (auto subTypePack = as<ConcreteTypePack>(subType))
    {
        ...
        return m_astBuilder
            ->getSubtypeWitnessPack(subType, superType, elementWitnesses.getArrayView().arrayView);
    }
...
SubtypeWitness* SemanticsVisitor::isTypeDifferentiable(Type* type)
{
    if (auto valueWitness =
        ...
        return valueWitness;
    else if (auto ptrWitness = isSubtype(
                 type,
