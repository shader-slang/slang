...
struct DifferentiableTypeConformanceContext
{
    ...
    IRInst*
    tryGetDifferentiableWitness(IRBuilder* builder, IRInst* originalType, DiffConformanceKind kind);
    ...
    List<IRInterfaceRequirementEntry*>
    findInterfaceLookupPath(IRInterfaceType* supType, IRInterfaceType* type);
    ...
    IRInst* getDifferentialForType(IRBuilder* builder, IRType* origType)
    {
        switch (origType->getOp())
        {
            ...
            {
                auto diffElementType = (IRType*)
                    getDifferentialForType(builder, as<IRArrayType>(origType)->getElementType());
                if (!diffElementType)
                    return nullptr;
    ...
    bool isDifferentiableValueType(IRType* origType)
    {
        for (; origType;)
        {
            switch (origType->getOp())
            {
                ...
                default:
                    return lookUpConformanceForType(origType, DiffConformanceKind::Value) !=
                           nullptr;
            }
        }
        return false;
    ...
    IRInst*
    buildArrayWitness(IRBuilder* builder, IRArrayType* pairType, DiffConformanceKind target);
...
UIndex
addPhiOutputArg(IRBuilder* builder, IRBlock* block, IRInst*& inoutTerminatorInst, IRInst* arg);
