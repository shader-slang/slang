...
ABSTRACT_SYNTAX_CLASS(ContainerDecl, Decl)
    ...
    bool isMemberDictionaryValid() const { return dictionaryLastMemberIndex == Members.getCount(); }
    ...
    // If it's >= 0, then the Members after dictionaryLastMemberIndex are all that need to be added.
    // If it < 0 it means that the dictionary/transparentMembers is invalid and needs to be recreated.
    Index dictionaryLastMemberIndex = -1;
