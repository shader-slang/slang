...
bool MetalSourceEmitter::tryEmitInstExprImpl(IRInst* inst, const EmitOpInfo& inOuterPrec)
{
    ...
    case kIROp_MakeMatrix:
        {
            // For Metal, when matrices are lowered to arrays of vectors,
            ...
            {
                // For array types, Metal requires brace initialization instead of constructor syntax
                if (as<IRArrayType>(dataType))
                {
                    emitType(dataType);
                    m_writer->emit("{");
            ...
            // Check if this is a lowered matrix (array of vectors)
            if (auto arrayType = as<IRArrayType>(dataType))
            {
                auto vectorType = as<IRVectorType>(arrayType->getElementType());
                if (vectorType)
                {
                    IRIntegerValue rowCount = getIntVal(arrayType->getElementCount());
                    IRIntegerValue colCount = getIntVal(vectorType->getElementCount());
                    
                    // Use brace initialization for arrays in Metal
                    emitType(dataType);
                    m_writer->emit("{");
