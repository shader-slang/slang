...
struct DefaultLayoutRulesImpl : SimpleLayoutRulesImpl
{
    ...
    SimpleArrayLayoutInfo
    GetArrayLayout(SimpleLayoutInfo elementInfo, LayoutSize elementCount) override
    {
...
struct GLSLSpecializationConstantLayoutRulesImpl : DefaultLayoutRulesImpl
{
    ...
    SimpleLayoutInfo
    GetVectorLayout(BaseType elementType, SimpleLayoutInfo, size_t elementCount) override
    {
        SLANG_UNUSED(elementType);
...
{
    virtual ObjectLayoutInfo
    GetObjectLayout(ShaderParameterKind kind, const Options& options) override
    {
        int slotCount = 1;
...
GLSLRayTracingLayoutRulesImpl
    kGLSLCallablePayloadParameterLayoutRulesImpl(LayoutResourceKind::CallablePayload);
GLSLRayTracingLayoutRulesImpl
    kGLSLHitAttributesParameterLayoutRulesImpl(LayoutResourceKind::HitAttributes);
...
HLSLRayTracingLayoutRulesImpl
    kHLSLCallablePayloadParameterLayoutRulesImpl(LayoutResourceKind::CallablePayload);
HLSLRayTracingLayoutRulesImpl
    kHLSLHitAttributesParameterLayoutRulesImpl(LayoutResourceKind::HitAttributes);
...
CUDARayTracingLayoutRulesImpl
    kCUDAHitAttributesParameterLayoutRulesImpl(LayoutResourceKind::HitAttributes);
...
struct GLSLLayoutRulesFamilyImpl : LayoutRulesFamilyImpl
{
    ...
    virtual LayoutRulesImpl* getShaderStorageBufferRules(CompilerOptionSet& compilerOptions
    ) override;
...
LayoutRulesImpl*
GLSLLayoutRulesFamilyImpl::getConstantBufferRules(CompilerOptionSet& compilerOptions)
{
...
LayoutRulesImpl*
GLSLLayoutRulesFamilyImpl::getParameterBlockRules(CompilerOptionSet& compilerOptions)
{
    if (compilerOptions.shouldUseScalarLayout())
        return &kScalarLayoutRulesImpl_;
    else if (compilerOptions.shouldUseDXLayout())
...
LayoutRulesImpl*
GLSLLayoutRulesFamilyImpl::getShaderStorageBufferRules(CompilerOptionSet& compilerOptions)
{
    if (compilerOptions.shouldUseScalarLayout())
        return &kScalarLayoutRulesImpl_;
    else if (compilerOptions.shouldUseDXLayout())
...
LayoutRulesImpl*
GLSLLayoutRulesFamilyImpl::getStructuredBufferRules(CompilerOptionSet& compilerOptions)
{
    if (compilerOptions.shouldUseScalarLayout())
        return &kScalarLayoutRulesImpl_;
    else if (compilerOptions.shouldUseDXLayout())
...
///
static TypeLayoutResult
createSimpleTypeLayout(SimpleLayoutInfo info, Type* type, LayoutRulesImpl* rules)
{
...
static TypeLayoutResult
createSimpleTypeLayout(const ObjectLayoutInfo& info, Type* type, LayoutRulesImpl* rules)
{
    RefPtr<TypeLayout> typeLayout = new TypeLayout();
...
RefPtr<TypeLayout>
applyOffsetToTypeLayout(RefPtr<TypeLayout> oldTypeLayout, RefPtr<VarLayout> offsetVarLayout)
{
...
IRVarLayout*
applyOffsetToVarLayout(IRBuilder* irBuilder, IRVarLayout* baseLayout, IRVarLayout* offsetLayout)
{
...
static bool
needsConstantBuffer(TypeLayoutContext const& context, RefPtr<TypeLayout> elementTypeLayout)
{
...
///
static TypeLayoutResult
_createTypeLayout(TypeLayoutContext const& context, Type* type, Decl* declForModifiers)
{
...
)
{
    ...
    if (auto originalArrayTypeLayout = originalTypeLayout.as<ArrayTypeLayout>())
    {
        ...
        return adjustedArrayTypeLayout;
                 ...
                 originalTypeLayout.as<ParameterGroupTypeLayout>())
    {
...
RefPtr<VarLayout>
StructTypeLayoutBuilder::addField(DeclRef<Decl> field, TypeLayoutResult fieldResult)
{
...
RefPtr<VarLayout>
StructTypeLayoutBuilder::addField(DeclRef<VarDeclBase> field, RefPtr<TypeLayout> fieldTypeLayout)
{
...
)
{
    ...
    for (auto elementResourceInfo : elementTypeLayout->resourceInfos)
    {
        ...
        //
        else if (elementCount.isInfinite() && adjustedElementTypeLayout != elementTypeLayout &&
...
static TypeLayoutResult
_updateLayout(TypeLayoutContext& context, Type* type, const TypeLayoutResult& result)
{
...
#undef CASE
    ...
    else if (auto matType = as<MatrixExpressionType>(type))
    {
        ...
        auto info =
            rules
                ->GetMatrixLayout(elementBaseType, elementInfo, layoutMajorCount, layoutMinorCount);
...
RefPtr<TypeLayout>
createTypeLayoutWith(const TypeLayoutContext& context, LayoutRulesImpl* rules, Type* type)
{
