    ...
    for (int tid = 0; tid < int(DimType::Count); tid++)
    {
        ...
        for (int includeMipInfo = 0; includeMipInfo < 2; ++includeMipInfo)
        {
            ...
            if (isArray)
            {
                ...
                // For cube map arrays, CUDA includes all 6 faces in the array size count
                // So we need to divide by 6 to get the actual array size for the user
                if (baseShape == SLANG_TEXTURE_CUBE) {
                    if (cuda.getLength() > 1 && cuda[cuda.getLength() - 1] != ';')
                        cuda << "; ";
                    cuda << "uint32_t a; asm(\\\"txq.array_size.b32 %0, [%1];\\\" : \\\"=r\\\"(a) : \\\"l\\\"($0)); a /= 6; *($"
                         << String(paramCount) << ") = a;";
                } else {
                    if (cuda.getLength() > 1 && cuda[cuda.getLength() - 1] != ';')
                        cuda << "; ";
                    cuda << "uint32_t a; asm(\\\"txq.array_size.b32 %0, [%1];\\\" : \\\"=r\\\"(a) : \\\"l\\\"($0)); *($"
                         << String(paramCount) << ") = a;";
                }
            ...
            if (isMultisample)
            {
                ...
                if (cuda.getLength() > 1 && cuda[cuda.getLength() - 1] != ';')
                    cuda << "; ";
                cuda << "uint32_t s; asm(\\\"txq.samples.b32 %0, [%1];\\\" : \\\"=r\\\"(s) : \\\"l\\\"($0)); *($"
                     << String(paramCount) << ") = s;";
                wgsl << "($" << String(paramCount)
            ...
            if (includeMipInfo)
            {
                ...
                if (cuda.getLength() > 1 && cuda[cuda.getLength() - 1] != ';')
                    cuda << "; ";
                cuda << "uint32_t l; asm(\\\"txq.num_mipmap_levels.b32 %0, [%1];\\\" : \\\"=r\\\"(l) : \\\"l\\\"($0)); *($"
                     << String(paramCount) << ") = l;";
                wgsl << "($" << String(paramCount)
