...
/// (`BorrowIn`), `[__ref]` passes it by mutable reference (`Ref`), `[nonmutating]` passes it by
/// value (`In`), and a `set` accessor (`SetterDecl`) defaults to a mutable `this` (`BorrowInOut`).
/// The
...
///
/// This is the query the front-end consumers use (and the one IR lowering uses at its leaf) to
/// read the cached declared mode; IR lowering's attribute-driven branch instead calls
/// `tryGetDeclaredThisParamPassingMode` directly. It first reads the
