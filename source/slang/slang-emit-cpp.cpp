    ...
    case kIROp_ArrayType:
        {
            ...
            if (auto nameHint = arrayType->findDecoration<IRNameHintDecoration>())
            {
                if (nameHint->getName() == UnownedStringSlice("CoopVec"))
                {
                    // Emit OptixCoopVec<T, N> for cooperative vectors in CUDA
