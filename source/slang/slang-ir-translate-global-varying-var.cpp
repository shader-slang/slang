    ...
    void processModule(IRModule* module)
    {
        ...
        for (auto entryPoint : entryPoints)
        {
            ...
            if (!entryPointParamsStructLayout)
                // Do we really want to continue here? What if we have no entry point
                // params.
                continue;

            // Need to fix up the offsets of the global varing inputs being added.
            // Find the size and offset of the last varying in kind param in the
            // entry point param struct. Adding these will give us the starting
            // offset that should be used for the first global.
            UInt nextOffset = 0;
            ...
            {
                // Should we error out if no sizeAttr or offsetAttr?
                auto sizeAttr = lastFieldVarLayout->getTypeLayout()->findSizeAttr(LayoutResourceKind::VaryingInput);
                size_t finiteSize = sizeAttr->getFiniteSize();
                auto offsetAttr = lastFieldVarLayout->findOffsetAttr(LayoutResourceKind::VaryingInput);
            ...
            for (auto input : inputVars)
            {
                ...
                else
                {
                    ...
                    if (auto layoutDecor = findVarLayout(input))
                    {
                        ...
                        {
                            // Make sure to add nextOffset here to adjust the global vars original
                            // offset.
                            // TODO: Is there an instance in which we will need to add the
                            //       offsetAttr?
                            varLayoutBuilder
                                .findOrAddResourceInfo(LayoutResourceKind::VaryingInput)
                                ->offset = (UInt)offsetAttr->getOffset() + nextOffset;
                        }
                    }
                ...
                // TODO: note about wgsl failures that occur without this?
                IRCloneEnv cloneEnv;
                ...
                // to connect the param to the old global var later on.
            ...
            if (entryPointLayout)
            {
                if (paramLayout)
                {
                    // TODO: Add note describing the difference between IRParameterGroupTypeLayout and IRStructTypeLayout
                    //       here. 
                    ...
                    {
                        // TODO: Replace this instance and the one in slang-emit.cpp with
                        // a diagnostic?
