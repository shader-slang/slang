...
#define SLANG_PASS(passFunc, ...)                                                          \
    ...
    // so they are always present before the first `calcRequiredLoweringPassSet`
    // scan that governs this call site; the flag cannot be a false-negative.
    ...
    // by the front end (slang-lower-to-ir.cpp), so it is always present before
    // the last `calcRequiredLoweringPassSet` scan; the flag cannot be a
    // false-negative. The pass diagnoses only from these insts, so when the flag
    // is false it is a pure no-op and gating drops no diagnostic.
