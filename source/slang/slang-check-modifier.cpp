...
)
{
    ...
    else if (auto waveSizeAttr = as<WaveSizeAttribute>(attr))
    {
        ...
        if (arg)
        {
            ...
            if (auto constIntVal = as<ConstantIntVal>(intValue))
            {
                ...
                {
                    getSink()
                        ->diagnose(attr, Diagnostics::invalidWaveSize, constIntVal->getValue());
                    return nullptr;
                }
            }
            value = intValue;
    ...
    else if (auto glslRequireShaderInputParameter =
                 as<GLSLRequireShaderInputParameterAttribute>(attr))
    {
        if (attr->args.getCount() != 1)
        {
            return nullptr;
    ...
    else if (auto inputAttachmentIndexLayoutAttribute =
    ...
    else if ((as<DomainAttribute>(attr)) || (as<MaxTessFactorAttribute>(attr)) ||
...
bool isModifierAllowedOnDecl(bool isGLSLInput, ASTNodeType modifierType, Decl* decl)
{
    switch (modifierType)
    {
        ...
        case ASTNodeType::HLSLExportModifier:
        case ASTNodeType::ExternCppModifier:
            return as<VarDeclBase>(decl) || as<AggTypeDeclBase>(decl) ||
                   as<NamespaceDeclBase>(decl) || as<CallableDecl>(decl) || as<TypeDefDecl>(decl) ||
                   as<PropertyDecl>(decl) || as<SyntaxDecl>(decl) || as<AttributeDecl>(decl) ||
...
)
{
    ...
    if (as<PrivateModifier>(m))
    {
        if (auto decl = as<Decl>(syntaxNode))
        {
            ...
            {
                getSink()
                    ->diagnose(m, Diagnostics::invalidUseOfPrivateVisibility, as<Decl>(syntaxNode));
                return m;
            }
        }
