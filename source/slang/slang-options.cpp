...
void initCommandOptions(CommandOptions& options)
{
    ...
    {
        { OptionKind::ArchiveType, "-archive-type", "-archive-type <archive-type>", "Set the archive type for -save-standard-modules. Default is zip." },
        ...
        { OptionKind::LoadStandardModules, "-load-standard-modules", "-load-standard-modules <filename>", "Load the standard modules from file." },
        { OptionKind::ReferenceModule, "-r", "-r <name>", "reference module <name>" },
        { OptionKind::SaveStandardModules, "-save-standard-modules", "-save-standard-modules <filename>", "Save the standard modules to an archive file." },
        { OptionKind::SaveStandardModulesBinSource, "-save-standard-modules-bin-source","-save-standard-modules-bin-source <filename>", "Same as -save-standard-modules but output "
...
struct OptionsParser
{
    ...
    slang::CompileStandardModulesFlags m_compileStdLibFlags;
    ...
    while (m_reader.hasArg())
    {
        ...
        switch (optionKind)
        {
            ...
            case OptionKind::NoCodeGen:
                ...
                break;
            case OptionKind::LoadStandardModules:
            {
                ...
                SLANG_RETURN_ON_FAIL(m_session->loadStandardModules(contents.getData(), contents.getSizeInBytes()));
            ...
            case OptionKind::SaveStandardModules:
            {
                CommandLineArg fileName;
                SLANG_RETURN_ON_FAIL(m_reader.expectArg(fileName));
                ...
                SLANG_RETURN_ON_FAIL(m_session->saveStandardModules(m_archiveType, blob.writeRef()));
                SLANG_RETURN_ON_FAIL(File::writeAllBytes(fileName.value, blob->getBufferPointer(), blob->getBufferSize()));
                break;
            }
            case OptionKind::SaveStandardModulesBinSource:
            {
                CommandLineArg fileName;
                SLANG_RETURN_ON_FAIL(m_reader.expectArg(fileName));
            ...
            case OptionKind::Doc:
            {
                ...
                m_compileStdLibFlags |= slang::CompileStandardModulesFlag::WriteDocumentation;
