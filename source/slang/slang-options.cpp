...
struct OptionsParser
{
    ...
    void
    addInputForeignShaderPath(String const& path, SlangSourceLanguage language, Stage impliedStage);

    static Profile::RawVal findGlslProfileFromPath(const String& path);

    SlangResult
    addInputPath(char const* inPath, SourceLanguage langOverride = SourceLanguage::Unknown);
...
SlangResult OptionsParser::addInputPath(char const* inPath, SourceLanguage langOverride)
{
    ...
    if (sourceLanguage == SLANG_SOURCE_LANGUAGE_UNKNOWN)
    {
        m_requestImpl->getSink()
            ->diagnose(SourceLoc(), Diagnostics::cannotDeduceSourceLanguage, inPath);
        return SLANG_FAIL;
    }
...
static SlangResult
_loadRepro(const String& path, DiagnosticSink* sink, EndToEndCompileRequest* request)
{
    List<uint8_t> buffer;
...
SlangResult
OptionsParser::_expectValue(ValueCategory valueCategory, CommandOptions::UserValue& outValue)
{
    CommandLineArg arg;
        ...
        switch (optionKind)
        {
            ...
            case OptionKind::LoadStdLib:
            {
                ...
                SLANG_RETURN_ON_FAIL(
                    m_session->loadStdLib(contents.getData(), contents.getSizeInBytes())
                );
            ...
            case OptionKind::ReproFallbackDirectory:
            {
                ...
                else
                {
                    ...
                    SlangPathType pathType;
                    if (SLANG_FAILED(
                            osFileSystem->getPathType(reproDirectory.value.getBuffer(), &pathType)
                        ) ||
                        pathType != SLANG_PATH_TYPE_DIRECTORY)
                    {
                        return SLANG_FAIL;
            ...
            case OptionKind::Target:
            {
                ...
                const CodeGenTarget format = (CodeGenTarget
                )TypeTextUtil::findCompileTargetFromName(name.value.getUnownedSlice());

                if (format == CodeGenTarget::Unknown)
                {
                    m_sink
                        ->diagnose(name.loc, Diagnostics::unknownCodeGenerationTarget, name.value);
                    return SLANG_FAIL;
                }
            ...
            case OptionKind::VulkanBindShift:
            {
                ...
                if (m_reader.hasArg() && m_reader.peekArg().value == toSlice("all"))
                {
                    m_reader.advance();
                    linkage->m_optionSet
                        .add(CompilerOptionName::VulkanBindShiftAll, (int)kind, (int)shift);
                }
                else
                {
                    Int set;
            ...
            case OptionKind::VulkanBindGlobals:
            {
                ...
                linkage->m_optionSet
                    .set(OptionKind::VulkanBindGlobals, (int)binding, (int)bindingSet);
                break;
            }
            ...
            case OptionKind::Language:
            {
                ...
                else
                {
                    ...
                    {
                        SLANG_RETURN_ON_FAIL(
                            addInputPath(m_reader.getValueAndAdvance().getBuffer(), sourceLanguage)
                        );
                    }
                }
            ...
            case OptionKind::DefaultDownstreamCompiler:
            {
                ...
                if (SLANG_FAILED(
                        TypeTextUtil::findPassThrough(compilerArg.value.getUnownedSlice(), compiler)
                    ))
                {
                    m_sink->diagnose(
                        compilerArg.loc,
            ...
            case OptionKind::SourceEmbedLanguage:
            {
                ...
                if (!SourceEmbedUtil::isSupported((SlangSourceLanguage
                    )m_requestImpl->m_sourceEmbedLanguage))
                {
...
SlangResult
OptionsParser::parse(SlangCompileRequest* compileRequest, int argc, char const* const* argv)
{
