...
AutoDiffSharedContext::getInterfaceEntryAtIndex(IRInterfaceType* interface, UInt index)
{
...
)
{
    ...
    if (auto baseType = getConformanceTypeFromWitness(foundResult))
    {
        if (baseType == sharedContext->differentiableInterfaceType &&
            ...
            return foundResult;
        else if (baseType == sharedContext->differentiablePtrInterfaceType &&
...
IRInst* DifferentiableTypeConformanceContext::
    getDifferentialTypeFromDiffPairType(IRBuilder*, IRDifferentialPairTypeBase*)
{
...
IRType*
DifferentiableTypeConformanceContext::differentiateType(IRBuilder* builder, IRInst* primalType)
{
...
)
{
    ...
    if (differentiateType(builder, extractExistentialType->getOperand(0)->getDataType()))
    {
        return tryExtractConformanceFromInterfaceType(
            ...
            (IRWitnessTable*)
                builder->emitExtractExistentialWitnessTable(extractExistentialType->getOperand(0))
        );
    }
                    ...
                    switch (inst->getOp())
                    {
                        ...
                        case kIROp_BackwardDiffIntermediateContextType:
                            ...
                            switch (inst->getOperand(0)->getOp())
                            {
                                ...
                                case kIROp_Specialize:
                                case kIROp_LookupWitness:
                                    if (auto innerFunc = as<IRFunc>(
                                            getResolvedInstForDecorations(inst->getOperand(0))
                                        ))
                                    {
                ...
                for (auto task : followUpWorkList)
                {
                    ...
                    switch (task.type)
                    {
                        ...
                        case FuncBodyTranscriptionTaskType::BackwardPropagate:
                            backwardPropagateTranscriber
                                .transcribeFunc(builder, primalFunc, diffFunc);
                            break;
                        default: break;
                    }
...
bool processAutodiffCalls(TargetProgram* target, IRModule* module, DiagnosticSink* sink, IRAutodiffPassOptions const&)
{
    SLANG_PROFILE;
    bool modified = false;
