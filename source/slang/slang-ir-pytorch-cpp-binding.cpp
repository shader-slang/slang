...
static IRInst* makeValueFromTargetTuple(IRBuilder& builder, IRType* type, IRInst* val)
{
    ...
    else if (auto matrixType = as<IRMatrixType>(type))
    {
        ...
        return builder
            .emitMakeMatrix(type, (UInt)resultElements.getCount(), resultElements.getBuffer());
    }
    else if (auto vectorType = as<IRVectorType>(type))
    {
        auto count = as<IRIntLit>(vectorType->getElementCount());
        ...
        return builder
            .emitMakeVector(type, (UInt)resultElements.getCount(), resultElements.getBuffer());
    }
    else if (auto arrayType = as<IRArrayType>(type))
    {
        auto arraySize = as<IRIntLit>(arrayType->getElementCount());
        ...
        return builder
            .emitMakeArray(type, (UInt)resultElements.getCount(), resultElements.getBuffer());
    }
    else if (auto structType = as<IRStructType>(type))
    {
        List<IRInst*> resultElements;
        ...
        return builder
            .emitMakeStruct(type, (UInt)resultElements.getCount(), resultElements.getBuffer());
    }
    else if (as<IRTargetTupleType>(type))
    {
        return val;
...
IRType*
translateToHostType(IRBuilder* builder, IRType* type, IRInst* func, DiagnosticSink* sink = nullptr)
{
...
IRInst* castHostToCUDAType(IRBuilder* builder, IRType* hostType, IRType* cudaType, IRInst* inst)
{
    ...
    switch (cudaType->getOp())
    {
        ...
        case kIROp_StructType:
        {
            ...
            return builder
                ->emitMakeStruct(cudaType, (UInt)resultFields.getCount(), resultFields.getBuffer());
        }
        case kIROp_ArrayType:
        {
...
void generateReflectionFunc(IRBuilder* builder, IRFunc* kernelFunc, IRFunc* hostFunc)
{
    ...
    for (auto param : kernelFunc->getParams())
    {
        ...
        {
            paramTypeNames.add(
                builder->emitGetNativeString(builder->getStringValue(pyExportHint->getExportName()))
            );
        }
        else
        {
