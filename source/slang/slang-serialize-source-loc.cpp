...
SlangResult
SerialSourceLocReader::read(const SerialSourceLocData* serialData, SourceManager* sourceManager)
{
    m_views.setCount(0);
...
)
{
    ...
    for (RiffContainer::Chunk* chunk = listChunk->m_containedChunks; chunk; chunk = chunk->m_next)
    {
        ...
        switch (dataChunk->m_fourCC)
        {
            ...
            {
                SLANG_RETURN_ON_FAIL(
                    SerialRiffUtil::readArrayChunk(moduleCompressionType, dataChunk, m_sourceInfos)
                );
                break;
            }
        }
