...
static void
reportMacroInvocationForContentAssist(Preprocessor* preprocessor, MacroInvocation* invocation)
{
    if (!preprocessor->contentAssistInfo)
        return;
...
static PreprocessorExpressionValue
ParseAndEvaluateUnaryExpression(PreprocessorDirectiveContext* context)
{
        ...
        case TokenType::Identifier:
        {
            if (token.getContent() == "defined")
            {
                ...
                if (leftParen.type != TokenType::Unknown)
                {
                    ...
                    {
                        GetSink(context)
                            ->diagnose(leftParen.loc, Diagnostics::seeOpeningToken, leftParen);
                        return 0;
                    }
                }
...
static PreprocessorExpressionValue
_skipOrParseAndEvaluateExpression(PreprocessorDirectiveContext* context)
{
...
static SlangResult
readFile(PreprocessorDirectiveContext* context, String const& path, ISlangBlob** outBlob)
{
