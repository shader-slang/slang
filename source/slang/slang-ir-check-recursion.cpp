    ...
    HashSet<IRFunc*>& callStack)
{
    for (auto block : func->getBlocks())
    {
        for (auto inst : block->getChildren())
        {
            ...
            if (callStack.add(callee))
            {
                // Not yet in callstack, so this call is not yet known to be
                // recursive.
                collectRecursiveFunctions(callee, recursiveFuncs, callStack);
                callStack.remove(callee);
            }
            else
            {
                // Our callee is already in the callstack, so that callee is
                // at least known to be recursive.
                recursiveFuncs.add(callee);
            }
        }
    }
}
...
void collectRecursiveFunctions(IRModule* module, HashSet<IRFunc*>& recursiveFuncs)
{
    for (auto globalInst : module->getGlobalInsts())
    {
        if (IRFunc* func = as<IRFunc>(globalInst))
        {
            HashSet<IRFunc*> callStack;
            callStack.add(func);
            if (recursiveFuncs.contains(func))
                continue;
            collectRecursiveFunctions(func, recursiveFuncs, callStack);
        }
    }
}
