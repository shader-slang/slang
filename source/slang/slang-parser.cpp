...
static RefPtr<Declarator>
parseDirectAbstractDeclarator(Parser* parser, DeclaratorParseOptions options)
{
...
static void
UnwrapDeclarator(ASTBuilder* astBuilder, RefPtr<Declarator> declarator, DeclaratorInfo* ioInfo)
{
    while (declarator)
    {
...
static Expr* parseGenericApp(Parser* parser, Expr* base)
{
    ...
    if (parser->tokenReader.peekToken().type == TokenType::OpRsh)
    {
        ...
        parser->sink
            ->diagnose(parser->tokenReader.peekToken(), Diagnostics::tokenTypeExpected, "'>'");
    return genericApp;
}
...
///
static Expr*
_applyTypeModifiersToTypeExpr(Parser* parser, Expr* typeExpr, Modifiers& ioBaseModifiers)
{
...
static TypeSpec
_applyModifiersToTypeSpec(Parser* parser, TypeSpec typeSpec, Modifiers const& inModifiers)
{
...
static DeclBase*
ParseDeclaratorDecl(Parser* parser, ContainerDecl* containerDecl, Modifiers const& inModifiers)
{
...
///
static void
parseHLSLRegisterNameAndOptionalComponentMask(Parser* parser, HLSLLayoutSemantic* semantic)
{
...
static RayPayloadAccessSemantic*
_parseRayPayloadAccessSemantic(Parser* parser, RayPayloadAccessSemantic* semantic)
{
...
static NodeBase* parseRequireCapabilityDecl(Parser* parser, void*)
{
    ...
    while (parser->LookAheadToken(TokenType::Identifier))
    {
        ...
        if (capName != CapabilityName::Invalid)
            ...
            parser->sink
                ->diagnose(capNameToken, Diagnostics::unknownCapability, capNameToken.getContent());
...
static void
CompleteDecl(Parser* parser, Decl* decl, ContainerDecl* containerDecl, Modifiers modifiers)
{
...
static DeclBase*
ParseDeclWithModifiers(Parser* parser, ContainerDecl* containerDecl, Modifiers modifiers)
{
...
static FloatFixKind
_fixFloatLiteralValue(BaseType type, IRFloatingPointValue value, IRFloatingPointValue& outValue)
{
...
static IntegerLiteralValue
_fixIntegerLiteral(BaseType baseType, IntegerLiteralValue value, Token* token, DiagnosticSink* sink)
{
    // TODO(JS):
    ...
    switch (peekTokenType(parser))
    {
        ...
        case TokenType::FloatingPointLiteral:
        {
            ...
            if (suffixCursor < suffixEnd)
            {
                ...
                {
                    parser->sink
                        ->diagnose(token, Diagnostics::invalidFloatingPointLiteralSuffix, suffix);
...
static SlangResult
parseSemanticVersion(Parser* parser, Token& outToken, SemanticVersion& outVersion)
{
    parser->ReadToken(TokenType::LParent);
    outToken = parser->ReadToken();
    parser->ReadToken(TokenType::RParent);
...
#undef CASE
            ...
            else if (auto glslOffset = as<GLSLOffsetLayoutAttribute>(modifier))
            {
                if (auto binding = listBuilder.find<GLSLBindingAttribute>())
                {
                    ...
                    parser->ReadToken(TokenType::OpAssign);
                    glslOffset->offset =
                        int64_t(getIntegerLiteralValue(parser->ReadToken(TokenType::IntegerLiteral))
                        );
...
static NodeBase* parseBuiltinRequirementModifier(Parser* parser, void* /*userData*/)
{
    ...
    parser->ReadToken(TokenType::LParent);
    modifier->kind =
        BuiltinRequirementKind(stringToInt(parser->ReadToken(TokenType::IntegerLiteral).getContent()
        ));
    parser->ReadToken(TokenType::RParent);
...
static SyntaxParseInfo
_makeParseModifier(const char* keywordName, const ReflectClassInfo& classInfo)
{
