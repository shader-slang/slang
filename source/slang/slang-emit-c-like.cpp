...
{
    if (auto requirePreludeDecor =
            as<IRRequirePreludeDecoration>(findBestTargetDecoration<IRRequirePreludeDecoration>(type
            )))
    {
...
/* static */ IRWaveSizeDecoration*
CLikeSourceEmitter::getComputeWaveSize(IRFunc* func, Int* outWaveSize)
{
...
List<IRWitnessTableEntry*>
CLikeSourceEmitter::getSortedWitnessTableEntries(IRWitnessTable* witnessTable)
{
            ...
            else
            {
                if (inst->getOp() == kIROp_GetElementPtr && doesTargetSupportPtrTypes())
                {
                    ...
                    emitDereferenceOperand(
                        inst->getOperand(0),
                        leftSide(rightSidePrec, postfixInfo)
                    );
                    m_writer->emit("[");
                    emitOperand(inst->getOperand(1), getInfo(EmitOp::General));
                    m_writer->emit("]");
...
void CLikeSourceEmitter::emitClass(IRClassType* classType)
{
    ...
    if (comWitnessTables.getCount())
    {
        ...
        m_writer->indent();
        m_writer->emit(
            "if (uuid == ISlangUnknown::getTypeGuid()) return static_cast<ISlangUnknown*>(this);\n"
        );
        for (auto wt : comWitnessTables)
        {
