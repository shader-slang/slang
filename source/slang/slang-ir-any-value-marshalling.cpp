...
struct AnyValueMarshallingContext
{
    ...
    struct TypeMarshallingContext
    {
        ...
        virtual void
        marshalBasicType(IRBuilder* builder, IRType* dataType, IRInst* concreteTypedVar) = 0;
        // Defines what to do with resource handle elements.
        virtual void
        marshalResourceHandle(IRBuilder* builder, IRType* dataType, IRInst* concreteTypedVar) = 0;
    ...
    {
        virtual void
        marshalBasicType(IRBuilder* builder, IRType* dataType, IRInst* concreteVar) override
        {
            switch (dataType->getOp())
            {
                case kIROp_IntType:
                ...
                case kIROp_BoolType:
                {
                    ...
                    if (fieldOffset < static_cast<uint32_t>(anyValInfo->fieldKeys.getCount()))
                    {
                        ...
                        auto dstVal =
                            builder
                                ->emitIntrinsicInst(builder->getUIntType(), kIROp_Select, 3, args);
                        auto dstAddr = builder->emitFieldAddress(
                            uintPtrType,
                            anyValueVar,
                            anyValInfo->fieldKeys[fieldOffset]
        ...
        virtual void
        marshalResourceHandle(IRBuilder* builder, IRType* dataType, IRInst* concreteVar) override
        {
