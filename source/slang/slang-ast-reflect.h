...
// Implementation for SLANG_ABSTRACT_CLASS(x) using reflection from C++ extractor in
// slang-ast-generated.h
...
public:                                                                              \
    ...
    SLANG_FORCE_INLINE static bool isDerivedFrom(ASTNodeType type)                   \
    {                                                                                \
        return int(type) >= int(kType) && int(type) <= int(ASTNodeType::LAST);       \
    }                                                                                \
    SLANG_CLASS_REFLECT_SUPER_##TYPE(SUPER) friend class ASTBuilder;                 \
...
#define SLANG_AST_NODE_VIRTUAL_CALL(base, methodName, methodParams)                       \
    switch (astNodeType)                                                                  \
    {                                                                                     \
        SLANG_ALL_ASTNode_##base(SLANG_AST_NODE_CASE, (methodName, methodParams)) default \
            : return SLANG_AST_NODE_INVOKE(methodName, methodParams);                     \
    }
...
#define SLANG_AST_NODE_CONST_VIRTUAL_CALL(base, methodName, methodParams)                       \
    switch (astNodeType)                                                                        \
    {                                                                                           \
        SLANG_ALL_ASTNode_##base(SLANG_AST_NODE_CONST_CASE, (methodName, methodParams)) default \
            : return SLANG_AST_NODE_INVOKE(methodName, methodParams);                           \
    }
