...
void ASTPrinter::addExpr(Expr* expr)
{
    ...
    else if (const auto invokeExpr = as<InvokeExpr>(expr))
    {
        if (const auto operatorExpr = as<OperatorExpr>(invokeExpr))
        {
            ...
            else if (as<PrefixExpr>(operatorExpr))
            {
                // Prefix operator
                if (operatorExpr->functionExpr && as<VarExpr>(operatorExpr->functionExpr) &&
                    as<VarExpr>(operatorExpr->functionExpr)->name)
            ...
            else if (as<PostfixExpr>(operatorExpr))
            {
                // Postfix operator
                if (invokeExpr->arguments.getCount() > 0)
                {
            ...
            else if (as<SelectExpr>(operatorExpr))
            {
    ...
    {
        if (as<PrimalSubstituteExpr>(higherOrderInvokeExpr))
        {
            sb << "__primal(";
        }
        else if (as<ForwardDifferentiateExpr>(higherOrderInvokeExpr))
        {
            sb << "__fwd_diff(";
        }
        else if (as<BackwardDifferentiateExpr>(higherOrderInvokeExpr))
        {
            sb << "__bwd_diff(";
        }
        else if (as<DispatchKernelExpr>(higherOrderInvokeExpr))
        {
    ...
    else if (as<DefaultConstructExpr>(expr))
    {
        sb << "default(";
        addType(expr->type);
        sb << ")";
