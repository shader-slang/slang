...
static bool
isBlockInRegion(IRDominatorTree* domTree, IRTerminatorInst* regionHeader, IRBlock* block)
{
...
static bool
arePhiArgsEquivalentInBranchesImpl(IRBlock* branch1, IRBlock* branch2, IRBlock* afterBlock)
{
...
static bool
isTrivialIfElse(IRIfElse* condBranch, bool& isTrueBranchTrivial, bool& isFalseBranchTrivial)
{
...
static bool trySimplifyIfElse(IRBuilder& builder, IRIfElse* ifElseInst)
{
    ...
    else
    {
        ...
        if (!afterBlock->getFirstParam())
        {
            ...
            else if (isFalseBranchTrivial && falseBlock != afterBlock &&
...
static bool
simplifyBoolPhiParam(IRIfElse* ifElse, Array<IRBlock*, 2>& preds, IRParam* param, UInt paramIndex)
{
