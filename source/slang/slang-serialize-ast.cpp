...
namespace Slang
{
    ...
    public:
        ...
        void encodeASTNodeContent(T* node)
        {
            ...
            ASTNodeDispatcher<T, void>::dispatch(node, [&](auto n)
            {
                _encodeDataOf(n);
            });
        }
        ...
        void encodePtr(CandidateExtensionList* list)
        {
            encode(list->candidateExtensions);
        }
    ...
    void encodeValue(String const& value)
    {
    ...
    void encodeValue(NameLoc const& value)
    {
        encode(value.name);
    }

    void encodeValue(SemanticVersion value)
    {
        encoder->encode(value.toInteger());
    }
    ...
    void encodeValue(SyntaxClass<NodeBase> const& value)
    {
        encode(value.getTag());
    }
    ...
    void encodeValue(TypeExp value)
    {
        encode(value.type);
    }
    ...
    void encodeValue(SPIRVAsmOperand::Flavor const& value)
    {
        encodeEnum(value);
    }
    ...
    void encodeValue(T value) { encoder->encodeBool(value); }
    ...
    void encodeValue(uint8_t value)
    {
        encoder->encode(UInt32(value));
    }

    void encodeValue(nullptr_t)
    {
        encoder->encode(nullptr);
    }
...
#include "slang-serialize-ast.cpp.fiddle"
    ...
    private:
        ...
        SlangResult decodeImportedDecls(RiffContainer::Chunk* importedDeclChunk)
        {
            ...
            while (decoder.hasElements())
            {
                ...
                else
                {
                    ...
                    auto importedNode = importedFromModule->findExportFromMangledName(mangledName.getUnownedSlice());
        ...
        {
            Module* module = _linkage->findOrImportModule(
                moduleName,
                _requestingSourceLoc,
                _sink);
            if (!module)
            {
        ...
        void decodeASTNodeContent(
            NodeBase* node,
            Decoder& decoder)
        {
            Decoder::WithObject withObject(decoder);

            ASTNodeDispatcher<NodeBase, void>::dispatch(node, [&](auto n)
            {
                _decodeDataOf(n, decoder);
            });
        ...
        void decodeValue(String& value, Decoder& decoder)
        {
        ...
        void decodeValue(NameLoc& value, Decoder& decoder)
        {
            decode(value.name, decoder);
        }
        ...
        void decodeValue(TypeExp& value, Decoder& decoder)
        {
            decode(value.type, decoder);
        }
        ...
        void decodeValue(BuiltinRequirementKind& value, Decoder& decoder) { decodeEnum(value, decoder); }
        ...
        void decodeValue(PreferRecomputeAttribute::SideEffectBehavior& value, Decoder& decoder) { decodeEnum(value, decoder); }
        void decodeValue(LogicOperatorShortCircuitExpr::Flavor& value, Decoder& decoder) { decodeEnum(value, decoder); }
        void decodeValue(TreatAsDifferentiableExpr::Flavor& value, Decoder& decoder) { decodeEnum(value, decoder); }
...
#include "slang-serialize-ast.cpp.fiddle"
    ...
    ModuleDecl* readSerializedModuleAST(
        ...
        SourceLoc requestingSourceLoc)
    {
        ASTDecodingContext context(linkage, astBuilder, sink, chunk, sourceLocReader, requestingSourceLoc);
        context.decodeAll();
