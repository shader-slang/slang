...
struct ReproUtil
{
    ...
    static SlangResult
    loadState(const String& filename, DiagnosticSink* sink, List<uint8_t>& outBuffer);
    static SlangResult loadState(Stream* stream, DiagnosticSink* sink, List<uint8_t>& outBuffer);
    static SlangResult
    loadState(const uint8_t* data, size_t size, DiagnosticSink* sink, List<uint8_t>& outBuffer);
    ...
    static SlangResult
    extractFiles(OffsetBase& base, RequestState* requestState, ISlangMutableFileSystem* fileSystem);
