...
static void
addOrAppendToNodeList(ShortList<ValNodeOperand, N>& list, ExpandedSpecializationArgs e, Ts... ts)
{
    for (auto arg : e)
    {
...
static void
addOrAppendToNodeList(List<ValNodeOperand>& list, ExpandedSpecializationArgs e, Ts... ts)
{
    for (auto arg : e)
    {
        list.add(ValNodeOperand(arg.val));
...
class Val : public NodeBase
{
    ...
    // Private use by stdlib deserialization only. Since we know the Vals serialized into stdlib is
    // already unique, we can just use `this` pointer as the `m_resolvedVal` so we don't need to
    // resolve them again.
    void _setUnique();
...
template<int N, typename T, typename... Ts>
static void
addOrAppendToNodeList(ShortList<ValNodeOperand, N>& list, Val::OperandView<T> l, Ts... ts)
{
    for (auto t : l)
        list.add(ValNodeOperand(t));
    addOrAppendToNodeList(list, ts...);
