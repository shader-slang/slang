...
struct FunctionParameterSpecializationContext
{
    ...
    void getCallInfoForArg(CallSpecializationInfo& ioInfo, IRInst* oldArg)
    {
        ...
        {
            // Heap-EXT analogue of CastDescriptorHandleToResource. The `heap`
            // operand is a hoistable uint-typed global
            // (kIROp_SPIRVResourceHeap / kIROp_SPIRVSamplerHeap, both
            // `hoistable = true` in slang-ir-insts.lua) emitted as an
            // UniformConstant pointer by emitDescriptorHeapBuiltinVar and used
            // directly as the OpUntypedAccessChainKHR base — parameterizing it
    ...
    IRInst* getSpecializedValueForArg(FuncSpecializationInfo& ioInfo, IRInst* oldArg)
    {
        ...
        {
            // Rebuild the load inside the cloned callee from a fresh `index`
            // param, reusing the heap global directly — see the matching
            // branch in `getCallInfoForArg` for why the heap must not be
            // parameterized. Materializing the load in-block also keeps the
            // texture out of the orphan-IRParam path that broke SPIR-V emit in
            // shader-slang/slang#11498.
            auto builder = getBuilder();
