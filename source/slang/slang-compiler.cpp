...
RefPtr<EntryPoint>
EntryPoint::create(Linkage* linkage, DeclRef<FuncDecl> funcDeclRef, Profile profile)
{
...
RefPtr<EntryPoint>
EntryPoint::createDummyForPassThrough(Linkage* linkage, Name* name, Profile profile)
{
...
getPassThroughTranslationUnit(EndToEndCompileRequest* endToEndReq, Int entryPointIndex)
{
...
static bool
_useEmittedSource(IDownstreamCompiler* compiler, TranslationUnitRequest* translationUnit)
{
...
SlangResult CodeGenContext::emitWithDownstreamForEntryPoints(ComPtr<IArtifact>& outArtifact)
{
    ...
    if (auto endToEndReq = isPassThroughEnabled())
    {
        ...
        sourceTarget = CodeGenTarget(
            TypeConvertUtil::getCompileTargetFromSourceLanguage((SlangSourceLanguage)sourceLanguage)
        );
    ...
    else
    {
        ...
        )TypeConvertUtil::getSourceLanguageFromTarget((SlangCompileTarget)sourceTarget);
    }
                ...
                switch (stage)
                {
                    ...
                    case Stage::Fragment:
                        options.pipelineType =
                            DownstreamCompileOptions::PipelineType::Rasterization;
                        break;
...
SlangResult
emitSPIRVForEntryPointsDirectly(CodeGenContext* codeGenContext, ComPtr<IArtifact>& outArtifact);
...
IArtifact*
TargetProgram::_createWholeProgramResult(DiagnosticSink* sink, EndToEndCompileRequest* endToEndReq)
{
...
SlangResult EndToEndCompileRequest::writeContainerToStream(Stream* stream)
{
    ...
    options.compressionType =
        linkage->m_optionSet.getEnumOption<SerialCompressionType>(CompilerOptionName::IrCompression
        );
