                                        ...
                                        public DeclVisitor<SemanticsDeclAttributesVisitor>
{
    ...
    void
    checkPrimalSubstituteOfAttribute(FunctionDeclBase* funcDecl, PrimalSubstituteOfAttribute* attr);
};
                                    ...
                                    public DeclVisitor<SemanticsDeclHeaderVisitor>
{
    ...
    void
    checkDerivativeMemberAttributeParent(VarDeclBase* varDecl, DerivativeMemberAttribute* attr);
...
///
static void
_dispatchDeclCheckingVisitor(Decl* decl, DeclCheckState state, SemanticsContext& shared);
        ...
        if (auto basicType = as<BasicExpressionType>(varDecl->getType()))
        {
            switch (basicType->getBaseType())
            {
                ...
                default:
                    getSink()->diagnose(
                        varDecl,
                        Diagnostics::staticConstRequirementMustBeIntOrBool
                    );
                    break;
            }
        }
...
_createCtor(SemanticsDeclVisitorBase* visitor, ASTBuilder* m_astBuilder, AggTypeDecl* decl)
{
...
)
{
    ...
    if (context->parentDecl->getMemberDictionary()
            .tryGetValue(requirementDeclRef.getName(), existingDecl))
    {
      ...
      public DeclVisitor<SemanticsDeclDifferentialConformanceVisitor>
{
    ...
    void visitInheritanceDecl(InheritanceDecl* inheritanceDecl)
    {
        ...
        if (!inheritanceDecl->witnessTable->getRequirementDictionary()
                 .tryGetValue(requirementDecl, witnessValue))
            return;
...
)
{
    ...
    for (Index i = 0; i < memberCount; i++)
    {
        ...
        else if (auto requiredConstraintDeclRef =
    ...
    return doesMemberSatisfyRequirement(
        m_astBuilder
            ->getMemberDeclRef(satisfyingGenericDeclRef, getInner(satisfyingGenericDeclRef)),
...
{
    CallableDecl* synFuncDecl =
        as<CallableDecl>(m_astBuilder->createByNodeType(requiredMemberDeclRef.getDecl()->astNodeType
        ));
...
)
{
    ...
    for (auto mm : genericDecl->members)
    {
        ...
        {
            args.add(
                DeclRefType::create(astBuilder, astBuilder->getDirectDeclRef(genericTypeParamDecl))
            );
        }
    ...
    // doesn't try and define what is already defined.
    {
        ...
        for (auto& [target, value] : newTargets)
        {
            ...
            if (found)
            {
                ...
                {
                    getSink()
                        ->diagnose(newDecl, Diagnostics::functionRedefinition, newDecl->getName());
                    hasConflict = true;
                }

                auto prevDecl = *found;
                getSink()
                    ->diagnose(prevDecl, Diagnostics::seePreviousDefinitionOf, prevDecl->getName());
            }
        }
...
void SemanticsDeclBasesVisitor::_validateExtensionDeclTargetType(ExtensionDecl* decl)
{
    if (auto targetDeclRefType = as<DeclRefType>(decl->targetType))
    {
        ...
        else if (auto genericTypeParamDecl =
                     targetDeclRefType->getDeclRef().as<GenericTypeParamDecl>())
        {
...
error:;
    ...
    {
        getSink()
            ->diagnose(decl->targetType.exp, Diagnostics::invalidExtensionOnType, decl->targetType);
    }
}
...
void SemanticsDeclHeaderVisitor::visitIncludeDecl(IncludeDecl* decl)
{
    ...
    getSink()
        ->diagnose(decl->moduleNameAndLoc.loc, Diagnostics::includedFileMissingImplementing, name);
}
...
SharedSemanticsContext::getCandidateExtensionsForTypeDecl(AggTypeDecl* decl)
{
...
SharedSemanticsContext::getFuncDifferentiableLevel(FunctionDeclBase* func)
{
    return _getFuncDifferentiableLevelImpl(func, 1);
}

FunctionDifferentiableLevel
SharedSemanticsContext::_getFuncDifferentiableLevelImpl(FunctionDeclBase* func, int recurseLimit)
{
    if (!func)
...
getCanonicalGenericConstraints(ASTBuilder* astBuilder, DeclRef<ContainerDecl> genericDecl)
{
...
error:;
    ...
    visitor->getSink()
        ->diagnose(attr, Diagnostics::customDerivativeSignatureMismatch, builder.produceString());
}
...
ArgsWithDirectionInfo
getImaginaryArgsToFunc(ASTBuilder* astBuilder, FunctionDeclBase* func, SourceLoc loc)
{
...
static void
checkCudaKernelAttribute(SemanticsVisitor* visitor, FunctionDeclBase* funcDecl, CudaKernelAttribute*)
{
        ...
        for (Index targetCaseIndex = 0; targetCaseIndex < targetCaseCount; targetCaseIndex++)
        {
                ...
                CapabilityName::Invalid)
            {
                ...
                if (!maybeRequireCapability)
                    ...
                    targetCap =
                        (maybeRequireCapability->capabilitySet.getTargetsThisHasButOtherDoesNot(set)
                        );
            }
            else
            {
...
{
    CapabilityDeclReferenceVisitor<ProcessFunc, ParentDiagnosticFunc>
        visitor(processFunc, parentDiagnosticFunc, maybeRequireCapability, context);
