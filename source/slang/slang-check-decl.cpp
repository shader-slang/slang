...
#undef CASE
        ...
        else
        {
            ...
            if(isUnsizedArrayType(varDecl->type))
            {
                if (auto initExpr = varDecl->initExpr)
                {
                    ...
                    initExpr = coerce(CoercionContext::Initializer, varDecl->type.Ptr(), initExpr);
                    varDecl->initExpr = initExpr;
        ...
        RefPtr<WitnessTable>        witnessTable)
    {
        ...
        // the expected type.
        //
        auto coercedCall = subVisitor.coerce(CoercionContext::Return, resultType, checkedCall);
        ...
        RefPtr<WitnessTable>        witnessTable)
    {
        ...
        for( auto requiredAccessorDeclRef : getMembersOfType<AccessorDecl>(requiredMemberDeclRef) )
        {
            ...
            if( as<GetterDecl>(requiredAccessorDeclRef) )
            {
                ...
                //
                auto coercedMemberRef = subVisitor.coerce(CoercionContext::Return, propertyType, synMemberRef);
                auto synReturn = m_astBuilder->create<ReturnStmt>();
    ...
    void SemanticsDeclBodyVisitor::visitEnumCaseDecl(EnumCaseDecl* decl)
    {
        ...
        {
            initExpr = CheckTerm(initExpr);
            initExpr = coerce(CoercionContext::General, tagType, initExpr);
    ...
    void SemanticsDeclBodyVisitor::visitParamDecl(ParamDecl* paramDecl)
    {
        ...
        if(auto initExpr = paramDecl->initExpr)
        {
            ...
            //
            initExpr = CheckTerm(initExpr);
            initExpr = coerce(CoercionContext::Initializer, typeExpr.type, initExpr);
