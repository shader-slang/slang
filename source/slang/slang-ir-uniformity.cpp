...
struct ValidateUniformityContext
{
    ...
    FunctionNonUniformInfo*
    getFunctionNonUniformInfo(IRCall* callInst, const FunctionNonUniformInfoKey& key)
    {
                    ...
                    switch (user->getOp())
                    {
                        ...
                        case kIROp_Call:
                        {
                            ...
                            bool isNonUniformCall =
                                callee->findDecorationImpl(kIROp_NonDynamicUniformReturnDecoration
                                ) != nullptr;
                            if (!isNonUniformCall)
                            {
                                for (UInt argi = 0; argi < callInst->getArgCount(); argi++)
