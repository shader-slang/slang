...
static LayoutSemanticInfo
_extractLayoutSemanticInfo(ParameterBindingContext* context, HLSLLayoutSemantic* semantic)
{
...
)
{
    ...
    else if (auto foundSubElementRegisterSpace =
    ...
    else if (auto foundSpecializationConstant =
...
// Generate the binding information for a shader parameter.
static void
completeBindingsForParameter(ParameterBindingContext* context, RefPtr<ParameterInfo> parameterInfo)
{
    auto varLayout = parameterInfo->varLayout;
...
static void
completeBindingsForParameter(ParameterBindingContext* context, RefPtr<VarLayout> varLayout)
{
    ParameterBindingInfo bindingInfos[kLayoutResourceKindCount];
...
)
{
    ...
    if (isKhronosTarget(context->getTargetRequest()))
    {
        ...
        {
            getSink(context)
                ->diagnose(indexAttr, Diagnostics::vkIndexWithoutVkLocation, decl->getName());
        }
    }
...
RefPtr<ProgramLayout> generateParameterBindings(TargetProgram* targetProgram, DiagnosticSink* sink)
{
    ...
    SharedParameterBindingContext
        sharedContext(layoutContext.getRulesFamily(), programLayout, targetProgram, sink);
    ...
    if (isD3DTarget(targetReq))
    {
        ...
        for (auto nvapiSlotModifier : sharedContext.nvapiSlotModifiers)
        {
            ...
            usedRangeSet->usedResourceRanges[(int)kind]
                .Add(nullptr, info.index, info.index + count);
        }
    }
