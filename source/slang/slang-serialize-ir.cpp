...
)
{
    ...
    for (RiffContainer::Chunk* chunk = module->m_containedChunks; chunk; chunk = chunk->m_next)
    {
        ...
        switch (dataChunk->m_fourCC)
        {
            ...
            {
                SLANG_RETURN_ON_FAIL(
                    SerialRiffUtil::readArrayUncompressedChunk(dataChunk, outData->m_rawSourceLocs)
                );
                break;
            }
            ...
            switch (op)
            {
                case kIROp_BoolLit:
                {
                    ...
                    irConst = static_cast<IRConstant*>(
                        module->_allocateInst(op, operandCount, prefixSize + sizeof(IRIntegerValue))
                    );
                ...
                case kIROp_VoidLit:
                {
                    SLANG_ASSERT(srcInst.m_payloadType == PayloadType::Empty);
                    irConst =
                        static_cast<IRConstant*>(module->_allocateInst(op, operandCount, prefixSize)
                        );
                    break;
                }
                case kIROp_BlobLit:
