...
struct DebugValueStoreContext
{
    ...
    void insertDebugValueStore(IRFunc* func)
    {
        ...
        auto setDebugValue = [&](IRInst* debugVar,
                                 IRInst* rootVar,
                                 IRInst* newValue,
                                 ArrayView<IRInst*> accessChain,
                                 ArrayView<IRInst*> types)
        {
            ...
            // Otherwise we need to load the entire composite value starting at the dynamic index
            // access chain and set it.
            auto compositePtr =
                builder.emitElementAddress(rootVar, accessChain.head(i), types.head(i));
