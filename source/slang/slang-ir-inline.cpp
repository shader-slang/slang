...
struct InliningPassBase
{
    ...
    //
    // Therefore we probably *don't* want to copy sourceLoc from the original definition in the
    // stdlib because
    //
    // * That won't be much use to the user (they can't easily see stdlib code currently for
    // example)
    // * That the definitions in stdlib are currently 'mundane' and largely exist to flesh out
    // language features - such that
    ...
    //
    // We *could* vary behavior if we knew if the function was defined in the stdlib. There doesn't
    // appear to be a decoration for this. We could find out by looking at the source loc and
    // checking if it's in the range of stdlib - this would actually be a fast and easy but to do
    // properly this way you'd want a way to mark that source range that would also work across
    // serialization.
    //
    ...
    ///
    void
    inlineSingleBlockFuncBody(CallSiteInfo const& callSite, IRCloneEnv* env, IRBuilder* builder)
    {
