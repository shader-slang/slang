...
SlangResult
SourceView::_findSourceMapLoc(SourceLoc loc, SourceLocType type, HandleSourceLoc& outLoc)
{
...
void SourceFile::setContents(ISlangBlob* blob)
{
    ...
    CharEncoding::getEncoding(type)
        ->decode(rawContentBegin + offset, int(rawContentSize - offset), decodedBuffer);
...
SourceFile*
SourceManager::createSourceFileWithString(const PathInfo& pathInfo, const String& contents)
{
