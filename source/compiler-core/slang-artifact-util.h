...
struct ArtifactUtil
{
    ...
    static IArtifact*
    findArtifactByDerivedDesc(IArtifact* artifact, FindStyle findStyle, const ArtifactDesc& desc);
    /// Find an artifact that predicate matches
    static IArtifact*
    findArtifactByPredicate(IArtifact* artifact, FindStyle findStyle, FindFunc func, void* data);
    /// Find by name
    static IArtifact*
    findArtifactByName(IArtifact* artifact, FindStyle findStyle, const char* name);
    /// Find by desc exactly
    static IArtifact*
    findArtifactByDesc(IArtifact* artifact, FindStyle findStyle, const ArtifactDesc& desc);
    ...
    static SlangResult
    calcPath(IArtifact* artifact, const UnownedStringSlice& basePath, StringBuilder& outPath);

    /// Given a desc and a baseName works out the the output file name
    static SlangResult
    calcName(IArtifact* artifact, const UnownedStringSlice& baseName, StringBuilder& outName);
