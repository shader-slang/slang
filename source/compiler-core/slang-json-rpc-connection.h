...
public:
    ...
    SlangResult
    initWithStdStreams(CallStyle callStyle = CallStyle::Default, Process* process = nullptr);
    ...
    SlangResult
    sendError(JSONRPC::ErrorCode errorCode, const UnownedStringSlice& msg, const JSONValue& id);
    ...
    template<typename T>
    SlangResult
    sendCall(const UnownedStringSlice& method, const T* args, const JSONValue& id = JSONValue())
    {
...
template<typename T>
SlangResult
JSONRPCConnection::toValidNativeOrSendError(const JSONValue& value, T* data, const JSONValue& id)
{
    ...
    if (!data->isValid())
    {
        ...
        if (rttiInfo->isNamed())
        {
            ...
            m_diagnosticSink
                .diagnose(SourceLoc(), JSONDiagnostics::argsAreInvalid, namedRttiInfo->m_name);
        }
