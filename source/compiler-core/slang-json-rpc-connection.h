...
{
public:
    /// Why the most recent read did not produce a JSON-RPC message.
    ///
    /// This is separate from SlangResult because several layers historically returned the
    /// same generic failure for a timeout, an ended stream, and malformed input. RPC clients
    /// need the distinction to decide whether starting a fresh peer can help.
    ...
    SlangResult sendNullResult(const JSONValue& id);
    ...
    /// Get the structured cause of the most recent failed message read.
...
protected:
    void _updateReadErrorFromTransport();
    ...
    ReadError m_readError = ReadError::None;
