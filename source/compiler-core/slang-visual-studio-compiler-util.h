...
struct VisualStudioCompilerUtil : public DownstreamCompilerBaseUtil
{
    ...
    static SlangResult parseOutput(const ExecuteResult& exeRes, IDiagnostics* outOutput);
...
public:
    ...
    virtual SlangResult parseOutput(const ExecuteResult& exeResult, IDiagnostics* diagnostics) SLANG_OVERRIDE { return Util::parseOutput(exeResult, diagnostics); }
