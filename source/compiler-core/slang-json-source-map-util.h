...
struct JSONSourceMapUtil
{
    ...
    static SlangResult
    decode(JSONContainer* container, JSONValue root, DiagnosticSink* sink, SourceMap& out);
    ...
    static SlangResult
    write(const SourceMap& sourceMap, DiagnosticSink* sink, ComPtr<ISlangBlob>& outBlob);
};
