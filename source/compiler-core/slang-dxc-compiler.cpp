...
SlangResult DXCDownstreamCompiler::init(ISlangSharedLibrary* library)
{
    ...
    SLANG_RETURN_ON_FAIL_HRESULT(m_createInstance(CLSID_DxcCompiler, __uuidof(dxcCompiler), (LPVOID*)dxcCompiler.writeRef()));
...
static SlangResult _handleOperationResult(IDxcOperationResult* dxcResult, IArtifactDiagnostics* diagnostics, ComPtr<IDxcBlob>& outBlob)
{
    ...
    SLANG_RETURN_ON_FAIL_HRESULT(dxcResult->GetStatus(&resultCode));
    ...
    else
    {
        ...
        SLANG_RETURN_ON_FAIL_HRESULT(dxcResult->GetResult(outBlob.writeRef()));
    }
...
SlangResult DXCDownstreamCompiler::compile(const CompileOptions& inOptions, IArtifact** outArtifact)
{
    ...
    SLANG_RETURN_ON_FAIL_HRESULT(m_createInstance(CLSID_DxcLibrary, __uuidof(dxcLibrary), (LPVOID*)dxcLibrary.writeRef()));
    ...
    SLANG_RETURN_ON_FAIL_HRESULT(dxcLibrary->CreateBlobWithEncodingFromPinned(
    ...
    SLANG_RETURN_ON_FAIL_HRESULT(dxcCompiler->Compile(dxcSourceBlob,
    ...
    if (libraries.getCount())
    {
        ...
        SLANG_RETURN_ON_FAIL_HRESULT(m_createInstance(CLSID_DxcLinker, __uuidof(linker), (void**)linker.writeRef()));
        ...
        for (Index i = 0; i < librariesCount; ++i)
        {
            ...
            SLANG_RETURN_ON_FAIL_HRESULT(linker->RegisterLibrary(linkLibraryNames[i], (IDxcBlob*)libraryBlobs[i].get()));
        }
        ...
        SLANG_RETURN_ON_FAIL_HRESULT(linker->Link(wideEntryPointName.begin(), wideProfileName.begin(), linkLibraryNames.getBuffer(), UINT32(librariesCount), nullptr, 0, linkDxcResult.writeRef()));
...
SlangResult DXCDownstreamCompiler::convert(IArtifact* from, const ArtifactDesc& to, IArtifact** outArtifact) 
{
    ...
    ComPtr<IDxcBlobEncoding> dxcSourceBlob;
    SLANG_RETURN_ON_FAIL_HRESULT(dxcLibrary->CreateBlobWithEncodingFromPinned((LPBYTE)dxilBlob->getBufferPointer(), (UINT32)dxilBlob->getBufferSize(), 0, dxcSourceBlob.writeRef()));

    ComPtr<IDxcBlobEncoding> dxcResultBlob;
    SLANG_RETURN_ON_FAIL_HRESULT(dxcCompiler->Disassemble(dxcSourceBlob, dxcResultBlob.writeRef()));
