...
SlangResult
ExtFileArtifactRepresentation::createRepresentation(const Guid& typeGuid, ICastable** outCastable)
{
...
SlangResult
OSFileArtifactRepresentation::createRepresentation(const Guid& typeGuid, ICastable** outCastable)
{
    // We can convert into a blob only, and only if we have a path
    // If it's referenced by a name only, it's a file that *can't* be loaded as a blob in general.
...
const char* OSFileArtifactRepresentation::getUniqueIdentity()
{
    if (m_uniqueIdentity.getLength() == 0)
    {
        ...
        ComPtr<ISlangBlob> uniqueIdentityBlob;
        if (SLANG_FAILED(
                fileSystem->getFileUniqueIdentity(m_path.getBuffer(), uniqueIdentityBlob.writeRef())
            ))
        {
            return nullptr;
        }
