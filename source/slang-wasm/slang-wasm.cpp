...
Module* Session::loadModuleFromSource(const std::string& slangCode)
{
    Slang::ComPtr<IModule> module;
    {
        ...
        catch(const Slang::AbortCompilationException& e)
        {
            // TODO: This currently happens when introducing a simple syntax error
            //       in the Slang code! Figure out why.
            g_error.type = std::string("INTERNAL");
            g_error.message = std::string(e.Message.begin(), e.Message.end());
            return nullptr;
        }
        if (!module)
        {
