...
def test_neural_frontend_training_converges(device_type: spy.DeviceType) -> None:
    ...
    try:
        ...
        initial_loss = float(module.eval_loss(params.device_address, xs_buf, ys_buf, sample_count))

        learning_rate = 0.1
        steps = 200
        for _ in range(steps):
            module.train_step(params.device_address, grads.device_address, xs_buf, ys_buf, sample_count, learning_rate)

        final_loss = float(module.eval_loss(params.device_address, xs_buf, ys_buf, sample_count))
...
def test_neural_multiworkgroup_atomicadd(device_type: spy.DeviceType) -> None:
    ...
    try:
        ...
        grad_kernel.dispatch(
            ...
            params_ptr=params.device_address,
            grads=grads,
            xs=xs_buf,
            ys=ys_buf,
            count=sample_count,
