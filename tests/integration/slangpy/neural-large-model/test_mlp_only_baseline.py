...
"""
SlangPy integration test: MLP-only image fitting baseline (no spatial encoding).

Matches tiny-cuda-nn's FullyFusedMLP test for direct comparison.
Architecture: (x,y) -> MLP 2->128->128->128->3 (Sigmoid) -> RGB

Uses the same slangpy dispatch pattern as the latent-grid MLP test:
...
class Network(spy.InstanceList):
    """MLP-only network — no latent grid, raw (x,y) input."""

    def __init__(self, module, device):
        super().__init__(module["Network"])
        np.random.seed(42)
        params_np = xavier_init([(2, 128), (128, 128), (128, 128), (128, 3)])
...
def _create_device_and_module(device_type=spy.DeviceType.cuda):
    """Create device and load the MLP-only Slang module."""
    ...
    module = spy.Module.load_from_file(device, str(TEST_DIR / "neural_mlp_only.slang"))
...
def test_training_convergence_mlp_only(device_type: spy.DeviceType) -> None:
    """Train MLP-only on GPU for 10K iterations and verify loss convergence."""
...
@pytest.mark.parametrize("device_type", [spy.DeviceType.cuda])
def test_inference_quality_mlp_only(device_type: spy.DeviceType) -> None:
    """Train MLP-only, render full image, and verify reconstruction quality."""
    device, module = _create_device_and_module(device_type)
    try:
        network = Network(module, device)
        image = spy.Tensor.load_from_image(device, str(REF_IMAGE_PATH), linearize=True)
        ...
        print("\nInference quality (256x256, MLP-only):")
        print(f"  MSE:  {mse:.6f}")
        ...
        # MLP-only has limited capacity — expect modest quality
        assert pass_30 >= 10.0, f"Too few pixels within 0.30: {pass_30:.1f}%"
        ...
        try:
            ...
            if out_img.ndim == 3 and out_img.shape[2] == 3:
                PILImage.fromarray(out_img).save(str(TEST_DIR / "mlp_only_output.png"))
                print(f"  Saved: {TEST_DIR / 'mlp_only_output.png'}")
...
@pytest.mark.parametrize("device_type", [spy.DeviceType.cuda])
def test_parameter_count_mlp_only(device_type: spy.DeviceType) -> None:
    """Verify FFLayer.ParameterCount matches expected values for 2->128->128->128->3."""
    device, module = _create_device_and_module(device_type)
    try:
        total_mlp = int(module.get_total_params())

        # Layer0: 2*128+128=384, Layer1: 128*128+128=16512, Layer2: same, Layer3: 128*3+3=387
        expected_mlp = 384 + 16512 + 16512 + 387  # = 33795

        assert total_mlp == expected_mlp, f"MLP: {total_mlp} != {expected_mlp}"
        print(f"\nParams: MLP={total_mlp}")
    finally:
        device.close()
