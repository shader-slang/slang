...
Result<void> Interpreter::begin(const std::string &entry_name)
{
    ...
    for (auto &[key, _] : image_staging_) {
        uint32_t s = key >> 16, b = key & 0xFFFF;
        for (auto &[id, var] : module_->variables) {
            // Images/samplers are UniformConstant. Filter on storage class,
            // same as the descriptor-binding loop above, so this can't match
            // an unrelated variable (e.g. a builtin) that happens to lack
            // DescriptorSet/Binding decorations and so defaults to (0, 0).
            if (var.storage_class != SpvStorageClassUniformConstant)
                continue;
            auto &dset = module_->decorations_of(id);
            if (dset.get(SpvDecorationDescriptorSet) == s && dset.get(SpvDecorationBinding) == b) {
        ...
        case SpvOpFunctionCall:
            {
                ...
                for (uint32_t i = 0; i < callee.parameters.size(); ++i) {
...
{
    for (auto &[id, var] : module_->variables) {
        // Only StorageBuffer/Uniform/UniformConstant variables are real
        // descriptors. Input-storage-class variables (e.g. builtins like
        // gl_GlobalInvocationID) carry no DescriptorSet/Binding decoration,
        // so DecorationSet::get() defaults both to 0 and would otherwise
        // spuriously match set=0, binding=0 — which of the two variables
        // "wins" then depends on the unordered_map's implementation-defined
        // iteration order. set_descriptor() already restricts its own match
        // to these storage classes; mirror that here so a lookup can never
        // land on a non-descriptor variable.
        if (var.storage_class != SpvStorageClassStorageBuffer
            && var.storage_class != SpvStorageClassUniform
            && var.storage_class != SpvStorageClassUniformConstant)
            continue;
