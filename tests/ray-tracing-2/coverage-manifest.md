...
| Vendor cluster and LSS intrinsics | `tests/{hlsl-intrinsic,cuda,language-feature}/**/*{cluster,lss}*.slang` | `compatibility/coexistence/vendor-geometry-intrinsics.slang` calls the existing intrinsics from a structural *ClosestHit* | D3D, Vulkan | Existing tests | **Coexists**: structural stage compilation is covered; hardware execution remains target-specific |
