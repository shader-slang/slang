...
"""
Single Layer Forward Pass Benchmark — WaveTangledVector + OptimalLayout

Uses WaveTangledVector.linearTransform<OptimalLayout> which on CUDA routes
through MMAHelperNew (m16n8k16, zero shared memory, warp shuffles only).
...
def find_neural_module_dir():
    ...
    for parent in [script_dir] + list(script_dir.parents):
        ...
        if build_dir.is_dir():
            # neural.slang-module lives at <prefix>/slang/neural.slang-module
            # We need <prefix> on the include path so "slang.neural" resolves.
            for candidate in sorted(build_dir.rglob("neural.slang-module")):
                # candidate.parent is the "slang/" dir; its parent is the include root
...
def run_benchmark(device_type_str: str, size: str, num_warps: int = 1, batch_size: int = DEFAULT_BATCH_SIZE):
    """Run single-layer forward benchmark via WaveTangledVector + OptimalLayout."""
    ...
    print(f"\n{'='*70}")
    print(f"Single Layer Forward (WaveTangledVector+OptimalLayout): {input_size} -> {output_size}, batch={batch_size}")
    ...
    print(f"  neural.slang-module at: {neural_module_dir / 'slang' / 'neural.slang-module'}")
    ...
    try:
        ...
        module = device.load_module("benchmark_single_layer_forward_new_mma.slang")

        # MMA kernel
        program = device.link_program(
            modules=[module],
            entry_points=[module.entry_point("compute_single_layer_forward_new_mma")]
        )
        ...
        # Tiled weights buffer (input to conversion)
        tiled_weights_buf = device.create_buffer(
            data=weights_tiled,
            usage=spy.BufferUsage.shader_resource,
        )

        # Native weights buffer (output of conversion) + bias appended
        native_params = np.zeros(tiled_weight_count + output_size, dtype=np.float16)
        native_params[tiled_weight_count:] = bias_data
        native_params_buf = device.create_buffer(
            data=native_params,
            usage=spy.BufferUsage.shader_resource | spy.BufferUsage.unordered_access,
        )

        # Run conversion kernel: one warp per tile
        ...
        # Output buffer: 1 checksum per thread (32 per sample)
        ...
        # CPU reference: y = W * x + b using input = [1, 2, ..., input_size]
        # Then compute checksum = sum(y) for thread 0
        W = weights_linear[:output_size * input_size].reshape(output_size, input_size).astype(np.float16)
        x0 = np.arange(1, input_size + 1, dtype=np.float16)
        b = bias_data.astype(np.float16)
        cpu_output = (W.astype(np.float32) @ x0.astype(np.float32) + b.astype(np.float32)).astype(np.float16)
        cpu_checksum = np.sum(cpu_output.astype(np.float32)).astype(np.float16)

        # Run once and validate
        kernel.dispatch(
        ...
        # Read thread 0's checksum for sample 0
...
def main():
    parser = argparse.ArgumentParser(description="Single Layer Forward Benchmark (WaveTangledVector + OptimalLayout)")
    ...
    print("="*70)
    print("Single Layer Forward Pass Benchmark (WaveTangledVector + OptimalLayout)")
    print("="*70)
    ...
    if args.all:
        ...
        print("\n" + "="*70)
        print("Summary: Single Layer Forward (WaveTangledVector + OptimalLayout)")
        print("="*70)
