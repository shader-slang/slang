...
ComPtr<slang::IModule>
compileShaderModuleFromFile(slang::ISession* slangSession, char const* filePath)
{
...
struct ExampleProgram : public TestBase
{
    ...
    ComPtr<gfx::IShaderProgram>
    loadComputeProgram(slang::IModule* slangModule, char const* entryPointName)
    {
    ...
    Result execute(int argc, char* argv[])
    {
        ...
        encoder
            ->bufferBarrier(printBuffer, ResourceState::UnorderedAccess, ResourceState::CopySource);
        encoder->endEncoding();
        commandBuffer->close();
