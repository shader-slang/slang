...
void Context::init(bool requireInt64Atomics)
{
...
Buffer Context::createBuffer(VkDeviceSize size, VkBufferUsageFlags usage)
{
    Buffer b = {};
    b.size = size;
...
void Context::upload(Buffer& b, const void* data, VkDeviceSize size)
{
    if (size > b.size)
...
void Context::download(Buffer& b, void* data, VkDeviceSize size)
{
    if (size > b.size)
...
ComputePipeline Context::createComputePipeline(
    const void* spirv,
    size_t spirvBytes,
...
void Context::destroyPipeline(ComputePipeline& p)
{
    if (p.pipeline)
...
VkDescriptorSet Context::allocateDescriptorSet(VkDescriptorSetLayout layout)
{
...
void Context::writeStorageBuffer(VkDescriptorSet set, uint32_t binding, const Buffer& buf)
{
