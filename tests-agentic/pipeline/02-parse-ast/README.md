...
| Token-level errors not previously reported by the lexer (here, an `@` punctuator inside a struct declaration) surface at parse time as parse errors via the DiagnosticSink. | negative | [#failure-modes](../../../docs/llm-generated/pipeline/02-parse-ast.md#failure-modes) | [`failure-mode-unrecognized-punctuator.slang`](failure-mode-unrecognized-punctuator.slang) |
| A generic declaration after a decl keyword is unambiguous; the parser collects the parameter list into a GenericDecl and continues with the inner declaration. | functional | [#generics-ambiguity](../../../docs/llm-generated/pipeline/02-parse-ast.md#generics-ambiguity) | [`generic-decl-after-keyword.slang`](generic-decl-after-keyword.slang) |
| In body position the parser attempts a generic-application parse for `<...>` after an identifier and commits if it parses cleanly, so `pickMax<int>(3, 7)` is treated as a generic call. | functional | [#generics-ambiguity](../../../docs/llm-generated/pipeline/02-parse-ast.md#generics-ambiguity) | [`generic-application-in-body.slang`](generic-application-in-body.slang) |
| When the generic-application parse does not commit cleanly the parser rolls back and parses `<` as the less-than operator, so `a < b` in a body context yields a boolean comparison Expr. | functional | [#generics-ambiguity](../../../docs/llm-generated/pipeline/02-parse-ast.md#generics-ambiguity) | [`less-than-in-body-is-comparison.slang`](less-than-in-body-is-comparison.slang) |
| Attribute parsing is modifier parsing in disguise — the `[name(args)]` token sequence becomes an `AttributeBase` modifier attached to the declaration that follows. | functional | [#modifier-parsing](../../../docs/llm-generated/pipeline/02-parse-ast.md#modifier-parsing) | [`attribute-bracket-form.slang`](attribute-bracket-form.slang) |
| Attribute parsing — `[name(args)]` — works wherever modifier parsing does; the `[unroll]` attribute attaches to a for-loop Stmt and the loop still executes correctly. | functional | [#modifier-parsing](../../../docs/llm-generated/pipeline/02-parse-ast.md#modifier-parsing) | [`attribute-on-stmt.slang`](attribute-on-stmt.slang) |
| Modifier tokens collected before a decl keyword attach to the resulting Decl through the Modifier chain rooted at ModifiableSyntaxNode; `static const int K = 21;` declares a usable compile-time-known binding. | functional | [#modifier-parsing](../../../docs/llm-generated/pipeline/02-parse-ast.md#modifier-parsing) | [`modifier-static-const-on-decl.slang`](modifier-static-const-on-decl.slang) |
| Modifier keywords like `nointerpolation` are not hard-coded in the parser but bound to syntax via the SyntaxParseInfo table (populated by populateBaseLanguageModule and the core module); the parser dispatches to the right callback by identifier lookup. | functional | [#syntax-as-declaration](../../../docs/llm-generated/pipeline/02-parse-ast.md#syntax-as-declaration) | [`modifier-keyword-from-syntax-table.slang`](modifier-keyword-from-syntax-table.slang) |
| A `for (init; cond; update) { body }` form parses into a Stmt with three distinct sub-expressions and a body block. | functional | [#the-major-node-families](../../../docs/llm-generated/pipeline/02-parse-ast.md#the-major-node-families) | [`stmt-for-loop.slang`](stmt-for-loop.slang) |
| A `struct` declaration parses into a Decl node whose members are themselves Decls; field accesses through `.` resolve to the parsed member. | functional | [#the-major-node-families](../../../docs/llm-generated/pipeline/02-parse-ast.md#the-major-node-families) | [`decl-struct-with-fields.slang`](decl-struct-with-fields.slang) |
| A `{ ... }` block parses into a Stmt that holds an ordered sequence of sub-statements, executed in source order. | functional | [#the-major-node-families](../../../docs/llm-generated/pipeline/02-parse-ast.md#the-major-node-families) | [`stmt-block-sequence.slang`](stmt-block-sequence.slang) |
| Assignment is parsed as a right-associative Expr, so `a = b = c = 7` reduces as `a = (b = (c = 7))` and all three names receive the value. | functional | [#the-major-node-families](../../../docs/llm-generated/pipeline/02-parse-ast.md#the-major-node-families) | [`expr-assign-right-associative.slang`](expr-assign-right-associative.slang) |
...
| Postfix `.` member access binds tightly and left-associatively, so `o.f.g + 1` parses as `((o.f).g) + 1`. | functional | [#the-major-node-families](../../../docs/llm-generated/pipeline/02-parse-ast.md#the-major-node-families) | [`expr-postfix-binds-tight.slang`](expr-postfix-binds-tight.slang) |
| The parser builds Expr nodes that honor operator precedence: `*` binds tighter than `+`, so `2 + 3 * 4` parses as `2 + (3 * 4)` and evaluates to 14. | functional | [#the-major-node-families](../../../docs/llm-generated/pipeline/02-parse-ast.md#the-major-node-families) | [`expr-precedence-mul-over-add.slang`](expr-precedence-mul-over-add.slang) |
| `extension <Type> { ... }` parses into an ExtensionDecl whose members become callable on the target type. | functional | [#the-major-node-families](../../../docs/llm-generated/pipeline/02-parse-ast.md#the-major-node-families) | [`decl-extension-adds-method.slang`](decl-extension-adds-method.slang) |
