# Prompt: tests-agentic/name-resolution/lookup/
...
Produce the test bundle at `tests-agentic/name-resolution/lookup/`,
anchored to
...
2. A test file per **verifiable lookup claim** in
   ...
   - **a name is filtered out by `LookupMask`** — observable through
     either a positive resolution that finds the *other* category
...
3. Coverage of the doc's structural sections. The doc's load-bearing
   ...
   - `#module-and-namespace` — re-opened namespace lookups merge;
     ...
     covers the *boundary*; this bundle covers the *merge*.)
...
`tests-agentic/name-resolution/scopes/` covers the following lookup-
...
- namespace re-opening merges members → scopes owns the positive
  ...
  can still cover the *unqualified* `using namespace` case because
