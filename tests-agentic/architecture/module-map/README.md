...
| (unspecified) | (unclassified) | `source/slang-core-module/` contains `slang-embedded-core-module.cpp` and `slang-embedded-core-module-source.cpp`. The fact that the core module *is embedded* is observable (covered by C-02); the filenames behind that embedding are not. | Not reachable via any allowed test directive. |
| (unspecified) | (unclassified) | `source/slang-glsl-module/` contains `slang-embedded-glsl-module.cpp`. Same logic as the core module — embedding is observable, filename is not. | Not reachable via any allowed test directive. |
| (unspecified) | (unclassified) | The actual `*.meta.slang` files live in `source/slang/`: `core.meta.slang`, `hlsl.meta.slang`, `diff.meta.slang`, `glsl.meta.slang`. The user-surface consequence of these files (the language they define) is visible everywhere; their file identity is not. | Not reachable via any allowed test directive. |
| (unspecified) | (unclassified) | `source/standard-modules/` contains `slang-standard-module-config.h.in` (a CMake template) and the `neural/` standard module. Build-system facts; the neural module is gated on `-experimental-feature` and the doc does not commit to a CLI shape. | Not reachable via any allowed test directive. |
| (unspecified) | (unclassified) | `prelude/slang-cpp-types-core.h` and `prelude/slang-cpp-types.h` are sub-headers included by `slang-cpp-prelude.h`. The CLI observable is the top-level `slang-cpp-prelude.h` include (covered by C-06); the inclusion of sub-headers is internal to the emit text. | Not reachable via any allowed test directive. |
| (unspecified) | (unclassified) | `prelude/slang-cpp-scalar-intrinsics.h` provides the C++ intrinsic implementations. Reached only through the `slang-cpp-prelude.h` include chain. | Not reachable via any allowed test directive. |
| (unspecified) | (unclassified) | `source/slang-llvm/`, `source/slang-glslang/`, `source/slang-dispatcher/`, `source/slang-rt/`, `source/slang-record-replay/`, `source/slang-wasm/`. The fact that slangc *can* delegate to LLVM or glslang is observable through `-target` (LLVM-backed CPU output, SPIR-V via glslang); the *existence* of these bridge directories is a build-system fact and not directly testable. | Not reachable via any allowed test directive. |
| (unspecified) | (unclassified) | That a discrepancy between this doc and the source tree should be resolved by updating `tests-agentic/_meta/manifest.yaml` and regenerating, not by hand-editing. A meta-process rule, not a runtime behavior. | Not reachable via any allowed test directive. |
| [#300-ir-pass-files](../../../docs/llm-generated/architecture/module-map.md#300-ir-pass-files) | (unclassified) | The "~300 IR-pass files" claim, plus the six category groupings (cleanup, specialization, differentiation, layout, validation, coverage instrumentation, target- specific lowering, shared utilities) with their example file prefixes. The doc explicitly delegates per-pass enumeration to the pipeline doc, and no module-map-anchored CLI observable exists for the count or grouping. | Not reachable via any allowed test directive. |
| [#api-surface-helpers](../../../docs/llm-generated/architecture/module-map.md#api-surface-helpers) | (unclassified) | The "API surface helpers" rows: `slang-api.cpp`, `slang-artifact-output-util.{h,cpp}`. These are the C++ glue behind the public API and are not directly visible from the CLI. | Not reachable via any allowed test directive. |
| [#include](../../../docs/llm-generated/architecture/module-map.md#include) | (unclassified) | The preprocessor, parser, and syntax-declaration file pairs in `source/slang/`. The user-visible effects of those subsystems (e.g. `#include` resolution, parse errors) are observable, but the filename / file-pair claim itself is not. Such behavioral claims belong in `pipeline/01-lex-preprocess` and `pipeline/02-parse`. | Not reachable via any allowed test directive. |
| [#include](../../../docs/llm-generated/architecture/module-map.md#include) | (unclassified) | `prelude/slang-hlsl-prelude.h` exists, but HLSL emit does not literally `#include` it the way CUDA / CPP do; it is conditionally consumed via `SLANG_HLSL_ENABLE_NVAPI`. The HLSL-prelude file's existence is therefore not observable by a simple FileCheck against HLSL emit. | Not reachable via any allowed test directive. |
| [#include](../../../docs/llm-generated/architecture/module-map.md#include) | (unclassified) | `prelude/slang-torch-prelude.h` exists; `-target torch` / `-target torch-binding` emit content from the prelude but do not always `#include` it by name. A FileCheck of the prelude's *contents* (`torch/extension.h`) is more brittle than the CUDA/CPP case and was not adopted. | Not reachable via any allowed test directive. |
| [#see-x-for-details](../../../docs/llm-generated/architecture/module-map.md#see-x-for-details) | (unclassified) | The doc's "see X for details" links into `cross-cutting/diagnostics.md`, `cross-cutting/targets.md`, `cross-cutting/serialization.md`, `cross-cutting/core-module.md`, `cross-cutting/ir-instructions.md`, `pipeline/05-ir-passes.md`. Navigation; each linked doc has its own bundle for any behavioral claims it makes. | Not reachable via any allowed test directive. |
| [#slang-llvm](../../../docs/llm-generated/architecture/module-map.md#slang-llvm) | (unclassified) | `prelude/slang-llvm.h` is for the `slang-llvm` integration; the LLVM/JIT path is engaged by `-target` but the prelude file itself is not in emitted text. | Not reachable via any allowed test directive. |
| [#slangc](../../../docs/llm-generated/architecture/module-map.md#slangc) | (unclassified) | `prelude/slang-cpp-host-prelude.h` is the host runtime prelude. It is not inlined into emitted text by `slangc`; it is consumed by `slang-rt` callers. No CLI observable. | Not reachable via any allowed test directive. |
| [#sourcecommit](../../../docs/llm-generated/architecture/module-map.md#sourcecommit) | (unclassified) | That the file enumeration corresponds to the recorded `source_commit` in the doc's front matter. A reproducibility contract, not slangc behavior. | Not reachable via any allowed test directive. |
| (unspecified) | api-only | The AST file inventory: `slang-ast-forward-declarations.h`, `slang-ast-base.{h,cpp}`, `slang-ast-all.h`, `slang-ast-decl.{h,cpp}`, `slang-ast-decl-ref.cpp`, `slang-ast-expr.h`, `slang-ast-stmt.h`, `slang-ast-type.{h,cpp}`, `slang-ast-modifier.{h,cpp}`, `slang-ast-val.{h,cpp}`, `slang-ast-builder.{h,cpp}`, `slang-ast-dispatch.h`, `slang-ast-iterator.h`, `slang-ast-dump.{h,cpp}`, `slang-ast-print.{h,cpp}`, `slang-ast-synthesis.{h,cpp}`, `slang-ast-natural-layout.{h,cpp}`, `slang-ast-support-types.{h,cpp}`, `slang-ast-boilerplate.cpp`. A C++ class hierarchy is not CLI-visible. | Not reachable via any allowed test directive. |
| [#irbuilder](../../../docs/llm-generated/architecture/module-map.md#irbuilder) | api-only | That `IRBuilder`, `IRInst`, `IRModule`, `Token`, `TokenKind`, `SourceManager`, `Linkage`, `Session`, `Module`, `IComponentType` exist as C++ classes / types in the cited files. C++ identity is invisible to the CLI; the relevant user-surface behaviors live in their respective subsystem bundles. | Not reachable via any allowed test directive. |
